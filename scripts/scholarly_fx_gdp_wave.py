#!/usr/bin/env python3
"""Scholarly FX OECD/FRED real GDP growth (NAEXKP01) FX differential wave (§58).

Fixed priors: FRED OECD MEI NAEXKP01*Q657S real GDP QoQ % → YoY %
(USD/GBP/JPY/CAD/AUD/CHF/NZD *Q657S; EUR=DEQ657S Germany
proxy — EZQ657S stale ~2023-01), pub_lag_months=3 + signal_lag=1m +
signal_lag_days=1. Primary high_gdp_xs (growth / economic-momentum —
high relative GDP YoY → appreciate). Scores on **GDP YoY %**. No HO
tuning. Locked sleeve untouched. Distinct from IP §42, CLI §37, BCI §39,
emp §41, UR §54, debt/GDP §29, CA/GDP §21.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED real GDP growth wave (§{SECTION}) ===")
    print(
        "FRED NAEXKP01 real GDP YoY (from Q657S QoQ; pub_lag=3) — high_gdp XS; "
        "growth / economic-momentum prior; full G10; EUR=DE proxy (EZQ657S stale ~2023-01); "
        "distinct from IP §42, CLI §37, BCI §39, emp §41, UR §54, debt/GDP §29, CA/GDP §21. "
        "Locked sleeve untouched."
    )

    try:
        lp = load_gdp_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL gdp load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_gdp_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_gdp_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED real GDP growth\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"gdp panel too thin: cols={list(lp.columns)}. "
    ...
    cov = gdp_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_gdp_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_gdp_panel.csv")
    print(
        f"gdp panel cols={list(lp.columns)} "
    ...
    try:
        ...
        print(
            f"US NAEXKP01USQ657S YoY [{us_gdp.dropna().index.min().date()}.."
            f"{us_gdp.dropna().index.max().date()}] n={us_gdp.dropna().shape[0]} "
            f"series={us_gdp.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US gdp load failed: {exc}")
        us_gdp = None
    ...
    cfg = GdpFxConfig()
    factors = gdp_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_gdp_override=us_gdp,
    )
    if not factors:
        print("FATAL: no gdp factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_gdp_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_gdp_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_gdp_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_gdp_risk_sweep.csv", index=False)
    ...
    meta = {
        ...
        "normalisation": (
            "FRED OECD MEI NAEXKP01*Q657S real GDP QoQ % → compounded YoY % "
            "(USD/GBP/JPY/CAD/AUD/CHF/NZD *Q657S; "
            "EUR=NAEXKP01DEQ657S Germany proxy — EZQ657S stale ~2023-01); "
            "XS on GDP YoY %; also 5y z / +Δ12 of YoY boarded; "
            "high_gdp primary (growth / economic-momentum → appreciate); "
            "n_long=n_short=2 on available foreign panel (full G10)"
        ),
        "eur_mapping": "NAEXKP01DEQ657S Germany proxy (EA EZQ657S stale ~2023-01)",
        "coverage_g10": "USD/EUR/GBP/JPY/AUD/CAD/CHF/NZD all mapped (full G10)",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "gdp_yoy_pct",
        "distinct_from": [
        ...
        "citations": [
            "Dahlquist & Hasseltoft (2020), JFE — economic momentum / GDP growth differentials",
            "Real GDP YoY growth as output / economic-momentum channel",
            "OECD MEI NAEXKP01*Q657S real GDP growth (QoQ→YoY %)",
            "Honesty: low GDP growth / activity-stress debtor premium",
        ],
    ...
    (REPORTS / "scholarly_fx_gdp_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED real GDP growth differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **real GDP YoY** "
        "(`NAEXKP01*Q657S` QoQ→YoY; full G10; EUR=Germany DEQ proxy) — GDP YoY XS — "
        "**distinct** from IP (§42), CLI (§37), BCI (§39), emp persons (§41), UR (§54), "
        "debt/GDP (§29), CA/GDP (§21).",
        f"**Data:** `{data_tag}` + free FRED NAEXKP01 YoY (from Q657S QoQ) after pub_lag. "
        f"EUR EA EZQ657S stale ~2023-01 → DEQ657S Germany proxy; full G10 mapped. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (quarterly GDP lag a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **real GDP YoY %**.",
        f"**Primary:** `{PRIMARY}` (long high relative GDP YoY / short low — "
        "growth / economic-momentum → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_gdp_*.csv`, "
        "`scholarly_fx_gdp_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_gdp_wave.md").write_text("\n".join(lines))
