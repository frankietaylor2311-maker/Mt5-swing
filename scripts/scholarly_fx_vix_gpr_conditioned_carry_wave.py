...
by VIX (Menkhoff global FX-vol proxy) and aggregate Caldara–Iacoviello GPR.
Literature: Lustig–Roussanov–Verdelhan (2011) + Menkhoff et al. (2012 JF) +
Caldara–Iacoviello (2022) / Liu–Zhang (2024).

Fixed priors: VIX/GPR bar_lag=1 + signal_lag=1d on trailing z (z_window=252,
min_periods=60), gate z≤0 / cool at z≥1 (§71/§72 symmetry — lit GPR often
1.5; documented), n_long=n_short=2, costs 1.5 bps/side.
Primary ``carry_low_vix``. Honesty: ``carry_raw`` + ``carry_high_vix``.
...
quest_fred_carry_gpr_study, gpr_regime, AI-GPR §25, country-GPR §45.
...
PREFIX = "scholarly_fx_vix_gpr_conditioned_carry"
...
def write_locked_verify() -> dict:
    ...
    lines = [
        f"# One-pct month quest — locked_verify_vix_gpr_conditioned_carry",
        "",
    ...
    lines += [
        "",
        "Config unmodified vs §72 vix_gpr_conditioned_soft / §68 cip_conditioned_carry "
        "— canonical windowed PASS.",
        "",
    ]
    (REPORTS / "quest_locked_verify_vix_gpr_conditioned_carry.md").write_text(
        "\n".join(lines)
    )
    log = (
        f"Locked sleeve re-verify (§{SECTION} vix_gpr_conditioned_carry): "
        f"{LOCKED_CFG.relative_to(ROOT)}\n"
        f"basket_tag={LOCKED_TAG} untouched\n"
        f"sha256={sha}\n"
        f"Canonical windowed PASS (same as §72 / §68 — config unmodified):\n"
        ...
        f"Wrote {REPORTS / 'quest_locked_verify_vix_gpr_conditioned_carry.md'}\n"
    )
    (REPORTS / "quest_locked_verify_vix_gpr_conditioned_carry.log").write_text(log)
    print(log, end="")
...
def main() -> int:
    ...
    print(
        f"=== scholarly FX VIX/GPR-conditioned Lustig–Verdelhan carry wave "
        f"(§{SECTION}) ==="
    )
    print(
        f"VIX + aggregate GPR stress gate/cool on IR3M carry — primary {PRIMARY}; "
        "Lustig–Roussanov–Verdelhan + Menkhoff et al. (2012 JF) + "
        "Caldara–Iacoviello / Liu–Zhang; "
        "distinct from fwd_carry §19, funding_liq §20, IG OAS §36, ACM TP §44, "
        "CIP XS §67, CIP×carry §68, soft EW §66, soft CIP §69, capital-sleeve "
        "§53/§70, CIP-conditioned soft §71, VIX/GPR-conditioned soft §72, "
        "combo §8, gpr_regime, AI-GPR §25, country-GPR §45. "
    ...
    try:
        ...
        (REPORTS / f"{PREFIX}_wave.md").write_text(
            f"# Scholarly FX: VIX/GPR-conditioned carry\n\n**FAILED:** {exc}\n"
        )
        return 1
    ...
    factors = vix_gpr_conditioned_carry_factor_returns(
        pair_ret, rates=rates, vix=vix, gpr=gpr, cfg=cfg
    )
    if not factors or PRIMARY not in factors:
        print("FATAL: no vix_gpr_conditioned_carry factors produced")
        return 1
    ...
    # Save stress series coverage for audit
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "menkhoff_vix_caldara_gpr_conditioned_lustig_verdelhan_carry",
        "data_tag": data_tag,
        "vix_bar_lag": 1,
        "gpr_bar_lag": 1,
        ...
        "primary": PRIMARY,
        "primary_sign_prior": (
            "trade Lustig–Verdelhan cash-rate carry only when lagged VIX z ≤ 0 "
            "— carry crashes when global FX-vol elevated (Menkhoff et al. 2012 JF); "
            "GPR companions parallel Caldara–Iacoviello / Liu–Zhang"
        ),
        "n_factors": len(board),
        "soft_nw_pos": soft,
        "hard_nw_pos": hard,
        ...
        "data_caveat": (
            ...
            "VIX/GPR gate z≤0 / cool z≥1 fixed a priori (§71/§72 symmetry; "
    ...
    lines = [
        f"# Scholarly FX: VIX/GPR-conditioned Lustig–Verdelhan carry wave (§{SECTION})",
        "",
        "**Path:** Menkhoff **VIX** (global FX-vol proxy) + Caldara–Iacoviello "
        "**aggregate GPR** stress × Lustig–Verdelhan IR3M carry gate/cool — "
        "**distinct** from fwd_carry (§19), funding_liq (§20), IG OAS (§36), "
        "ACM TP (§44), CIP XS (§67), CIP×carry (§68), soft EW (§66), soft CIP (§69), "
        "capital-sleeve (§53/§70), CIP-conditioned soft (§71), VIX/GPR-conditioned "
        "soft (§72), combo (§8), gpr_regime / AI-GPR (§25) / country-GPR (§45).",
        ...
        f"`n_long=n_short=2`. **PIT:** VIX/GPR bar_lag=1 + signal_lag={cfg.signal_lag}d "
        f"on trailing z ({cfg.z_window}d); carry signal_lag={cfg.carry_signal_lag}d.",
        f"**Primary:** `{PRIMARY}` (trade carry only when lagged VIX z ≤ 0). "
        "Honesty `carry_raw` + `carry_high_vix`. Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "Gate z≤0 / cool z≥1 fixed (§71/§72 symmetry; lit GPR often 1.5 — "
        "documented, not HO-tuned). z_window=252 daily (NOT monthly CIP 60).",
        "",
        f"Artifacts: `reports/{PREFIX}_*.csv`, "
        f"`{PREFIX}_meta.json`, `quest_locked_verify_vix_gpr_conditioned_carry.md`.",
        "",
    ]
    (REPORTS / f"{PREFIX}_wave.md").write_text("\n".join(lines))
