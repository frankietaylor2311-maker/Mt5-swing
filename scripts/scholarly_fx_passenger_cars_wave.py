#!/usr/bin/env python3
"""Scholarly FX OECD/FRED passenger-car registrations (SLRTCR03) FX differential wave (§60).

Fixed priors: FRED OECD MEI SLRTCR03 passenger-car registrations YoY %
(USD/GBP/JPY/CAD/AUD *GYSAM; EUR=ESPSLRTCR03GYSAM Spain proxy —
DEU/FRA 404, EA19 stale ~2018-12; NZD/CHF unmapped stale),
pub_lag_months=2 + signal_lag=1m + signal_lag_days=1. Primary high_cars_xs
(growth-channel / durable auto demand). Scores on **passenger-car
registrations YoY %**. No HO tuning. Locked sleeve untouched. Distinct from
retail-sales §46, building-permits §43, construction §59, IP §42.

Fall-through: housing starts/completions ODCNPI02/04 404 + US-only HOUST;
mfg orders ODMN* 404 + US-only AMTMNO — thin foreign panels.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED passenger-car registrations wave (§{SECTION}) ===")
    print(
        "FRED SLRTCR03 passenger-car regs YoY (GYSAM; pub_lag=2) — high_cars XS; "
        "growth-channel / durable auto-demand prior; EUR=ESP Spain proxy; "
        "NZD/CHF unmapped; distinct from retail §46, permits §43, construction §59, IP §42. "
        "Locked sleeve untouched. Fall-through: housing starts + mfg orders thin/404."
    )

    try:
        lp = load_passenger_cars_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL passenger_cars load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_passenger_cars_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_passenger_cars_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED passenger-car registrations\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"passenger_cars panel too thin: cols={list(lp.columns)}. "
    ...
    cov = passenger_cars_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_passenger_cars_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_passenger_cars_panel.csv")
    print(
        f"passenger_cars panel cols={list(lp.columns)} "
    ...
    try:
        ...
        print(
            f"US USASLRTCR03GYSAM YoY [{us_cars.dropna().index.min().date()}.."
            f"{us_cars.dropna().index.max().date()}] n={us_cars.dropna().shape[0]} "
            f"series={us_cars.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US passenger_cars load failed: {exc}")
        us_cars = None
    ...
    factors = passenger_cars_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_cars_override=us_cars,
    )
    if not factors:
        print("FATAL: no passenger_cars factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_passenger_cars_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_passenger_cars_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_passenger_cars_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_passenger_cars_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_passenger_cars_slrtcr03",
        "data_tag": data_tag,
        ...
        "score_unit": "passenger_cars_yoy_pct",
        "z_low": cfg.z_low,
        ...
        "normalisation": (
            "FRED OECD MEI SLRTCR03 passenger-car regs YoY % "
            "(USD/GBP/JPY/CAD/AUD *GYSAM; EUR=ESPSLRTCR03GYSAM Spain proxy; "
            "NZD/CHF unmapped stale); "
            "XS on car-regs YoY %; also 5y z / +Δ12 of YoY boarded; "
            "high_cars primary (growth-channel / durable auto demand → appreciate); "
            "n_long=n_short=2 on available foreign panel (EUR/GBP/JPY/CAD/AUD)"
        ),
        "eur_mapping": "ESPSLRTCR03GYSAM Spain proxy (DEU/FRA 404; EA19 stale ~2018-12)",
        "coverage_g10": "USD/EUR/GBP/JPY/AUD/CAD mapped; NZD/CHF unmapped stale",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "passenger_cars_yoy_pct",
        "distinct_from": [
            "retail_sales_46",
            "building_permits_43",
        ...
        "citations": [
            ...
            "Growth-channel / Taylor-rule FX applied to passenger-car demand (durable goods)",
            "OECD MEI SLRTCR03 passenger-car registrations YoY (GYSAM)",
            "Honesty: low car-regs growth / demand-stress debtor premium",
            "Fall-through: housing starts ODCNPI02/04 404 thin; mfg orders ODMN* 404 thin",
        ],
        "fallthrough_from": [
    ...
    (REPORTS / "scholarly_fx_passenger_cars_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED passenger-car registrations differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **passenger-car registrations YoY** "
        "(`SLRTCR03` GYSAM; EUR=Spain ESP proxy; NZD/CHF unmapped) — car-regs YoY XS — "
        "**distinct** from retail-sales (§46), building-permits (§43), construction (§59), "
        "IP (§42), CLI (§37), BCI (§39).",
        f"**Data:** `{data_tag}` + free FRED SLRTCR03 YoY after pub_lag. "
        f"EUR=ESPSLRTCR03GYSAM Spain proxy; NZD/CHF unmapped stale; "
        f"USD/GBP/JPY/CAD/AUD live. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (auto/retail ~1–2m lag a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **passenger-car registrations YoY %**.",
        f"**Primary:** `{PRIMARY}` (long high relative car-regs YoY / short low — "
        "growth-channel / durable auto demand → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_passenger_cars_*.csv`, "
        "`scholarly_fx_passenger_cars_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_passenger_cars_wave.md").write_text("\n".join(lines))
