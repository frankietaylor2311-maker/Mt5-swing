#!/usr/bin/env python3
"""Scholarly FX term-structure / yield-curve wave (OECD LT − short slope).

Fixed priors: Chen–Tsang / Ang–Chen style slope differentials vs USD,
LT-level sort, slope×carry interaction, secondary UIP (IRSTCI + IR3M).
pub_lag=1m + signal_lag=1m; n_long=n_short=2; costs 1.5 bps/side. No HO tuning.
...
from mt5_swing.data.fred_yields import (
    download_default_yield_panel,
    load_curve_panels,
    load_ir3m_panel,
    yield_coverage_table,
)
from mt5_swing.data.loader import load_ohlc_csv
from mt5_swing.strategies.yield_curve_fx import YieldCurveFxConfig, yield_curve_factor_returns
...
def main() -> int:
    ...
    print("=== scholarly FX term-structure / yield-curve wave ===")

    try:
        download_default_yield_panel(force=False)
        panels = load_curve_panels(pub_lag_months=PUB_LAG, download=True)
        cov = yield_coverage_table(download=False)
        cov.to_csv(REPORTS / "scholarly_fx_curve_coverage.csv", index=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL yield load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_curve_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_curve_wave.md").write_text(
            f"# Scholarly FX: yield-curve / term-structure\n\n**FAILED:** {exc}\n"
        )
        return 1

    ir3m_diff = None
    try:
        ir3m = load_ir3m_panel(pub_lag_months=PUB_LAG, download=True)
        if "USD" in ir3m.columns:
            ir3m_diff = ir3m.drop(columns=["USD"]).sub(ir3m["USD"], axis=0)
            print(f"IR3M panel: {list(ir3m.columns)}")
        else:
            print("WARN IR3M missing USD — skip uip_ir3m_xs")
    except Exception as exc:  # noqa: BLE001
        print(f"WARN IR3M load failed (UIP secondary skipped): {exc}")

    pair_ret, pair_close, data_tag = prefer_ftmo_or_yahoo(USD_MAJORS)
    print(f"FX panel: {list(pair_ret.columns)} tag={data_tag}")
    print(
        f"lt={list(panels['lt'].columns)} slope_diff cols={list(panels['slope_diff'].columns)} "
        f"lt_notes={panels['lt'].attrs.get('notes')}"
    )

    # Persist panels for audit
    panels["slope_diff"].to_csv(REPORTS / "scholarly_fx_curve_slope_diff.csv")
    panels["lt_diff"].to_csv(REPORTS / "scholarly_fx_curve_lt_diff.csv")
    panels["st_diff"].to_csv(REPORTS / "scholarly_fx_curve_st_diff.csv")

    cfg = YieldCurveFxConfig(signal_lag=SIGNAL_LAG)
    factors = yield_curve_factor_returns(
        pair_ret,
        panels["slope_diff"],
        panels["lt_diff"],
        panels["st_diff"],
    ...
    if not factors:
        print("FATAL: no curve factors produced")
        return 1

    summaries = [factor_summary(r, name) for name, r in factors.items()]
    pd.DataFrame(summaries).to_csv(REPORTS / "scholarly_fx_curve_factor_summary.csv", index=False)
    for name, r in factors.items():
        monthly_returns(r).to_csv(REPORTS / f"scholarly_fx_curve_{name}_monthly.csv", header=[name])
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_curve_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_curve_risk_sweep.csv", index=False)
    ...
    meta = {
        ...
        "primary": PRIMARY,
        "lt_cols": list(panels["lt"].columns),
        "slope_diff_cols": list(panels["slope_diff"].columns),
        "lt_notes": panels["lt"].attrs.get("notes", {}),
        "ir3m_available": ir3m_diff is not None,
        ...
        "refs": [
            "Chen & Tsang (2013) — relative yield-curve factors and FX",
            "Ang & Chen — yield-curve / carry interactions in currency risk premia",
            "Lustig, Stathopoulos & Verdelhan (2019) — term structure of currency risk premia",
            "Fama (1984) — forward-premium / UIP puzzle (secondary uip_* legs)",
            "OECD IRLTLT01* LT govt − IRSTCI01* immediate short = slope",
        ],
    }
    (REPORTS / "scholarly_fx_curve_meta.json").write_text(json.dumps(meta, indent=2, default=str))

    lines: list[str] = []
    lines.append("# Scholarly FX: term-structure / yield-curve factors\n\n")
    lines.append(
        "**Prior (fixed):** Relative yield-curve slope and level vs USD predict FX "
        "(Chen–Tsang / Ang–Chen style); slope×carry interaction; secondary UIP on "
        f"short / 3m differentials. pub_lag={PUB_LAG}m + signal_lag={SIGNAL_LAG}m; "
        f"n_long=n_short={cfg.n_long}; costs={cfg.cost_bps_side} bps/side. No HO tuning.\n\n"
    )
    lines.append(
        f"**Data:** `{data_tag}` D1 FX + FRED OECD LT (`IRLTLT01*`) − immediate "
        f"(`IRSTCI01*`) slope. LT cols: {', '.join(panels['lt'].columns)}. "
        f"IR3M secondary: {'yes' if ir3m_diff is not None else 'no'}.\n\n"
    )
    ...
    lines.append(
        "### Honest gaps\n\n"
        "- OECD LT − immediate short is a **simple slope**, not Nelson–Siegel / Svensson "
        "level-slope-curvature from a full zero curve.\n"
    ...
    (REPORTS / "scholarly_fx_curve_wave.md").write_text("".join(lines))
    print(f"wrote {REPORTS / 'scholarly_fx_curve_wave.md'}")
