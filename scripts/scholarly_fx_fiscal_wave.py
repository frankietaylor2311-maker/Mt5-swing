#!/usr/bin/env python3
"""Scholarly FX fiscal-balance / government-budget differential wave (twin deficits).

Fixed priors: IMF WEO GGNLBA via FRED, pub_lag_months=15 + signal_lag=1m.
Optional CA surplus blend (scholarly only). No HO tuning. Locked sleeve untouched.
"""
...
def main() -> int:
    ...
    print("=== scholarly FX fiscal-balance / government-budget wave ===")
    print(
        "Twin deficits / fiscal sustainability — IMF WEO GGNLBA via FRED; "
        "distinct from CA (§21), CB-BS, macro-diff. Locked sleeve untouched."
    )

    try:
        fiscal = load_fiscal_gdp_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL fiscal load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_fiscal_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_fiscal_wave.md").write_text(
            f"# Scholarly FX: fiscal-balance / government budget\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = fiscal_coverage(fiscal)
    cov.to_csv(REPORTS / "scholarly_fx_fiscal_coverage.csv", index=False)
    fiscal.to_csv(REPORTS / "scholarly_fx_fiscal_gdp_panel.csv")
    print(
        f"Fiscal panel cols={list(fiscal.columns)} "
        f"[{fiscal.dropna(how='all').index.min().date()}.."
        f"{fiscal.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))

    try:
        us_mts = load_us_mts_surplus(download=True, force=False)
        print(
            f"US MTS surplus [{us_mts.dropna().index.min().date()}.."
            f"{us_mts.dropna().index.max().date()}] n={us_mts.dropna().shape[0]}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN MTS load failed: {exc}")
        us_mts = None
    ...
    if ca is not None:
        try:
            ...
            ca_surplus_ret = ca_factors.get("ca_surplus_xs")
        except Exception as exc:  # noqa: BLE001
    ...
    factors = fiscal_balance_factor_returns(
        ...
        cfg=cfg,
        us_mts=us_mts,
        ca_surplus_returns=ca_surplus_ret,
    )
    if not factors:
        print("FATAL: no fiscal factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_fiscal_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_fiscal_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_fiscal_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_fiscal_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "path": "fiscal_balance_government_budget",
        "data_tag": data_tag,
        ...
        "citations": [
            "Abell (1990), Twin Deficits during the 1980s, JMacro",
            "Corsetti, Dedola & Leduc on fiscal shocks and exchange rates",
            "Dai & Philippon on fiscal deficits and risk premia",
            "Twin deficits / fiscal sustainability FX channel (survey literature)",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": fiscal.attrs.get("unmapped", []),
    }
    (REPORTS / "scholarly_fx_fiscal_meta.json").write_text(
    ...
    lines = [
        ...
        "",
        "**Path:** Twin deficits / fiscal sustainability — IMF WEO general-government "
        "net lending/borrowing (% GDP) via FRED — **distinct** from CA (§21), CB-BS, "
        "macro-diff, PPP/BS.",
        f"**Data:** `{data_tag}` + free FRED `GGNLBA*188N` (+ US MTS `MTSDS133FMS`). "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (~April Y+1 for year-Y) + "
        f"signal_lag={cfg.signal_lag}m + 1d weight lag.",
        f"**Primary:** `{PRIMARY}` (long surplus / short deficit). "
