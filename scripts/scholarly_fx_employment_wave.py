#!/usr/bin/env python3
"""Scholarly FX employment-growth (OECD MEI LFEMTTTT) differential wave (§41).

Fixed priors: FRED OECD MEI LFEMTTTT employment persons (monthly/quarterly →
monthly after pub_lag_months=3), + signal_lag_days=1 (no extra month lag).
Primary high_emp_xs (high relative emp YoY → appreciate). Scores on **YoY %**
of levels. No HO tuning. Locked sleeve untouched.
...
Retail-sales SARTMISMEI already probed stale — not re-run.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX Employment-growth (LFEMTTTT) wave (§{SECTION}) ===")
    print(
        "OECD MEI LFEMTTTT employment persons via FRED (monthly/quarterly → monthly after "
        "pub_lag=3) — emp YoY XS; distinct from macro-diff UR, "
        "OECD CLI/CCI/BCI, WUI, EPU/TPU, IG OAS. Locked sleeve untouched. SARTMISMEI skipped."
    )

    try:
        emp = load_employment_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL employment load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_employment_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_employment_wave.md").write_text(
            f"# Scholarly FX: Employment-growth\n\n**FAILED:** {exc}\n"
        )
        return 1

    if emp.empty or emp.shape[1] < 4:
        msg = (
            f"Employment panel too thin: cols={list(emp.columns)}. "
    ...
    cov = employment_coverage(emp)
    cov.to_csv(REPORTS / "scholarly_fx_employment_coverage.csv", index=False)
    emp.to_csv(REPORTS / "scholarly_fx_employment_panel.csv")
    print(
        f"Emp panel cols={list(emp.columns)} "
        f"[{emp.dropna(how='all').index.min().date()}.."
        f"{emp.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))
    print(f"Series source note: {emp.attrs.get('source')}")
    print(f"Score basis: {emp.attrs.get('score_basis')} unit={emp.attrs.get('unit')}")
    print(f"Failed/alt documented: {emp.attrs.get('failed_or_alt')}")

    us_emp = None
    try:
        us_emp = load_us_employment(download=True, force=False)
        print(
            f"US LFEMTTTTUSM647S [{us_emp.dropna().index.min().date()}.."
            f"{us_emp.dropna().index.max().date()}] n={us_emp.dropna().shape[0]} "
            f"series={us_emp.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US employment load failed: {exc}")
        us_emp = None
    ...
    factors = employment_factor_returns(
        pair_ret,
        emp,
        cfg=cfg,
        us_emp_override=us_emp,
    )
    if not factors:
        print("FATAL: no employment factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_employment_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_employment_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_employment_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_employment_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_employment_lfemtttt",
        "data_tag": data_tag,
        ...
        "normalisation": (
            "OECD MEI LFEMTTTT employment persons levels (monthly + quarterly → "
            "monthly ffill after pub_lag=3); XS on YoY % of levels; also 5y z / "
            "Δ12 of YoY boarded; high-emp primary (labour strength → appreciate); "
            "n_long=n_short=2 full G10 foreign"
        ),
        "eur_mapping": "LFEMTTTTDEQ647S Germany proxy (EZ Q647S stale 2022-10; FR/IT/ES Q647S alts)",
        "coverage_g10": "full G10 mapped (USD/JPY/CAD/AUD monthly M647S; EUR/GBP/NZD/CHF quarterly Q647S)",
        "series_map": emp.attrs.get("series_map", {}),
        "score_basis": "yoy_pct_of_levels",
        "distinct_from": [
            ...
            "house_price_35",
            "equity_diff",
            "retail_sales_sartmismei_stale",
        ],
        "citations": [
            "Dahlquist–Hasseltoft-style macro–FX differentials applied to labour / employment growth",
            "Labour-market strength → subsequent FX appreciation prior",
            "Honesty: low emp / labour-stress debtor premium",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": emp.attrs.get("unmapped", []),
        "failed_or_alt": emp.attrs.get("failed_or_alt", {}),
        "soft_best": soft_best,
        "primary_sweep": prim_sweep.iloc[0].to_dict() if len(prim_sweep) else None,
    }
    (REPORTS / "scholarly_fx_employment_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: Employment-growth (LFEMTTTT) differential wave (§{SECTION})",
        "",
        "**Path:** OECD MEI employment persons via FRED (`LFEMTTTT*M647S` / "
        "`*Q647S`) — emp YoY XS — **distinct** from macro-diff UR/CPI/IP, "
        "OECD CLI (§37) / CCI (§38) / BCI (§39), WUI (§40), EPU/TPU, IG OAS (§36), "
        "CA/TB, fiscal/debt, BIS REER/credit, reserves, house-price, money-growth, "
        "equity-diff. Retail-sales SARTMISMEI stale — skipped.",
        f"**Data:** `{data_tag}` + free FRED LFEMTTTT persons (monthly + quarterly → "
        f"monthly after pub_lag). EUR = `LFEMTTTTDEQ647S` (Germany proxy; EZ stale). "
        f"Full G10 mapped. `n_long=n_short=2` a priori. **PIT:** "
        f"pub_lag_months={PUB_LAG_M} (labour a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **YoY % of levels**.",
        f"**Primary:** `{PRIMARY}` (long high relative emp YoY / short low — "
        "labour strength → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_employment_*.csv`, "
        "`scholarly_fx_employment_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_employment_wave.md").write_text("\n".join(lines))
