#!/usr/bin/env python3
"""Causal trailing downside-vol scale consistency wave on locked sleeve.
...
LOOKBACKS = [42, 63, 84]
TARGETS = [0.004, 0.005, 0.006, 0.008]
...
def tag_dd(lb: int, tgt: float) -> str:
    return f"ddown_lb{lb}_t{tgt}"
...
def pick_ddown_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for lb, tgt in itertools.product(LOOKBACKS, TARGETS):
        params = {"lookback_days": int(lb), "target_ddown": float(tgt)}
        st = cor.stats_from_port(
            apply_trailing_downside_vol_scale(
                port24,
                lookback_days=params["lookback_days"],
                target_ddown=params["target_ddown"],
                cool_floor=0.35,
                lo=0.25,
            )
        )
    ...
    seen = set()
    for item in ranked:
        key = (item[1]["lookback_days"], round(item[1]["target_ddown"], 4))
        if key in seen:
            continue
        seen.add(key)
        uniq.append(item)
...
def pick_ddown_on_transformed(port24_xf, base_st_xf):
    best = None
    for lb, tgt in itertools.product(LOOKBACKS, TARGETS):
        params = {"lookback_days": int(lb), "target_ddown": float(tgt)}
        st = cor.stats_from_port(
...
def main() -> None:
    ...
    # --- ddown alone: full grid on locked (12) ---
    ddown_soft_ports = {}
    for lb, tgt in itertools.product(LOOKBACKS, TARGETS):
        params = {"lookback_days": int(lb), "target_ddown": float(tgt)}
        ports = apply_ports(
        ...
        name = f"lock+{tag_dd(lb, tgt)}"
        row = cor.eval_idea(name, "ddown", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"ddown: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            ddown_soft_ports[name] = ports
    ...
    # --- wrebal + nested ddown on 2024 ---
    soft_structures = {}
    w24 = wrebal_ports.get("2024")
    if w24 is not None:
        wst = cor.stats_from_port(w24)
        picked = pick_ddown_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ...
            name = f"{tag_w(frozen_w)}+{tag_dd(params['lookback_days'], params['target_ddown'])}_frozen"
            row = cor.eval_idea(name, "frozen_wrebal_ddown", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"wrebal+ddown: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                soft_structures[name] = ports

    # --- wrebal + mtd + nested ddown ---
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        st24 = cor.stats_from_port(p24)
        picked = pick_ddown_on_transformed(p24, st24)
        ...
        dports = apply_ports(
            apply_trailing_downside_vol_scale,
            ports,
            lookback_days=dparams["lookback_days"],
            target_ddown=dparams["target_ddown"],
            cool_floor=0.35,
            lo=0.25,
        )
        iname = f"{name}+{tag_dd(dparams['lookback_days'], dparams['target_ddown'])}"
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_ddown", dports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+ddown: {iname} soft={row.soft_pass} note={row.note} "
            f"HO_pos={row.pos_holdout*100:.0f}%",
            flush=True,
        )
        if row.soft_pass:
            soft_structures[iname] = dports
        # Keep mtd-only softs as blend targets too
        soft_structures[name] = ports

    # Also keep ddown-alone softs for blend (cap)
    for n, p in list(ddown_soft_ports.items())[:2]:
        soft_structures[n] = p

    # --- IS return blends: locked vs best soft-structure (α nested on 2024) ---
    # Prefer wrebal+mtd / wrebal+mtd+ddown softs; cap blend targets
    blend_targets = []
    ...
    # Cap board display if somehow huge (should be ~30-40)
    ...
    out_csv = ROOT / "reports" / "quest_ddown_vol_board.csv"
    out_md = ROOT / "reports" / "quest_ddown_vol_consistency.md"
    out_json = ROOT / "configs" / "quest_ddown_vol_selected.json"
    ...
    selected = {
        "wave": "causal_trailing_downside_vol_scale",
        "data_source": ds,
