...
Fixed priors: FRED DFII10 / T10YIE + foreign LT−CPI YoY proxies.
Daily pub_lag=1d / monthly pub_lag=1m + signal_lag. No HO tuning.
Locked sleeve untouched. Distinct from nominal yield-curve / PPP / CB-BS.
"""
...
from mt5_swing.strategies.real_rate_fx import RealRateFxConfig, real_rate_factor_returns
...
INITIAL = 100_000.0
PRIMARY = "us_real_usd"
PUB_LAG_D = DEFAULT_DAILY_PUB_LAG_DAYS
...
def main() -> int:
    ...
    print("=== scholarly FX real-rate / breakeven differential wave ===")
    print(
        "Frankel / Meese–Rogoff / Dahlquist–Hasseltoft — DFII10 + T10YIE + "
        "foreign LT−CPI proxies; distinct from nominal curve, PPP, CB-BS. "
        "Locked sleeve untouched."
    )

    try:
        panels = load_real_rate_panels(
            download=True,
            daily_pub_lag_days=PUB_LAG_D,
            monthly_pub_lag_months=PUB_LAG_M,
        )
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL real-rate load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_real_rate_meta.json").write_text(
            json.dumps(meta, indent=2)
        )
        (REPORTS / "scholarly_fx_real_rate_wave.md").write_text(
            f"# Scholarly FX: real-rate / breakeven\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = real_rate_coverage(panels)
    cov.to_csv(REPORTS / "scholarly_fx_real_rate_coverage.csv", index=False)
    panels["rr_diff_vs_usd"].to_csv(REPORTS / "scholarly_fx_real_rate_rr_diff.csv")
    panels["real_proxy_m"].to_csv(REPORTS / "scholarly_fx_real_rate_proxy_panel.csv")
    panels["us_real_d"].to_csv(
        REPORTS / "scholarly_fx_real_rate_dfii10.csv", header=["DFII10"]
    )
    panels["us_be_d"].to_csv(
        REPORTS / "scholarly_fx_real_rate_t10yie.csv", header=["T10YIE"]
    )
    print(
        f"US real [{panels['us_real_d'].dropna().index.min().date()}.."
        f"{panels['us_real_d'].dropna().index.max().date()}] "
        f"BE same; proxy cols={list(panels['real_proxy_m'].columns)} "
        f"pub_lag_d={PUB_LAG_D} pub_lag_m={PUB_LAG_M}"
    )
    ...
    factors = real_rate_factor_returns(
        pair_ret,
        us_real=panels["us_real_d"],
        us_be=panels["us_be_d"],
        rr_diff=panels["rr_diff_vs_usd"],
        cfg=cfg,
    )
    if not factors:
        print("FATAL: no real-rate factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_real_rate_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_real_rate_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_real_rate_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_real_rate_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "path": "real_rate_breakeven_differential",
        "data_tag": data_tag,
        "daily_pub_lag_days": PUB_LAG_D,
        "monthly_pub_lag_months": PUB_LAG_M,
        ...
        "foreign_real_honest_note": (
            "Foreign real rates = OECD LT govt − CPI YoY proxies; "
            "not true TIPS/linker yields where unavailable on free FRED"
        ),
        "distinct_from": [
            "yield_curve_nominal_slope_LT_ST",
            "PPP_real_FX_CPI_levels",
            "Balassa_Samuelson",
            "cb_balance_sheet_QE",
            "macro_diff_CPI_IP_UR",
            "funding_liquidity_NFCI",
        ],
        "citations": [
            "Frankel (1979), On the Mark: Real Interest Differentials, AER",
            "Meese & Rogoff (1988), Was It Real? Exchange Rate-Interest Differential, JF",
            "Dahlquist & Hasseltoft (2013), International Bond Risk Premia, JIE",
            "Lustig, Stathopoulos & Verdelhan (2019), Term Structure of Currency Carry Trade Risk Premia, JF",
            "Hofmann, Shim & Shin (BIS), bond risk premia / real rates and FX",
        ],
    ...
    lines = [
        "# Scholarly FX: real-rate / breakeven differential wave",
        "",
        "**Path:** Frankel / Meese–Rogoff real interest differential + US TIPS/BE "
        "state — **distinct** from nominal yield-curve (§16), PPP/CPI real FX, "
        "and CB-BS QE (§22).",
        f"**Data:** `{data_tag}` + free FRED `DFII10` / `T10YIE` / `DGS10` + "
        f"OECD LT − CPI YoY proxies. **PIT:** daily pub_lag={PUB_LAG_D}d, "
        f"monthly pub_lag={PUB_LAG_M}m + signal_lag={cfg.signal_lag_days}d / "
        f"{cfg.signal_lag_months}m.",
        f"**Primary:** `{PRIMARY}` (high US real → long USD). "
        "Foreign real = LT−CPI proxy (not linkers). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_real_rate_*.csv`, "
        "`scholarly_fx_real_rate_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_real_rate_wave.md").write_text("\n".join(lines))
