#!/usr/bin/env python3
"""Causal rolling skew cool consistency wave on locked sleeve.

Distinct from apply_rolling_upside_ratio_cool (RMS ratio),
apply_rolling_burst_cool (single fat day), apply_rolling_return_pctile_cool
(return level), weekly/month mean cool overlays.

Lag-1 rolling Fisher/sample skew of daily equity returns; if skew >=
skew_thresh → cool_scale else 1.0. Clip [lo, 1], hi<=1.
...
def tag_cool(lb: int, thr: float, cool: float) -> str:
    return f"skc_lb{lb}_t{thr}_c{cool}"
...
def _cool_grid():
    return itertools.product(LOOKBACKS, SKEW_THRESH, COOL_SCALES)
...
def pick_cool_2024(port24, base_st24, top_k: int = 3):
    ...
    for lb, thr, cool in _cool_grid():
        params = {
            ...
            "skew_thresh": float(thr),
            "cool_scale": float(cool),
        }
        st = cor.stats_from_port(
            apply_rolling_skew_cool(port24, **params, lo=0.25)
        )
    ...
    for item in ranked:
        key = (
            item[1]["lookback_days"],
            round(item[1]["skew_thresh"], 4),
...
def pick_cool_on_transformed(port24_xf, base_st_xf):
    ...
    for lb, thr, cool in _cool_grid():
        ...
        st = cor.stats_from_port(
            apply_rolling_skew_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    sparse_cool = list(itertools.product(LOOKBACKS, [1.0], COOL_SCALES))
    for lb, thr, cool in sparse_cool:
        ...
        ports = apply_ports(apply_rolling_skew_cool, base_ports, **params, lo=0.25)
        name = f"lock+{tag_cool(lb, thr, cool)}"
        row = cor.eval_idea(name, "skew_cool", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"skew_cool: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    # Nest top-k skew cool alone on locked (2024-only pick → board)
    if base24 is not None and base_st24 is not None:
        for sc, params, _ in pick_cool_2024(base24, base_st24, top_k=3):
            ports = apply_ports(
                apply_rolling_skew_cool, base_ports, **params, lo=0.25
            )
            name = (
                f"lock+{tag_cool(params['lookback_days'], params['skew_thresh'], params['cool_scale'])}"
                f"_nested"
            )
            row = cor.eval_idea(
                name, "skew_cool_nested", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"skew_cool_nested: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    # lock+mtd then nest skew (a few rows)
    lock_mtd_ports = {}
    for mtd in PRIOR_MTD:
        ports = apply_ports(
            apply_mtd_gain_clip, base_ports, tau=mtd["tau"], after_clip=mtd["after"]
        )
        name = f"lock+{tag_mtd(mtd)}"
        row = cor.eval_idea(name, "lock_mtd", ports, is_labels, conf_labels)
        board.append(row)
        lock_mtd_ports[name] = (mtd, ports)
        print(
            f"lock+mtd: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
            flush=True,
        )

    for name, (mtd, ports) in list(lock_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        ...
        _, cparams, _ = picked
        lports = apply_ports(
            apply_rolling_skew_cool, ports, **cparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_cool(cparams['lookback_days'], cparams['skew_thresh'], cparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "lock_mtd_skew_cool", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"lock+mtd+skew: {iname} soft={row.soft_pass} note={row.note} "
            f"HO_pos={row.pos_holdout*100:.0f}% top3={row.top3_holdout*100:.0f}%",
            flush=True,
        )
        if row.soft_pass:
            cool_soft_ports[iname] = lports
    ...
    if w24 is not None:
        ...
        for sc, params, _ in pick_cool_2024(w24, wst, top_k=2):
            ports = apply_ports(
                apply_rolling_skew_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_cool(params['lookback_days'], params['skew_thresh'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(
                name, "frozen_wrebal_skew_cool", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"wrebal+skew: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_skew_cool", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+skew: {iname} soft={row.soft_pass} note={row.note} "
            f"HO_pos={row.pos_holdout*100:.0f}% top3={row.top3_holdout*100:.0f}%",
            flush=True,
        )
        if row.soft_pass:
    ...
    for n, p in soft_structures.items():
        if "wrebal" in n and ("skc_" in n or "mtd_" in n):
    ...
    out_csv = ROOT / "reports" / "quest_skew_cool_board.csv"
    out_md = ROOT / "reports" / "quest_skew_cool_consistency.md"
    out_json = ROOT / "configs" / "quest_skew_cool_selected.json"
