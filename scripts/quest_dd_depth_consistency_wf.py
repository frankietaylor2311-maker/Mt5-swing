#!/usr/bin/env python3
"""Causal rolling DD-depth percentile cool consistency wave on locked sleeve.

Distinct from apply_peak_proximity_cool (fixed eps near high) and
apply_rolling_return_pctile_cool (rich trailing return). Cools when lag-1
drawdown depth sits at/below a causal rolling LOW percentile of its own history
(unusually shallow / complacent). RF=0.08, VT=0.0025, overlay hi<=1. Nested
picks on 2024 only; HO never for selection. Soft mean floor 0.0095 on BOTH
2024 and 2025_IS.
"""
...
def tag_dd(pb: int, hb: int, pct: float, cool: float) -> str:
    return f"dddepth_pb{pb}_hb{hb}_p{pct}_c{cool}"
...
def _grid():
    return itertools.product(PEAK_BARS, HIST_BARS, PCTILES, COOL_SCALES)


def pick_dddepth_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for pb, hb, pct, cool in _grid():
        params = {
            "peak_bars": int(pb),
            "hist_bars": int(hb),
            "pctile": float(pct),
        ...
        st = cor.stats_from_port(
            apply_rolling_dd_depth_cool(port24, **params, lo=0.25)
        )
...
def pick_dddepth_on_transformed(port24_xf, base_st_xf):
    best = None
    for pb, hb, pct, cool in _grid():
        params = {
            "peak_bars": int(pb),
        ...
        st = cor.stats_from_port(
            apply_rolling_dd_depth_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    # Sparse lock+grid: mid pctile × cool_scales extremes to keep board size sane
    dd_soft_ports = {}
    sparse = list(itertools.product(PEAK_BARS, HIST_BARS, [0.20], [0.35, 0.5, 0.65]))
    for pb, hb, pct, cool in sparse:
        params = {
            "peak_bars": int(pb),
            "hist_bars": int(hb),
            "pctile": float(pct),
        ...
        ports = apply_ports(
            apply_rolling_dd_depth_cool, base_ports, **params, lo=0.25
        )
        name = f"lock+{tag_dd(pb, hb, pct, cool)}"
        row = cor.eval_idea(name, "dddepth", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"dddepth: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            dd_soft_ports[name] = ports
    ...
    if w24 is not None:
        ...
        picked = pick_dddepth_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ports = apply_ports(
                apply_rolling_dd_depth_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_dd(params['peak_bars'], params['hist_bars'], params['pctile'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_dddepth", ports, is_labels, conf_labels)
            board.append(row)
            print(f"wrebal+dddepth: {name} soft={row.soft_pass} note={row.note}", flush=True)
            if row.soft_pass:
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        picked = pick_dddepth_on_transformed(p24, st24)
        if picked is None or picked[0] == float("-inf"):
            continue
        _, pparams, _ = picked
        lports = apply_ports(
            apply_rolling_dd_depth_cool, ports, **pparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_dd(pparams['peak_bars'], pparams['hist_bars'], pparams['pctile'], pparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_dddepth", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+dddepth: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(dd_soft_ports.items())[:2]:
        soft_structures[n] = p
    ...
    out_csv = ROOT / "reports" / "quest_dd_depth_board.csv"
    out_md = ROOT / "reports" / "quest_dd_depth_consistency.md"
    out_json = ROOT / "configs" / "quest_dd_depth_selected.json"
    ...
    selected = {
        "wave": "causal_rolling_dd_depth_percentile_cool",
        "data_source": ds,
        ...
        "dddepth_grid": {
            ...
            "lo": 0.25,
            "rule": "lag-1 DD depth <= lag-1 rolling LOW pctile → cool",
        },
    }
