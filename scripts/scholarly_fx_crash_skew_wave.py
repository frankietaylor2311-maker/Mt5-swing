#!/usr/bin/env python3
"""Scholarly FX Brunnermeier–Nagel–Pedersen crash-risk / return-skewness wave.

Currencies with more negative trailing return skewness / deeper left-tail
shortfall earn a crash-risk premium (BNP 2008). Built from free OHLC only —
distinct from Menkhoff FX-RV (§15), Lustig carry, and AI-GPR roles (§25).

Fixed priors: formation 63d/126d, skip=1, signal_lag=1; monthly XS rebalance;
...
from mt5_swing.strategies.fx_crash_skew_fx import FxCrashSkewConfig, crash_skew_factor_returns
...
def main() -> int:
    ...
    print("=== scholarly FX BNP crash-risk / return-skewness wave ===")
    print(
        "Brunnermeier–Nagel–Pedersen (2008) — trailing skew / left-tail XS + "
        "mom×skew + scholarly carry×crash cool. Distinct from Menkhoff FX-RV, "
        "Lustig carry, AI-GPR. Locked sleeve untouched."
    )
    ...
    try:
        ...
        print(f"WARN rates: {exc}; carry_crash_cool skipped")
        rates = None

    cfg = FxCrashSkewConfig()
    factors = crash_skew_factor_returns(pair_ret, rates=rates, cfg=cfg)
    if not factors:
        print("FATAL: no crash-skew factors produced")
        return 1
    ...
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_crash_skew_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_crash_skew_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_crash_skew_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_crash_skew_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "path": "brunnermeier_nagel_pedersen_crash_skew",
        "section": 26,
        "data_tag": data_tag,
        "signal_lag": cfg.signal_lag,
        "skip_days": cfg.skip_days,
        "formation_days": cfg.formation_days,
        "formation_days_long": cfg.formation_days_long,
        "left_tail_q": cfg.left_tail_q,
        ...
        "citations": [
            ...
            "Crashes, Review of Financial Studies"
        ],
    }
    (REPORTS / "scholarly_fx_crash_skew_meta.json").write_text(
    ...
    lines = [
        ...
        "",
        "**Path:** BNP (2008) currency crash-risk premium via trailing return "
        "skewness / left-tail shortfall XS sorts — **distinct** from Menkhoff "
        "FX-RV (§15), Lustig carry, and AI-GPR roles (§25).",
        f"**Data:** `{data_tag}` Yahoo D1 USD majors (OHLC only; no paid IV/RR). "
        f"**PIT:** skip={cfg.skip_days}d + signal_lag={cfg.signal_lag}d; "
        f"formation {cfg.formation_days}/{cfg.formation_days_long}d; "
        f"left-tail q={cfg.left_tail_q}; costs {cfg.cost_bps_per_side} bps/side.",
        f"**Primary:** `{PRIMARY}` (long more-negative skew / high crash risk). "
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_crash_skew_*.csv`, "
        "`scholarly_fx_crash_skew_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_crash_skew_wave.md").write_text("\n".join(lines))
