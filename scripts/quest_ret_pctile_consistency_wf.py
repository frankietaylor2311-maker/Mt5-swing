#!/usr/bin/env python3
"""Causal rolling-return percentile cool consistency wave on locked sleeve.

Distinct from apply_runup_throttle (fixed thresh), peak-proximity (near high),
and win-streak / hit-rate / Sharpe cools. Cools when lag-1 trailing return sits
at/above a causal rolling percentile of its own history. RF=0.08, VT=0.0025,
overlay hi<=1. Nested picks on 2024 only; HO never for selection. Soft mean
floor 0.0095 on BOTH 2024 and 2025_IS.
"""
...
from mt5_swing.portfolio.overlays import (
    apply_mtd_gain_clip,
    apply_rolling_return_pctile_cool,
)
...
def tag_rp(tb: int, hb: int, pct: float, cool: float) -> str:
    return f"retpct_tb{tb}_hb{hb}_p{pct}_c{cool}"
...
def _grid():
    return itertools.product(TRAIL_BARS, HIST_BARS, PCTILES, COOL_SCALES)


def pick_retpct_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for tb, hb, pct, cool in _grid():
        params = {
            "trail_bars": int(tb),
            "hist_bars": int(hb),
            "pctile": float(pct),
        ...
        st = cor.stats_from_port(
            apply_rolling_return_pctile_cool(port24, **params, lo=0.25)
        )
...
def pick_retpct_on_transformed(port24_xf, base_st_xf):
    best = None
    for tb, hb, pct, cool in _grid():
        params = {
            "trail_bars": int(tb),
        ...
        st = cor.stats_from_port(
            apply_rolling_return_pctile_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    # Sparse lock+grid: only cool_scales extremes × subset to keep board size sane
    retpct_soft_ports = {}
    sparse = list(itertools.product(TRAIL_BARS, HIST_BARS, [0.85], [0.35, 0.5, 0.65]))
    for tb, hb, pct, cool in sparse:
        params = {
            "trail_bars": int(tb),
            "hist_bars": int(hb),
            "pctile": float(pct),
        ...
        ports = apply_ports(
            apply_rolling_return_pctile_cool, base_ports, **params, lo=0.25
        )
        name = f"lock+{tag_rp(tb, hb, pct, cool)}"
        row = cor.eval_idea(name, "retpct", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"retpct: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            retpct_soft_ports[name] = ports
    ...
    if w24 is not None:
        ...
        picked = pick_retpct_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ports = apply_ports(
                apply_rolling_return_pctile_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_rp(params['trail_bars'], params['hist_bars'], params['pctile'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_retpct", ports, is_labels, conf_labels)
            board.append(row)
            print(f"wrebal+retpct: {name} soft={row.soft_pass} note={row.note}", flush=True)
            if row.soft_pass:
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        picked = pick_retpct_on_transformed(p24, st24)
        if picked is None or picked[0] == float("-inf"):
            continue
        _, pparams, _ = picked
        lports = apply_ports(
            apply_rolling_return_pctile_cool, ports, **pparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_rp(pparams['trail_bars'], pparams['hist_bars'], pparams['pctile'], pparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_retpct", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+retpct: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(retpct_soft_ports.items())[:2]:
        soft_structures[n] = p
    ...
    out_csv = ROOT / "reports" / "quest_ret_pctile_board.csv"
    out_md = ROOT / "reports" / "quest_ret_pctile_consistency.md"
    out_json = ROOT / "configs" / "quest_ret_pctile_selected.json"
    ...
    selected = {
        "wave": "causal_rolling_return_pctile_cool",
        "data_source": ds,
        ...
        "retpct_grid": {
            ...
            "lo": 0.25,
            "rule": "lag-1 trail ret >= lag-1 rolling pctile → cool",
        },
    }
