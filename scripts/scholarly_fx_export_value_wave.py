#!/usr/bin/env python3
"""Scholarly FX OECD/FRED merchandise export-value YoY (XTEXVA01) FX differential wave (§61).

Fixed priors: FRED OECD MEI XTEXVA01 merchandise export-value YoY %
(full G10 *M657S; EUR=XTEXVA01DEM657S Germany proxy — EZM657S 404,
EZM667S stale ~2023-04; unit=VALUE not volume),
pub_lag_months=2 + signal_lag=1m + signal_lag_days=1. Primary high_xval_xs
(growth-channel / external-demand). Scores on **merchandise export-value
YoY %**. No HO tuning. Locked sleeve untouched. Distinct from trade-balance
§30, CA §21, ToT, retail §46, IP §42, cars §60, construction §59, GDP §58.
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED merchandise export-value wave (§{SECTION}) ===")
    print(
        "FRED XTEXVA01 merchandise export-value YoY (M657S; pub_lag=2) — high_xval XS; "
        "growth-channel / external-demand prior; EUR=DEM Germany proxy; "
        "full G10; distinct from TB §30, CA §21, retail §46, IP §42, cars §60, construction §59, GDP §58. "
        "Locked sleeve untouched. Fall-through: CIP/basis Missing; hours/vacancies thin; PRSVTT*/ODMN* 404. "
        "Unit=VALUE not volume."
    )

    try:
        lp = load_export_value_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL export_value load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_export_value_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_export_value_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED merchandise export-value\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"export_value panel too thin: cols={list(lp.columns)}. "
    ...
    cov = export_value_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_export_value_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_export_value_panel.csv")
    print(
        f"export_value panel cols={list(lp.columns)} "
    ...
    us_xval = None
    try:
        us_xval = load_us_export_value(download=True, force=False)
        print(
            f"US XTEXVA01USM657S YoY [{us_xval.dropna().index.min().date()}.."
            f"{us_xval.dropna().index.max().date()}] n={us_xval.dropna().shape[0]} "
            f"series={us_xval.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US export_value load failed: {exc}")
        us_xval = None
    ...
    factors = export_value_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_xval_override=us_xval,
    )
    if not factors:
        print("FATAL: no export_value factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_export_value_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_export_value_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_export_value_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_export_value_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_merchandise_export_value_xtexva01",
        "data_tag": data_tag,
        ...
        "score_unit": "export_value_yoy_pct",
        "z_low": cfg.z_low,
        ...
        "normalisation": (
            "FRED OECD MEI XTEXVA01 merchandise export-value YoY % "
            "(full G10 *M657S; EUR=XTEXVA01DEM657S Germany proxy — EZM657S 404, "
            "EZM667S stale ~2023-04; unit=VALUE not volume; *M667S levels not mixed); "
            "XS on export-value YoY %; also 5y z / +Δ12 of YoY boarded; "
            "high_xval primary (growth-channel / external demand → appreciate); "
            "n_long=n_short=2 on available foreign panel (full G10)"
        ),
        "eur_mapping": "XTEXVA01DEM657S Germany proxy (EZM657S 404; EZM667S stale ~2023-04)",
        ...
        "score_basis": "export_value_yoy_pct",
        "distinct_from": [
            "trade_balance_30",
        ...
        "citations": [
            ...
            "Growth-channel FX on merchandise export-value YoY (external demand / trade intensity)",
            "OECD MEI XTEXVA01 merchandise export-value YoY (M657S; value not volume)",
            "Honesty: low export-value growth / external-demand-stress premium",
    ...
    (REPORTS / "scholarly_fx_export_value_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED merchandise export-value differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **merchandise export-value YoY** "
        "(`XTEXVA01` M657S; EUR=Germany DEM proxy; full G10; unit=VALUE not volume) — export-value YoY XS — "
        "**distinct** from trade-balance (§30), CA (§21), ToT (UV 404), retail (§46), IP (§42), "
        "passenger-cars (§60), construction (§59), GDP (§58).",
        f"**Data:** `{data_tag}` + free FRED XTEXVA01 YoY after pub_lag. "
        f"EUR=XTEXVA01DEM657S Germany proxy (EZM657S 404); full G10 live. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (trade/export ~1–2m lag a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **merchandise export-value YoY %** (value not volume).",
        f"**Primary:** `{PRIMARY}` (long high relative export-value YoY / short low — "
        "growth-channel / external demand → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_export_value_*.csv`, "
        "`scholarly_fx_export_value_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_export_value_wave.md").write_text("\n".join(lines))
