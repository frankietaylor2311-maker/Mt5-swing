#!/usr/bin/env python3
"""Scholarly FX CPI / inflation YoY (OECD MEI / HICP) differential wave (§52).

Fixed priors: FRED CPI/HICP index→YoY (CPIAUCSL; CP0000EZ19M086NEST;
GBP/JPY/CAD/CHF *CPIALLMINMEI; AUD/NZD *CPIALLQINMEI Q→YoY), pub_lag_months=1 +
signal_lag=1m + signal_lag_days=1. Primary high_cpi_xs (Dahlquist–Hasseltoft
economic momentum — high relative CPI YoY → appreciate). Scores on **CPI YoY %**.
JPY STALE ~2021-06; GBP/CAD/CHF/AUD/NZD mild ~2025. No HO tuning. Locked sleeve untouched.
Distinct from PPI §51, IP §42, retail §46, employment §41, ULC/LP/CU §48–50,
building-permits §43, CLI/CCI/BCI, macro_diff EW blend, PPP real FX, WUI, EPU/TPU.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD CPI / inflation YoY wave (§{SECTION}) ===")
    print(
        "FRED CPI/HICP index→YoY (pub_lag=1) — CPI YoY XS; Dahlquist–Hasseltoft "
        "economic-momentum; JPY STALE ~2021-06; distinct from PPI §51, IP §42, "
        "retail §46, ULC/LP/CU §48–50, macro_diff EW. Locked sleeve untouched."
    )

    try:
        lp = load_cpi_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL CPI load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_cpi_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_cpi_wave.md").write_text(
            f"# Scholarly FX: OECD CPI / inflation\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"CPI panel too thin: cols={list(lp.columns)}. "
    ...
    cov = cpi_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_cpi_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_cpi_panel.csv")
    print(
        f"CPI panel cols={list(lp.columns)} "
    ...
    us_cpi = None
    try:
        us_cpi = load_us_cpi(download=True, force=False)
        print(
            f"US CPIAUCSL→YoY [{us_cpi.dropna().index.min().date()}.."
            f"{us_cpi.dropna().index.max().date()}] n={us_cpi.dropna().shape[0]} "
            f"series={us_cpi.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US CPI load failed: {exc}")
        us_cpi = None
    ...
    cfg = CpiFxConfig()
    factors = cpi_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_cpi_override=us_cpi,
    )
    if not factors:
        print("FATAL: no CPI factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_cpi_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_cpi_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_cpi_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_cpi_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_cpi_inflation_yoy",
        "data_tag": data_tag,
        ...
        "normalisation": (
            "FRED CPI/HICP index→YoY (CPIAUCSL; CP0000EZ19M086NEST; "
            "GBP/JPY/CAD/CHF *CPIALLMINMEI; AUD/NZD *CPIALLQINMEI Q→YoY "
            "quarterly→monthly after pub_lag=1); XS on CPI YoY; also 5y z / "
            "+Δ12 of YoY boarded; high_cpi primary (Dahlquist economic-momentum → appreciate); "
            "n_long=n_short=2 full G10 foreign"
        ),
        "eur_mapping": "CP0000EZ19M086NEST EA19 HICP (live; CPHPTT01EZM659N YoY staler — not mixed)",
        "coverage_g10": "full G10 mapped (USD/EUR live; JPY STALE ~2021-06; GBP/CAD/CHF/AUD/NZD mild ~2025)",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "yoy_growth_derived_from_index",
        "distinct_from": [
            "ppi_51",
            "employment_41",
        ...
        "citations": [
            "Dahlquist & Hasseltoft (2020), JFE — economic momentum incl. CPI",
            "OECD MEI / national CPI + EA19 HICP",
            "Honesty: low CPI / PPP high-inflation short",
        ],
    ...
    (REPORTS / "scholarly_fx_cpi_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD CPI / inflation YoY differential wave (§{SECTION})",
        "",
        "**Path:** FRED CPI/HICP index→YoY (`CPIAUCSL` / `CP0000EZ19M086NEST` / "
        "`*CPIALLMINMEI` / `*CPIALLQINMEI`) — CPI YoY XS — **distinct** from PPI (§51), "
        "IP (§42), retail (§46), employment (§41), ULC/LP/CU (§48–50), building-permits (§43), "
        "CLI/CCI/BCI, macro_diff EW blend, PPP real FX, WUI, EPU/TPU.",
        f"**Data:** `{data_tag}` + free FRED CPI/HICP index→YoY after pub_lag. "
        f"**STALE** — JPY ends ~2021-06 (hard); GBP/CAD ~2025-03, CHF ~2025-04, "
        f"AUD/NZD Q ~2025-01 (mild); USD/EUR live mid-2026. EUR = `CP0000EZ19M086NEST`. "
        f"Full G10 mapped. `n_long=n_short=2`. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (CPI a priori = macro_diff default) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **CPI YoY %**.",
        f"**Primary:** `{PRIMARY}` (long high relative CPI YoY / short low — "
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_cpi_*.csv`, "
        "`scholarly_fx_cpi_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_cpi_wave.md").write_text("\n".join(lines))
