#!/usr/bin/env python3
"""Causal consecutive win-day cool consistency wave on locked sleeve.
...
def tag_ws(n: int, cool: float) -> str:
    return f"wstreak_n{n}_c{cool}"
...
def pick_wstreak_2024(port24, base_st24, top_k: int = 3):
    ranked = []
...
def pick_wstreak_on_transformed(port24_xf, base_st_xf):
    best = None
    for n, cool in itertools.product(STREAK_NS, COOL_SCALES):
        params = {"streak_n": int(n), "cool_scale": float(cool)}
        st = cor.stats_from_port(
...
def main() -> None:
    ...
    # --- wstreak alone: full grid on locked ---
    wstreak_soft_ports = {}
    for n, cool in itertools.product(STREAK_NS, COOL_SCALES):
        params = {"streak_n": int(n), "cool_scale": float(cool)}
        ports = apply_ports(
        ...
        name = f"lock+{tag_ws(n, cool)}"
        row = cor.eval_idea(name, "wstreak", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"wstreak: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            wstreak_soft_ports[name] = ports
    ...
    # --- wrebal + nested wstreak on 2024 ---
    soft_structures = {}
    w24 = wrebal_ports.get("2024")
    if w24 is not None:
        wst = cor.stats_from_port(w24)
        picked = pick_wstreak_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ...
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_ws(params['streak_n'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_wstreak", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"wrebal+wstreak: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                soft_structures[name] = ports

    # --- wrebal + mtd + nested wstreak ---
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        st24 = cor.stats_from_port(p24)
        picked = pick_wstreak_on_transformed(p24, st24)
        ...
        lports = apply_ports(
            apply_consecutive_win_cool,
            ports,
            streak_n=wparams["streak_n"],
            cool_scale=wparams["cool_scale"],
            lo=0.25,
        )
        iname = f"{name}+{tag_ws(wparams['streak_n'], wparams['cool_scale'])}"
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_wstreak", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+wstreak: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(wstreak_soft_ports.items())[:2]:
        soft_structures[n] = p
    ...
    out_csv = ROOT / "reports" / "quest_win_streak_board.csv"
    out_md = ROOT / "reports" / "quest_win_streak_consistency.md"
    out_json = ROOT / "configs" / "quest_win_streak_selected.json"
    ...
    selected = {
        "wave": "causal_consecutive_win_day_cool",
        "data_source": ds,
        ...
        "wstreak_grid": {
            ...
            "lo": 0.25,
            "rule": "lagged consecutive positive daily returns >= streak_n → cool",
        },
    }
