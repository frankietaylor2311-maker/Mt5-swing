#!/usr/bin/env python3
"""Scholarly FX Dahlquist-style equal-weight soft-signal recombination wave (§66).

Pre-registered BEFORE any holdout peek (2026-09-24):
  SOFT_LEGS = ["bci_chg_xs", "high_ip_xs", "high_ppi_xs", "low_gdp_xs", "low_cars_xs"]
  — only factors that already cleared soft |NW t|≥1.5 with positive full-sample
    mean on their own boards (documented §39/§42/§51/§58/§60).
  One BCI seed only (bci_chg_xs); NOT both BCI legs.
  Board: soft_ew5 (primary), soft_ew_growth, soft_ew_honesty,
         soft_ew4_no_{bci,ip,ppi}, soft_ew3_core.
  ...
  Distinct from combo §8, capital-sleeve mix §53, macro_diff blend.
  Literature: Dahlquist & Hasseltoft (2020) Economic momentum — EW of
  macro-momentum signals; recombination after single-series promote=0.
...
BCI_CFG = OecdBciFxConfig()  # signal_lag=0, pub_lag=2, cost 1.5
IP_CFG = IndustrialProductionFxConfig()  # signal_lag=0, pub_lag=2
PPI_CFG = PpiFxConfig()  # signal_lag=1, pub_lag=2
GDP_CFG = GdpFxConfig()  # signal_lag=1, pub_lag=3
CARS_CFG = PassengerCarsFxConfig()  # signal_lag=1, pub_lag=2
...
def rebuild_soft_legs(pair_ret: pd.DataFrame) -> dict[str, pd.Series]:
    """Rebuild SOFT_LEGS daily returns with source-wave PIT configs (no CSV peek)."""
...
def main() -> int:
    ...
    print(f"=== scholarly FX soft-signal recombination stack wave (§{SECTION}) ===")
    print(
        "PRE-REGISTERED SOFT_LEGS (before HO): "
        f"{SOFT_LEGS}. Primary={PRIMARY}. "
        "Dahlquist–Hasseltoft EW of soft XS daily returns. "
        "Distinct from combo §8 / capital-sleeve §53 / macro_diff. "
    ...
    try:
        legs = rebuild_soft_legs(pair_ret)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL soft-leg rebuild: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_soft_signal_stack_meta.json").write_text(
            json.dumps(meta, indent=2)
        )
        (REPORTS / "scholarly_fx_soft_signal_stack_wave.md").write_text(
            f"# Scholarly FX: soft-signal recombination stack\n\n**FAILED:** {exc}\n"
        )
        return 1

    # Coverage of individual legs (non-null days)
    cov_rows = []
    ...
    pd.DataFrame(cov_rows).to_csv(
        REPORTS / "scholarly_fx_soft_signal_stack_coverage.csv", index=False
    )

    # Also dump leg full-sample summaries (sanity — not for HO selection)
    leg_summ = [factor_summary(legs[n], n) for n in SOFT_LEGS]
    pd.DataFrame(leg_summ).to_csv(
        REPORTS / "scholarly_fx_soft_signal_stack_leg_summary.csv", index=False
    )
    ...
    cfg = SoftSignalStackConfig()
    board = soft_signal_stack_factor_returns(legs, cfg=cfg)
    if not board or PRIMARY not in board:
        print("FATAL: soft stack board empty / primary missing")
        return 1

    summaries = [factor_summary(r, name) for name, r in board.items()]
    # Stable order matching BOARD_SPECS
    order = list(BOARD_SPECS.keys())
    summaries.sort(key=lambda s: order.index(s["factor"]) if s["factor"] in order else 99)
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_soft_signal_stack_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_soft_signal_stack_{name}_monthly.csv",
            header=[name],
        )
    ...
    win_df.to_csv(
        REPORTS / "scholarly_fx_soft_signal_stack_window_stats.csv", index=False
    )
    ...
    if len(sweep_df):
        sweep_df.to_csv(
            REPORTS / "scholarly_fx_soft_signal_stack_risk_sweep.csv", index=False
        )
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "dahlquist_soft_signal_ew_recombination",
        "data_tag": data_tag,
        "soft_legs_preregistered": list(SOFT_LEGS),
        ...
        "normalisation": (
            ...
            "(row-wise nanmean; ≥2 legs/day). Membership fixed a priori from §39/42/"
            "51/58/60 soft boards — no HO tuning. Costs already in source factors "
            "(1.5 bps/side)."
        ),
        "distinct_from": [
            "combo_8",
        ...
        "citations": [
            ...
            "Soft boards §39 BCI / §42 IP / §51 PPI / §58 GDP honesty / §60 cars honesty",
        ],
        "soft_best": soft_best,
    ...
    lines = [
        f"# Scholarly FX: Dahlquist-style soft-signal EW recombination (§{SECTION})",
        "",
        "**Path:** Equal-weight of pre-registered soft-boarded scholarly XS daily "
        "returns — **distinct** from combo (§8), capital-sleeve mix (§53), "
        "macro_diff blend, and single-series BCI/IP/PPI/GDP/cars waves.",
        f"**Data:** `{data_tag}` + free FRED/OECD panels rebuilt with source-wave "
        f"PIT lags. **SOFT_LEGS (a priori):** `{SOFT_LEGS}`. "
        f"Row-wise nanmean; require ≥{cfg.min_legs} legs/day. "
        f"Costs 1.5 bps/side (inside source factors).",
        f"**Primary:** `{PRIMARY}` (EW of all 5 soft legs). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_soft_signal_stack_*.csv`, "
        "`scholarly_fx_soft_signal_stack_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_soft_signal_stack_wave.md").write_text("\n".join(lines))
