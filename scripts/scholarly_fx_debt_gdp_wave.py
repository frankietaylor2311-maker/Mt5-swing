#!/usr/bin/env python3
"""Scholarly FX government debt/GDP differential wave (fiscal sustainability).

Fixed priors: IMF WEO GGGDTA via FRED (GGXWDG* 404), pub_lag_months=15 + signal_lag=1m.
Optional fiscal_surplus blend (scholarly only). No HO tuning. Locked sleeve untouched.
"""
...
from mt5_swing.data.fred_fiscal_balance import load_fiscal_gdp_panel
from mt5_swing.data.loader import load_ohlc_csv
from mt5_swing.strategies.debt_gdp_fx import DebtGdpFxConfig, debt_gdp_factor_returns
...
def main() -> int:
    ...
    print("=== scholarly FX government debt/GDP wave ===")
    print(
        "Debt overhang / fiscal sustainability — IMF WEO GGGDTA via FRED "
        "(GGXWDG*188N 404); distinct from fiscal GGNLBA (§28), CA (§21), CB-BS, "
        "macro-diff. Locked sleeve untouched."
    )

    try:
        debt = load_debt_gdp_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL debt load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_debt_gdp_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_debt_gdp_wave.md").write_text(
            f"# Scholarly FX: government debt/GDP\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = debt_coverage(debt)
    cov.to_csv(REPORTS / "scholarly_fx_debt_gdp_coverage.csv", index=False)
    debt.to_csv(REPORTS / "scholarly_fx_debt_gdp_panel.csv")
    print(
        f"Debt panel cols={list(debt.columns)} "
        f"[{debt.dropna(how='all').index.min().date()}.."
        f"{debt.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))
    print(f"Series source note: {debt.attrs.get('source')}")
    print(f"Failed aliases documented: {debt.attrs.get('failed_aliases')}")

    us_q = None
    try:
        us_q = load_us_federal_debt_gdp_q(download=True, force=False)
        print(
            f"US federal debt/GDP Q [{us_q.dropna().index.min().date()}.."
            f"{us_q.dropna().index.max().date()}] n={us_q.dropna().shape[0]} "
            f"series={us_q.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US quarterly debt load failed: {exc}")
        us_q = None

    fiscal_surplus_ret = None
    try:
        fiscal = load_fiscal_gdp_panel(download=True, force=False)
        print(f"Fiscal panel for blend: cols={list(fiscal.columns)}")
    except Exception as exc:  # noqa: BLE001
        print(f"WARN fiscal load for blend failed: {exc}")
        fiscal = None
    ...
    if fiscal is not None:
        try:
            fis_factors = fiscal_balance_factor_returns(
                pair_ret, fiscal, cfg=FiscalBalanceFxConfig()
            )
            fiscal_surplus_ret = fis_factors.get("fiscal_surplus_xs")
        except Exception as exc:  # noqa: BLE001
            print(f"WARN fiscal factor build failed: {exc}")

    cfg = DebtGdpFxConfig()
    # Prefer higher-freq US quarterly federal debt/GDP for US tilts when available
    factors = debt_gdp_factor_returns(
        pair_ret,
        debt,
        cfg=cfg,
        us_debt_override=us_q,
        fiscal_surplus_returns=fiscal_surplus_ret,
    )
    if not factors:
        print("FATAL: no debt/GDP factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_debt_gdp_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_debt_gdp_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_debt_gdp_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_debt_gdp_risk_sweep.csv", index=False)
    ...
    us_tilt_source = (
        us_q.attrs.get("series_id") if us_q is not None else debt.attrs.get("series_map", {}).get("USD")
    )
    meta = {
        "ok": True,
        "path": "government_debt_gdp",
        "data_tag": data_tag,
        ...
        "fred_mnemonic_note": (
            "Brief suggested GGXWDG*188N (IMF GGXWDG_NGDP); those 404 on FRED. "
            "Live actuals are GGGDTA*188N (same WEO release)."
        ),
        "us_tilt_series": us_tilt_source,
        "distinct_from": [
        ...
        "citations": [
            "Reinhart–Rogoff debt overhang / fiscal sustainability lineage",
            "Twin deficits / external-adjustment surveys (stock vs flow)",
            "Della Corte–Riddiough–Sarno debtor premium (honesty alternate)",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": debt.attrs.get("unmapped", []),
        "failed_aliases": debt.attrs.get("failed_aliases", {}),
    }
    (REPORTS / "scholarly_fx_debt_gdp_meta.json").write_text(
    ...
    lines = [
        ...
        "",
        "**Path:** Debt overhang / fiscal sustainability — IMF WEO general-government "
        "gross debt (% GDP) via FRED — **distinct** from fiscal balance GGNLBA (§28), "
        "CA (§21), CB-BS, macro-diff, equity-diff, AI-GPR, ToT, dollar-beta, crash-skew.",
        f"**Data:** `{data_tag}` + free FRED `GGGDTA*188N` "
        "(brief `GGXWDG*188N` **404 on FRED** — documenting honest alias failure) "
        f"+ US quarterly `GFDEGDQ188S` for tilts. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (~April Y+1 for year-Y) + "
        f"signal_lag={cfg.signal_lag}m + 1d weight lag; US-Q pub_lag=4m.",
        f"**Primary:** `{PRIMARY}` (long low debt / short high debt). "
        "NZD/CHF unmapped on free FRED. Locked sleeve untouched.",
        "",
