#!/usr/bin/env python3
"""Scholarly FX CIP-stress-conditioned Dahlquist soft-signal EW wave (§71).

Multi-factor structure: §66 soft_ew_macro5 (SOFT_LEGS EW) gated / cooled by
aggregate government-bond CIP stress (UST premium = −mean G10 cip_govt).
Literature: Dahlquist–Hasseltoft (2020) + Du–Tepper–Verdelhan (2018) JF +
Du–Keerati–Schreger (2025).

Fixed priors: tenor **5y** cip_govt (DEFAULT_TENOR — same §67/§68 a priori),
pub_lag_days=1, signal_lag_months=1 + 1d weight lag, gate z≤0 / cool at z≥1,
soft legs source-wave PIT, costs 1.5 bps/side inside source factors.
Primary ``soft_low_cip_stress``. Honesty: ``soft_raw`` + ``soft_high_cip_stress``.
...
Lustig–Verdelhan carry), CIP XS §67, capital-sleeve §53 / §70, combo §8.
...
PREFIX = "scholarly_fx_cip_conditioned_soft"
...
def write_locked_verify() -> dict:
    ...
    lines = [
        "# One-pct month quest — locked_verify_cip_conditioned_soft",
        "",
    ...
    lines += [
        "",
        "Config unmodified vs §70 capital_sleeve_soft / §68 cip_conditioned_carry — "
        "canonical windowed PASS.",
        "",
    ]
    (REPORTS / "quest_locked_verify_cip_conditioned_soft.md").write_text("\n".join(lines))
    log = (
        f"Locked sleeve re-verify (§{SECTION} cip_conditioned_soft): "
        f"{LOCKED_CFG.relative_to(ROOT)}\n"
        f"basket_tag={LOCKED_TAG} untouched\n"
        f"sha256={sha}\n"
        f"Canonical windowed PASS (same as §70 capital_sleeve_soft — config unmodified):\n"
        ...
        f"Wrote {REPORTS / 'quest_locked_verify_cip_conditioned_soft.md'}\n"
    )
    (REPORTS / "quest_locked_verify_cip_conditioned_soft.log").write_text(log)
    print(log, end="")
...
def main() -> int:
    ...
    print(f"=== scholarly FX CIP-conditioned soft-signal EW wave (§{SECTION}) ===")
    print(
        f"cip_govt {TENOR} stress gate/cool on soft_ew_macro5 — primary {PRIMARY}; "
        "Dahlquist–Hasseltoft + Du–Tepper–Verdelhan / Du–Keerati–Schreger; "
    ...
    try:
        ...
        (REPORTS / f"{PREFIX}_wave.md").write_text(
            f"# Scholarly FX: CIP-conditioned soft\n\n**FAILED:** {exc}\n"
        )
        return 1
    ...
    factors = cip_conditioned_soft_factor_returns(
        soft_ew,
        lp,
        pair_ret=pair_ret,
        cfg=cfg,
    ...
    if not factors or PRIMARY not in factors:
        print("FATAL: no cip_conditioned_soft factors / primary missing")
        return 1
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "du_schreger_cip_conditioned_dahlquist_soft_ew",
        "data_tag": data_tag,
        "tenor": TENOR,
        "tenor_prior": "5y a priori (same §67/§68; not HO-tuned)",
        "value_col": "cip_govt",
        "unit": "bps",
        ...
        "primary": PRIMARY,
        "primary_sign_prior": (
            "trade Dahlquist soft_ew_macro5 only when lagged CIP-stress "
            "z (UST premium) ≤ 0 — sit out when intermediary dollar-funding stress elevated"
        ),
        "n_factors": len(board),
        "soft_nw_pos": soft,
        "hard_nw_pos": hard,
        ...
        "data_caveat": (
            ...
            "Soft legs rebuilt with §66 source-wave PIT; costs inside source factors."
        ),
        "normalisation": (
            f"CIP stress = −mean(cip_govt) at {TENOR}; month-end after "
            f"pub_lag_days={PUB_LAG_D}; trailing z {cfg.z_window}m; gate z≤0 / cool z≥"
            f"{cfg.z_high}; soft_ew_macro5 = EW SOFT_LEGS (≥2 legs/day)"
        ),
    ...
    lines = [
        f"# Scholarly FX: CIP-conditioned soft-signal EW wave (§{SECTION})",
        "",
        "**Path:** Du–Schreger **government-bond CIP** stress "
        f"(−mean `cip_govt` bps, tenor=**{TENOR}**) × Dahlquist–Hasseltoft "
        "``soft_ew_macro5`` gate/cool — **distinct** from soft_ew (§66 ungated), "
        "soft-stack CIP enrichment (§69 EW membership), CIP-conditioned carry (§68 "
        "gates Lustig–Verdelhan *carry*), CIP XS (§67), capital-sleeve (§53/§70), "
        "combo (§8). Explicit: do **not** overlay coolers on locked fx4plus.",
        ...
        f"**PIT:** CIP pub_lag_days={PUB_LAG_D} + "
        ...
        f"**Primary:** `{PRIMARY}` (trade soft only when lagged CIP-stress z ≤ 0). "
        "Honesty `soft_raw` + `soft_high_cip_stress`. Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        f"`{PREFIX}_meta.json`, `quest_locked_verify_cip_conditioned_soft.md`.",
        "",
    ]
    (REPORTS / f"{PREFIX}_wave.md").write_text("\n".join(lines))
