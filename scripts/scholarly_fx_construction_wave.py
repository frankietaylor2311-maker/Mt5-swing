#!/usr/bin/env python3
"""Scholarly FX OECD/FRED construction-production (PRCNTO01) FX differential wave (§59).

Fixed priors: FRED OECD MEI PRCNTO01 construction YoY %
(USD/EUR/GBP/CAD *GYSAM; JPY=JPNPRCNTO01GYSAQ quarterly YoY;
AUD/NZD/CHF=PRCNTO01*Q657S QoQ→YoY; EUR=DEUPRCNTO01GYSAM Germany proxy),
pub_lag_months=2 + signal_lag=1m + signal_lag_days=1. Primary high_cons_xs
(growth-channel / coincident construction output). Scores on **construction
YoY %**. No HO tuning. Locked sleeve untouched. Distinct from building-
permits §43, IP §42, GDP §58.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED construction-production wave (§{SECTION}) ===")
    print(
        "FRED PRCNTO01 construction YoY (GYSAM/GYSAQ/Q657S; pub_lag=2) — high_cons XS; "
        "growth-channel / coincident construction prior; full G10; EUR=DE proxy; "
        "JPY=GYSAQ; AUD/NZD/CHF QoQ→YoY; distinct from permits §43, IP §42, GDP §58. "
        "Locked sleeve untouched."
    )

    try:
        lp = load_construction_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL construction load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_construction_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_construction_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED construction-production\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"construction panel too thin: cols={list(lp.columns)}. "
    ...
    cov = construction_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_construction_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_construction_panel.csv")
    print(
        f"construction panel cols={list(lp.columns)} "
    ...
    us_cons = None
    try:
        us_cons = load_us_construction(download=True, force=False)
        print(
            f"US USAPRCNTO01GYSAM YoY [{us_cons.dropna().index.min().date()}.."
            f"{us_cons.dropna().index.max().date()}] n={us_cons.dropna().shape[0]} "
            f"series={us_cons.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US construction load failed: {exc}")
        us_cons = None
    ...
    factors = construction_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_cons_override=us_cons,
    )
    if not factors:
        print("FATAL: no construction factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_construction_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_construction_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_construction_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_construction_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_construction_prcnto01",
        "data_tag": data_tag,
        ...
        "score_unit": "construction_yoy_pct",
        "z_low": cfg.z_low,
        ...
        "normalisation": (
            "FRED OECD MEI PRCNTO01 construction YoY % "
            "(USD/EUR/GBP/CAD *GYSAM; JPY=GYSAQ; AUD/NZD/CHF *Q657S QoQ→YoY; "
            "EUR=DEUPRCNTO01GYSAM Germany proxy); "
            "XS on construction YoY %; also 5y z / +Δ12 of YoY boarded; "
            "high_cons primary (growth-channel / coincident construction → appreciate); "
            "n_long=n_short=2 on available foreign panel (full G10)"
        ),
        "eur_mapping": "DEUPRCNTO01GYSAM Germany proxy (live ~2026-06)",
        "coverage_g10": "USD/EUR/GBP/JPY/AUD/CAD/CHF/NZD all mapped (full G10)",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "construction_yoy_pct",
        "distinct_from": [
            "building_permits_43",
        ...
        "citations": [
            ...
            "OECD MEI PRCNTO01 construction production YoY (GYSAM / GYSAQ / Q657S)",
            "Honesty: low construction growth / activity-stress debtor premium",
        ],
    ...
    (REPORTS / "scholarly_fx_construction_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED construction-production differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **construction production YoY** "
        "(`PRCNTO01` GYSAM/GYSAQ/Q657S; full G10; EUR=Germany DEU proxy) — construction YoY XS — "
        "**distinct** from building-permits (§43), IP (§42), GDP (§58), CLI (§37), "
        "BCI (§39).",
        f"**Data:** `{data_tag}` + free FRED PRCNTO01 YoY after pub_lag. "
        f"EUR=DEUPRCNTO01GYSAM; JPY=GYSAQ; AUD/NZD/CHF QoQ→YoY; full G10 mapped. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (construction/IP-like lag a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **construction YoY %**.",
        f"**Primary:** `{PRIMARY}` (long high relative construction YoY / short low — "
        "growth-channel / coincident construction → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_construction_*.csv`, "
        "`scholarly_fx_construction_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_construction_wave.md").write_text("\n".join(lines))
