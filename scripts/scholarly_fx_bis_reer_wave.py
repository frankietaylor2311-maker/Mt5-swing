#!/usr/bin/env python3
"""Scholarly FX BIS REER undervaluation / mean-reversion wave.

Fixed priors: FRED BIS real broad EER RB*BIS, pub_lag_months=2 + signal_lag=1m.
Primary reer_cheap_xs (long low REER z). No HO tuning. Locked sleeve untouched.
Distinct from homemade PPP CPI real-FX.
"""
...
from mt5_swing.strategies.bis_reer_fx import BisReerFxConfig, bis_reer_factor_returns
...
def main() -> int:
    ...
    print("=== scholarly FX BIS REER wave ===")
    print(
        "Official BIS real broad EER via FRED RB*BIS — undervaluation / "
        "mean-reversion XS; distinct from homemade PPP CPI real-FX, TB (§30), "
        "debt (§29), fiscal (§28). Locked sleeve untouched."
    )

    try:
        reer = load_bis_reer_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL REER load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_bis_reer_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_bis_reer_wave.md").write_text(
            f"# Scholarly FX: BIS REER\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = reer_coverage(reer)
    cov.to_csv(REPORTS / "scholarly_fx_bis_reer_coverage.csv", index=False)
    reer.to_csv(REPORTS / "scholarly_fx_bis_reer_panel.csv")
    print(
        f"REER panel cols={list(reer.columns)} "
        f"[{reer.dropna(how='all').index.min().date()}.."
        f"{reer.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))
    print(f"Series source note: {reer.attrs.get('source')}")
    print(f"Failed/alt documented: {reer.attrs.get('failed_or_alt')}")

    us_reer = None
    try:
        us_reer = load_us_bis_reer(download=True, force=False)
        print(
            f"US RBUSBIS [{us_reer.dropna().index.min().date()}.."
            f"{us_reer.dropna().index.max().date()}] n={us_reer.dropna().shape[0]} "
            f"series={us_reer.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US REER load failed: {exc}")
        us_reer = None
    ...
    cfg = BisReerFxConfig()
    factors = bis_reer_factor_returns(
        pair_ret,
        reer,
        cfg=cfg,
        us_reer_override=us_reer,
    )
    if not factors:
        print("FATAL: no BIS REER factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_bis_reer_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_bis_reer_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_bis_reer_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_bis_reer_risk_sweep.csv", index=False)
    ...
    meta = {
        ...
        "normalisation": "trailing z of BIS real broad REER index (RB*BIS)",
        "eur_mapping": "RBXMBIS euro-area (RBDEBIS Germany alt documented)",
        "distinct_from": [
            "ppp_real_fx_bilateral_CPI",
            "trade_balance_30",
            "debt_gdp_29",
        ...
        "citations": [
            ...
            "Taylor / REER misalignment literature; BIS real broad EER",
            "Optional REER momentum (high z → long) as contrast prior",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": reer.attrs.get("unmapped", []),
        "failed_or_alt": reer.attrs.get("failed_or_alt", {}),
    }
    (REPORTS / "scholarly_fx_bis_reer_meta.json").write_text(
    ...
    lines = [
        ...
        "",
        "**Path:** Official BIS real broad effective exchange rates via FRED "
        "`RB*BIS` — multilateral REER undervaluation / mean-reversion XS — "
        "**distinct** from homemade bilateral PPP CPI real-FX, TB (§30), "
        "debt/GDP (§29), fiscal GGNLBA (§28), CA, CB-BS, macro-diff, equity-diff, "
        "AI-GPR, ToT, dollar-beta, crash-skew.",
        f"**Data:** `{data_tag}` + free FRED `RB*BIS`. EUR = `RBXMBIS` (euro-area; "
        f"`RBDEBIS` Germany alt documented). Full G10 mapped incl. NZD/CHF. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (conservative monthly BIS release) + "
        f"signal_lag={cfg.signal_lag}m + 1d weight lag. Primary z lookback="
        f"{cfg.z_window}m (alt {cfg.z_window_alt}m boarded).",
        f"**Primary:** `{PRIMARY}` (long low REER z / short high — Rogoff/Taylor "
        "undervaluation mean-reversion). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_bis_reer_*.csv`, "
        "`scholarly_fx_bis_reer_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_bis_reer_wave.md").write_text("\n".join(lines))
