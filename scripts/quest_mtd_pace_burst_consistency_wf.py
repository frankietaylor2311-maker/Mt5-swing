#!/usr/bin/env python3
"""Causal MTD-pace cool + single-bar burst cool consistency wave on locked sleeve.

Distinct from apply_mtd_gain_clip (fixed absolute tau) and from
apply_rolling_return_pctile_cool (multi-bar trail). Pace cool uses calendar
day-fraction * monthly_target * pace_mult as the dynamic MTD threshold.
Burst cool uses lag-1 |daily return| vs own high rolling percentile.
...
MONTHLY_TARGETS = [0.008, 0.01, 0.012]
PACE_MULTS = [1.0, 1.25, 1.5]
COOL_SCALES = [0.35, 0.5, 0.65]
BURST_HIST = [63, 126, 252]
BURST_PCT = [0.85, 0.90, 0.95]
...
def tag_pace(mt: float, pm: float, cool: float) -> str:
    return f"pace_mt{mt}_pm{pm}_c{cool}"


def tag_burst(hb: int, pct: float, cool: float) -> str:
    return f"burst_hb{hb}_p{pct}_c{cool}"
...
def _pace_grid():
    return itertools.product(MONTHLY_TARGETS, PACE_MULTS, COOL_SCALES)


def _burst_grid():
    return itertools.product(BURST_HIST, BURST_PCT, COOL_SCALES)


def pick_pace_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for mt, pm, cool in _pace_grid():
        params = {
            "monthly_target": float(mt),
            "pace_mult": float(pm),
            "cool_scale": float(cool),
        }
        st = cor.stats_from_port(
            apply_mtd_pace_cool(port24, **params, lo=0.25)
        )
    ...
    for item in ranked:
        key = (
            round(item[1]["monthly_target"], 4),
            round(item[1]["pace_mult"], 4),
...
def pick_burst_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for hb, pct, cool in _burst_grid():
        params = {
            "hist_bars": int(hb),
            "pctile": float(pct),
            "cool_scale": float(cool),
        ...
        st = cor.stats_from_port(
            apply_rolling_burst_cool(port24, **params, lo=0.25)
        )
        sc = cor.score_2024_pick(st, base_st24)
        if sc == float("-inf"):
            continue
...
def pick_pace_on_transformed(port24_xf, base_st_xf):
    best = None
    for mt, pm, cool in _pace_grid():
        params = {
            "monthly_target": float(mt),
        ...
        st = cor.stats_from_port(
            apply_mtd_pace_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    pace_soft_ports = {}
    # Sparse board: mt x pm mid x cool extremes
    sparse_pace = list(itertools.product(MONTHLY_TARGETS, [1.25], [0.35, 0.5, 0.65]))
    for mt, pm, cool in sparse_pace:
        params = {
            "monthly_target": float(mt),
            "pace_mult": float(pm),
            "cool_scale": float(cool),
        }
        ports = apply_ports(apply_mtd_pace_cool, base_ports, **params, lo=0.25)
        name = f"lock+{tag_pace(mt, pm, cool)}"
        row = cor.eval_idea(name, "pace", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"pace: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% "
            f"top3HO={row.top3_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            pace_soft_ports[name] = ports

    burst_soft_ports = {}
    sparse_burst = list(itertools.product([126], BURST_PCT, [0.35, 0.5, 0.65]))
    for hb, pct, cool in sparse_burst:
        params = {
            "hist_bars": int(hb),
            "pctile": float(pct),
            "cool_scale": float(cool),
        }
        ports = apply_ports(apply_rolling_burst_cool, base_ports, **params, lo=0.25)
        name = f"lock+{tag_burst(hb, pct, cool)}"
        row = cor.eval_idea(name, "burst", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"burst: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% "
            f"top3HO={row.top3_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            burst_soft_ports[name] = ports
    ...
    if w24 is not None:
        ...
        for sc, params, _ in pick_pace_2024(w24, wst, top_k=2):
            ports = apply_ports(
                apply_mtd_pace_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_pace(params['monthly_target'], params['pace_mult'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_pace", ports, is_labels, conf_labels)
            board.append(row)
            print(f"wrebal+pace: {name} soft={row.soft_pass} note={row.note}", flush=True)
            if row.soft_pass:
                soft_structures[name] = ports
        for sc, params, _ in pick_burst_2024(w24, wst, top_k=2):
            ports = apply_ports(
                apply_rolling_burst_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_burst(params['hist_bars'], params['pctile'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_burst", ports, is_labels, conf_labels)
            board.append(row)
            print(f"wrebal+burst: {name} soft={row.soft_pass} note={row.note}", flush=True)
            if row.soft_pass:
                soft_structures[name] = ports
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        picked = pick_pace_on_transformed(p24, st24)
        if picked is None or picked[0] == float("-inf"):
            continue
        _, pparams, _ = picked
        lports = apply_ports(apply_mtd_pace_cool, ports, **pparams, lo=0.25)
        iname = (
            f"{name}+"
            f"{tag_pace(pparams['monthly_target'], pparams['pace_mult'], pparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_pace", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+pace: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(pace_soft_ports.items())[:2]:
        soft_structures[n] = p
    for n, p in list(burst_soft_ports.items())[:2]:
        soft_structures[n] = p

    blend_targets = []
    for n, p in soft_structures.items():
        if "wrebal" in n and ("pace_" in n or "mtd_" in n or "burst_" in n):
    ...
    out_csv = ROOT / "reports" / "quest_mtd_pace_burst_board.csv"
    out_md = ROOT / "reports" / "quest_mtd_pace_burst_consistency.md"
    out_json = ROOT / "configs" / "quest_mtd_pace_burst_selected.json"
    ...
    sel = {
        ...
        "hard_n": hard_n,
        "promote_n": promote_n,
        "best": {k: (None if (isinstance(v, float) and (v != v)) else v) for k, v in best.items()},
    ...
    lines = [
        "# Quest wave — MTD pace cool + rolling burst cool",
        "",
        f"**data_source:** {ds}",
