...
Fixed priors: FRED BIS Q*PAM770A credit/GDP %, pub_lag_months=5 + signal_lag=1m.
Primary low_credit_gap_xs (long negative trailing-trend gap). No HO tuning.
Locked sleeve untouched. Distinct from debt (§29), fiscal (§28), REER (§31).
"""
...
from mt5_swing.strategies.bis_credit_fx import BisCreditFxConfig, bis_credit_factor_returns
...
def main() -> int:
    ...
    print("=== scholarly FX BIS private credit / credit-gap wave (§32) ===")
    print(
        "BIS private credit/GDP via FRED Q*PAM770A — Borio–Drehmann / Basel "
        "credit-cycle XS; distinct from debt (§29), fiscal (§28), TB (§30), "
        "REER (§31). Locked sleeve untouched."
    )

    try:
        credit = load_bis_credit_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL credit load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_bis_credit_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_bis_credit_wave.md").write_text(
            f"# Scholarly FX: BIS credit/GDP\n\n**FAILED:** {exc}\n"
        )
        return 1

    if credit.empty or credit.shape[1] < 3:
        msg = (
            f"Credit panel too thin: cols={list(credit.columns)}. "
            "Would fall back to TRESEG*/reserves wave — but board needs ≥3 ccys."
        )
        print(f"FATAL: {msg}")
        meta = {"ok": False, "error": msg, "promote": False, "fallback": "reserves_TRESEG"}
        (REPORTS / "scholarly_fx_bis_credit_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_bis_credit_wave.md").write_text(
            f"# Scholarly FX: BIS credit/GDP\n\n**FAILED data:** {msg}\n"
        )
    ...
    cov = credit_coverage(credit)
    cov.to_csv(REPORTS / "scholarly_fx_bis_credit_coverage.csv", index=False)
    credit.to_csv(REPORTS / "scholarly_fx_bis_credit_panel.csv")
    print(
        f"Credit panel cols={list(credit.columns)} "
        f"[{credit.dropna(how='all').index.min().date()}.."
        f"{credit.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))
    print(f"Series source note: {credit.attrs.get('source')}")
    print(f"Failed/alt documented: {credit.attrs.get('failed_or_alt')}")

    us_credit = None
    try:
        us_credit = load_us_bis_credit(download=True, force=False)
        print(
            f"US QUSPAM770A [{us_credit.dropna().index.min().date()}.."
            f"{us_credit.dropna().index.max().date()}] n={us_credit.dropna().shape[0]} "
            f"series={us_credit.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US credit load failed: {exc}")
        us_credit = None
    ...
    factors = bis_credit_factor_returns(
        pair_ret,
        credit,
        cfg=cfg,
        us_credit_override=us_credit,
    )
    if not factors:
        print("FATAL: no BIS credit factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_bis_credit_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_bis_credit_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_bis_credit_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_bis_credit_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": 32,
        "path": "bis_private_credit_gap",
        "data_tag": data_tag,
        "pub_lag_months": PUB_LAG_M,
        "signal_lag_months": cfg.signal_lag,
        "gap_lookback_months": cfg.gap_lookback,
        ...
        "normalisation": (
            "credit gap = credit/GDP − trailing mean (180m ≈ 60q HP-like one-sided); "
            "also level / 5y z / Δ12 boarded"
        ),
        "eur_mapping": "QXMPAM770A euro-area (QDEPAM770A Germany alt documented)",
        "distinct_from": [
            "debt_gdp_GGGDTA_29",
        ...
        "citations": [
            "Borio & Drehmann — assessing the risk of banking crises (BIS)",
            "Basel credit-to-GDP gap / financial-cycle early-warning",
            "Honesty alternate: high-credit debtor/fragile FX risk premium",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": credit.attrs.get("unmapped", []),
        "failed_or_alt": credit.attrs.get("failed_or_alt", {}),
        "soft_best": soft_best,
        "primary_sweep": prim_sweep.iloc[0].to_dict() if len(prim_sweep) else None,
    }
    (REPORTS / "scholarly_fx_bis_credit_meta.json").write_text(
    ...
    lines = [
        "# Scholarly FX: BIS private credit-to-GDP / credit-gap wave (§32)",
        "",
        "**Path:** BIS private credit/GDP via FRED `Q*PAM770A` — Borio–Drehmann / "
        "Basel credit-cycle gap XS — **distinct** from government debt/GDP (§29 "
        "GGGDTA*), fiscal GGNLBA (§28), TB (§30), BIS REER (§31), CA, CB-BS, "
        "funding-liq, macro-diff, equity-diff, AI-GPR, ToT, dollar-beta, crash-skew.",
        f"**Data:** `{data_tag}` + free FRED `Q*PAM770A` (% GDP). Brief `CRDQ*APABIS` "
        f"is absolute credit (documented unused). EUR = `QXMPAM770A` (euro-area; "
        f"`QDEPAM770A` Germany alt). Full G10 mapped incl. NZD/CHF. Pre-computed "
        f"credit-gap FRED IDs 404 — gap = level − trailing 180m mean (a priori). "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (conservative quarterly BIS) + "
        f"signal_lag={cfg.signal_lag}m + 1d weight lag.",
        f"**Primary:** `{PRIMARY}` (long negative credit gap / short positive — "
        "lean balance-sheet prior). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_bis_credit_*.csv`, "
        "`scholarly_fx_bis_credit_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_bis_credit_wave.md").write_text("\n".join(lines))
