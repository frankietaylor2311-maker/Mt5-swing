#!/usr/bin/env python3
"""Scholarly FX monthly trade-balance wave (high-freq external imbalance).

Fixed priors: OECD MEI XTEXVA/XTIMVA via FRED → TB/exports, pub_lag_months=2
+ signal_lag=1m. Optional CA-debtor blend (scholarly only). No HO tuning.
...
from mt5_swing.data.fred_current_account import load_ca_gdp_panel
...
def main() -> int:
    ...
    print("=== scholarly FX monthly trade-balance wave ===")
    print(
        "High-freq external imbalance — OECD MEI XTEXVA/XTIMVA via FRED "
        "(TB/exports); distinct from quarterly CA (§21), debt (§29), fiscal (§28). "
        "Locked sleeve untouched."
    )

    try:
        tb = load_trade_balance_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL TB load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_trade_balance_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_trade_balance_wave.md").write_text(
            f"# Scholarly FX: monthly trade-balance\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = trade_balance_coverage(tb)
    cov.to_csv(REPORTS / "scholarly_fx_trade_balance_coverage.csv", index=False)
    tb.to_csv(REPORTS / "scholarly_fx_trade_balance_panel.csv")
    print(
        f"TB panel cols={list(tb.columns)} "
        f"[{tb.dropna(how='all').index.min().date()}.."
        f"{tb.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))
    print(f"Series source note: {tb.attrs.get('source')}")
    print(f"Failed/alt documented: {tb.attrs.get('failed_or_alt')}")

    us_bop = None
    try:
        us_bop = load_us_bopgstb(download=True, force=False)
        print(
            f"US BOPGSTB [{us_bop.dropna().index.min().date()}.."
            f"{us_bop.dropna().index.max().date()}] n={us_bop.dropna().shape[0]} "
            f"series={us_bop.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US BOPGSTB load failed: {exc}")
        us_bop = None

    ca_debtor_ret = None
    try:
        ca = load_ca_gdp_panel(download=True, force=False)
        print(f"CA panel for blend: cols={list(ca.columns)}")
    except Exception as exc:  # noqa: BLE001
        print(f"WARN CA load for blend failed: {exc}")
        ca = None
    ...
    if ca is not None:
        try:
            ca_factors = current_account_factor_returns(
                pair_ret, ca, cfg=CurrentAccountFxConfig()
            )
            ca_debtor_ret = ca_factors.get("ca_debtor_xs")
        except Exception as exc:  # noqa: BLE001
            print(f"WARN CA factor build failed: {exc}")

    cfg = TradeBalanceFxConfig()
    # Prefer BOPGSTB (goods+services) for US tilts when available
    factors = trade_balance_factor_returns(
        pair_ret,
        tb,
        cfg=cfg,
        us_tb_override=us_bop,
        ca_debtor_returns=ca_debtor_ret,
    )
    if not factors:
        print("FATAL: no trade-balance factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_trade_balance_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_trade_balance_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_trade_balance_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_trade_balance_risk_sweep.csv", index=False)
    ...
    us_tilt_source = (
        us_bop.attrs.get("series_id")
        if us_bop is not None
        else tb.attrs.get("series_map", {}).get("USD")
    )
    meta = {
        "ok": True,
        "path": "monthly_trade_balance",
        "data_tag": data_tag,
        ...
        "normalisation": "tb_over_exports = (EXP-IMP)/EXP from OECD MEI XTEXVA/XTIMVA",
        ...
        "citations": [
            ...
            "Monthly goods TB as high-frequency external-balance channel vs quarterly CA",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": tb.attrs.get("unmapped", []),
        "failed_or_alt": tb.attrs.get("failed_or_alt", {}),
    }
    (REPORTS / "scholarly_fx_trade_balance_meta.json").write_text(
    ...
    lines = [
        ...
        "",
        "**Path:** High-frequency external imbalance — OECD MEI merchandise "
        "exports/imports via FRED → TB/exports — **distinct** from quarterly CA (§21), "
        "debt/GDP (§29), fiscal GGNLBA (§28), CB-BS, macro-diff, equity-diff, AI-GPR, "
        "ToT, dollar-beta, crash-skew.",
        f"**Data:** `{data_tag}` + free FRED `XTEXVA01*M667S` / `XTIMVA01*M667S` "
        f"+ US `BOPGSTB` for tilts. EUR = Germany proxy (EZM ends ~2023-04). "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (conservative monthly release) + "
        f"signal_lag={cfg.signal_lag}m + 1d weight lag.",
        f"**Primary:** `{PRIMARY}` (long deficit / short surplus — DCRS debtor premium). "
        "Full G10 mapped incl. NZD/CHF. Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_trade_balance_*.csv`, "
        "`scholarly_fx_trade_balance_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_trade_balance_wave.md").write_text("\n".join(lines))
