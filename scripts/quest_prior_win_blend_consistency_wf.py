#!/usr/bin/env python3
"""Prior-month win throttle + IS return-blend consistency wave on locked sleeve.
...
from mt5_swing.portfolio.overlays import (
    apply_mtd_gain_clip,
    apply_prior_month_win_throttle,
)
...
BLEND_ALPHAS = [0.25, 0.4, 0.55, 0.7]


def blend_ports(a: pd.Series, b: pd.Series, alpha: float) -> pd.Series:
    """Causal daily-return blend: (1-α)*a + α*b, rebuild equity from a's start."""
...
def tag_win(params) -> str:
    return f"win_t{params['win_tau']}_cs{params['cool_scale']}"
...
def pick_win_2024(port24, base_st24, top_k: int = 2):
    grid = list(
        itertools.product([0.015, 0.02, 0.025, 0.03], [0.35, 0.5, 0.65, 0.75])
    )
    ranked = []
    for wt, cs in grid:
        params = {"win_tau": float(wt), "cool_scale": float(cs)}
        st = cor.stats_from_port(
            apply_prior_month_win_throttle(
                port24,
                win_tau=params["win_tau"],
    ...
    seen = set()
    for item in ranked:
        key = (round(item[1]["win_tau"], 4), round(item[1]["cool_scale"], 4))
        if key in seen:
            continue
        seen.add(key)
        uniq.append(item)
...
def pick_win_on_transformed(port24_xf, base_st_xf):
    best = None
    for wt, cs in itertools.product(
        [0.015, 0.02, 0.025, 0.03], [0.35, 0.5, 0.65, 0.75]
    ):
        params = {"win_tau": float(wt), "cool_scale": float(cs)}
        st = cor.stats_from_port(
            apply_prior_month_win_throttle(
...
def main() -> None:
    ...
    # --- win throttle alone ---
    win_ranked = pick_win_2024(base24, base_st24, top_k=2) if base24 is not None else []
    best_win = win_ranked[0] if win_ranked else None
    if best_win:
        sc, params, _ = best_win
        ports = apply_ports(
            apply_prior_month_win_throttle,
            base_ports,
            win_tau=params["win_tau"],
        ...
        row = cor.eval_idea(
            f"lock+{tag_win(params)}", "win_throttle", ports, is_labels, conf_labels
        )
        board.append(row)
        print(f"win alone: {row.idea} soft={row.soft_pass} note={row.note}", flush=True)
    else:
        # diagnostic fallback: best gated by score even if -inf empty
        fallback = None
        for wt, cs in itertools.product(
            [0.015, 0.02, 0.025, 0.03], [0.35, 0.5, 0.65, 0.75]
        ):
            ...
            st = cor.stats_from_port(
                apply_prior_month_win_throttle(
                    base24, win_tau=params["win_tau"], cool_scale=params["cool_scale"], lo=0.25
                )
            )
            if st is None or not st.gates:
                continue
            sc = -st.top3 + 0.15 * st.mean_mo
            if fallback is None or sc > fallback[0]:
                fallback = (sc, params, st)
        if fallback:
            _, params, _ = fallback
            ports = apply_ports(
                apply_prior_month_win_throttle,
            ...
            row = cor.eval_idea(
                f"lock+{tag_win(params)}_fb", "win_throttle", ports, is_labels, conf_labels
            )
            board.append(row)
            best_win = fallback
            print(f"win fallback: {row.idea} note={row.note}", flush=True)

    # --- mtd prior alone ---
    for mtd in PRIOR_MTD:
        ports = apply_ports(
            apply_mtd_gain_clip, base_ports, tau=mtd["tau"], after_clip=mtd["after"]
        )
        row = cor.eval_idea(
            f"lock+{tag_mtd(mtd)}", "mtd_prior", ports, is_labels, conf_labels
        )
        board.append(row)
    ...
    # --- wrebal + mtd + win (nested win on 2024 transformed) ---
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        st24 = cor.stats_from_port(p24)
        picked = pick_win_on_transformed(p24, st24)
        ...
        wports = apply_ports(
            apply_prior_month_win_throttle,
            ports,
            win_tau=wparams["win_tau"],
            cool_scale=wparams["cool_scale"],
            lo=0.25,
        )
        iname = f"{name}+{tag_win(wparams)}"
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_win", wports, is_labels, conf_labels
        )
        board.append(row)
        wrebal_mtd_ports[iname] = (mtd, wports)
        print(
            f"wrebal+mtd+win: {iname} soft={row.soft_pass} note={row.note} "
            f"HO_pos={row.pos_holdout*100:.0f}%",
            flush=True,
        )

    # --- IS return blends: locked vs overlay ---
    blend_targets = []
    # locked vs best soft-ish wrebal+mtd
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        if "win_" in name or name.endswith("_frozen") and "+mtd_" in name:
            blend_targets.append((name, ports))
    # Prefer the two mtd-only wrebal ports + any win variants (cap)
    blend_targets = blend_targets[:4]

    for tname, tports in blend_targets:
        # Nested α on 2024 only
        best_a = None
        ...
        if best_a is None:
            continue
        _, alpha, _ = best_a
        # Also board all alphas lightly (only best + 0.4/0.55)
    ...
    out_csv = ROOT / "reports" / "quest_prior_win_blend_board.csv"
    out_md = ROOT / "reports" / "quest_prior_win_blend_consistency.md"
    out_json = ROOT / "configs" / "quest_prior_win_blend_selected.json"
    bdf.to_csv(out_csv, index=False)

    # Best soft by score among soft_pass, else best by score overall
    ...
    selected = {
        "wave": "prior_month_win_throttle_plus_is_blend",
        "data_source": ds,
        ...
        "best_win_params": best_win[1] if best_win else None,
    }
    ...
    show = bdf.sort_values("score", ascending=False).head(12)
    ...
    lines.append("")
    lines.append(f"**Promote: {'YES' if promote_n else 'NO'}. Locked tag unchanged unless promote.**")
    out_md.write_text("\n".join(lines) + "\n")
