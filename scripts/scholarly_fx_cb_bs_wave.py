...
Fixed priors: FRED WALCL / ECBASSETSW / JPNASSETS, weekly pub_lag=7d /
monthly pub_lag=1m + signal_lag=1d. No HO tuning. Locked sleeve untouched.
"""
...
INITIAL = 100_000.0
PRIMARY = "walcl_pb_fx"
PUB_LAG_W = DEFAULT_WEEKLY_PUB_LAG_DAYS
PUB_LAG_M = DEFAULT_MONTHLY_PUB_LAG_MONTHS
PUB_LAG_GDP_Q = DEFAULT_GDP_PUB_LAG_QUARTERS
...
def main() -> int:
    ...
    print("=== scholarly FX CB balance-sheet / QE differential wave ===")
    print(
        "Neely / Gagnon / Bauer–Neely — WALCL + ECBASSETSW + JPNASSETS; "
        "distinct from NFCI funding & CA imbalances. Locked sleeve untouched."
    )

    try:
        yoy = load_cb_yoy_panel(
            ("USD", "EUR", "JPY"),
            download=True,
            weekly_pub_lag_days=PUB_LAG_W,
            monthly_pub_lag_months=PUB_LAG_M,
            force=False,
        )
        walcl = load_cb_assets(
            "USD",
            download=False,
            weekly_pub_lag_days=PUB_LAG_W,
            monthly_pub_lag_months=PUB_LAG_M,
        )
        walcl_yoy = yoy_growth(walcl)
        diff = us_vs_peer_yoy_diff(yoy, peers=("EUR", "JPY"))
        us_bs_gdp = load_bs_gdp_ratio(
            "USD",
            download=True,
            weekly_pub_lag_days=PUB_LAG_W,
            monthly_pub_lag_months=PUB_LAG_M,
            gdp_pub_lag_quarters=PUB_LAG_GDP_Q,
            force=False,
        )
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL CB-BS load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_cb_bs_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_cb_bs_wave.md").write_text(
            f"# Scholarly FX: CB balance-sheet / QE\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = cb_coverage(yoy)
    cov.to_csv(REPORTS / "scholarly_fx_cb_bs_coverage.csv", index=False)
    yoy.to_csv(REPORTS / "scholarly_fx_cb_bs_yoy_panel.csv")
    diff.to_csv(REPORTS / "scholarly_fx_cb_bs_us_peer_diff.csv", header=["us_peer_diff"])
    print(
        f"YoY panel cols={list(yoy.columns)} "
        f"[{yoy.dropna(how='all').index.min().date()}.."
        f"{yoy.dropna(how='all').index.max().date()}] "
        f"pub_lag_w={PUB_LAG_W}d pub_lag_m={PUB_LAG_M}m"
    )
    ...
    factors = cb_balance_sheet_factor_returns(
        pair_ret,
        walcl_yoy=walcl_yoy,
        walcl_level=walcl,
        us_peer_diff=diff,
    ...
    if not factors:
        print("FATAL: no CB-BS factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_cb_bs_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_cb_bs_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_cb_bs_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_cb_bs_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "path": "cb_balance_sheet_qe_differential",
        "data_tag": data_tag,
        "weekly_pub_lag_days": PUB_LAG_W,
        "monthly_pub_lag_months": PUB_LAG_M,
        "gdp_pub_lag_quarters": PUB_LAG_GDP_Q,
        ...
        "citations": [
            "Gagnon, Raskin, Remache & Sack (2011), The Federal Reserve's Large-Scale Asset Purchases, IJCB",
            "Neely (2015), Unconventional Monetary Policy Effects on Exchange Rates, JBF",
            "Bauer & Neely (2014), International Channels of the Fed's Unconventional Monetary Policy, JIMF",
        ],
        "coverage": cov.to_dict(orient="records"),
        "uk_assets_note": "UKASSETS discontinued 2014-09 — not in primary panel",
    }
    (REPORTS / "scholarly_fx_cb_bs_meta.json").write_text(
    ...
    lines = [
        "# Scholarly FX: central-bank balance-sheet / QE differential wave",
        "",
        "**Path:** Neely / Gagnon / Bauer–Neely portfolio-balance QE–FX channel — "
        "**distinct** from NFCI funding-liquidity and CA/GDP imbalances.",
        f"**Data:** `{data_tag}` + free FRED `WALCL` / `ECBASSETSW` / `JPNASSETS` "
        f"(+ optional `GDP` for BS/GDP). **PIT:** weekly pub_lag={PUB_LAG_W}d, "
        f"monthly pub_lag={PUB_LAG_M}m, GDP pub_lag={PUB_LAG_GDP_Q}Q + "
        f"signal_lag={cfg.signal_lag}d.",
        f"**Primary:** `{PRIMARY}` (high Fed BS YoY → long FX / short USD). "
