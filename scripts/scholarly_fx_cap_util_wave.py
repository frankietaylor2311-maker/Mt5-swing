...
Fixed priors: FRED OECD MEI ULQECU01{ISO2}Q657S early-estimate CU YoY
(quarterly → monthly after pub_lag_months=3) + signal_lag=1m + 1d weight lag.
Primary high_cu_xs (high relative CU YoY → cyclical strength / demand pressure).
Scores on **YoY growth as reported**. Panel STALE ~2023-07 on free FRED.
No HO tuning. Locked sleeve untouched. Companion completing ULC §48 / LP §49
trilogy; distinct from employment §41, IP §42, ULC §48, LP §49, CLI/BCI,
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD capacity-utilization (ULQECU01) wave (§{SECTION}) ===")
    print(
        "OECD MEI ULQECU01{ISO2}Q657S CU YoY via FRED (quarterly → monthly after "
        "pub_lag=3) — CU YoY XS cyclical strength; STALE ~2023-07; companion "
        "completing ULC §48 / LP §49 trilogy; distinct from employment §41, IP §42, "
        "ULC §48, LP §49, CLI/BCI, building-permits, retail. "
        "Locked sleeve untouched."
    )

    try:
        lp = load_cap_util_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL CU load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_cap_util_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_cap_util_wave.md").write_text(
            f"# Scholarly FX: OECD capacity-utilization\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"CU panel too thin: cols={list(lp.columns)}. "
    ...
    cov = cap_util_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_cap_util_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_cap_util_panel.csv")
    print(
        f"CU panel cols={list(lp.columns)} "
    ...
    us_cu = None
    try:
        us_cu = load_us_cap_util(download=True, force=False)
        print(
            f"US ULQECU01USQ657S [{us_cu.dropna().index.min().date()}.."
            f"{us_cu.dropna().index.max().date()}] n={us_cu.dropna().shape[0]} "
            f"series={us_cu.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US CU load failed: {exc}")
        us_cu = None
    ...
    cfg = CapUtilFxConfig()
    factors = cap_util_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_cu_override=us_cu,
    )
    if not factors:
        print("FATAL: no CU factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_cap_util_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_cap_util_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_cap_util_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_cap_util_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_cap_util_ulqecu01",
        "data_tag": data_tag,
        ...
        "score_unit": "cap_util_yoy_pct_as_reported",
        "z_low": cfg.z_low,
        ...
        "normalisation": (
            "OECD MEI ULQECU01{ISO2}Q657S early-estimate CU YoY quarterly → "
            "monthly ffill after pub_lag=3; XS on YoY as reported; also 5y z / "
            "+Δ12 of YoY boarded; high_cu primary (cyclical strength → appreciate); "
            "n_long=n_short=2 full G10 foreign; signal_lag=1m + 1d weight lag; "
            "STALE panel ends ~2023-07 on free FRED"
        ),
        "eur_mapping": "ULQECU01DEQ657S Germany proxy (EZ also ~2023-07 stale; FR/IT/NL alts)",
        "coverage_g10": "full G10 mapped (USD/EUR/GBP/JPY/CAD/AUD/NZD/CHF ULQECU01*Q657S quarterly; STALE ~2023-07)",
        ...
        "citations": [
            ...
            "(OECD MEI ULQECU; Phillips-curve / output-gap adjacent)",
            "Companion completing ULC (§48) / LP (§49) trilogy",
            "Honesty: low CU / slack / stress debtor premium",
        ],
    ...
    (REPORTS / "scholarly_fx_cap_util_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD capacity-utilization (ULQECU01) differential wave (§{SECTION})",
        "",
        "**Path:** OECD MEI early-estimate capacity-utilization YoY via FRED (`ULQECU01{ISO2}Q657S`) — "
        "CU YoY XS cyclical strength — **companion** completing ULC (§48) / LP (§49) trilogy; "
        "**distinct** from employment (§41), IP (§42), ULC (§48), LP (§49), CLI/BCI, "
        "building-permits (§43), retail (§46).",
        f"**Data:** `{data_tag}` + free FRED ULQECU01 Q657S YoY (quarterly → "
        f"monthly after pub_lag). **STALE** — all G10 end ~2023-04..07 on free FRED. "
        f"EUR = `ULQECU01DEQ657S` (Germany proxy). Full G10 mapped. `n_long=n_short=2`. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (CU labour/activity a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **YoY growth as reported**.",
        f"**Primary:** `{PRIMARY}` (long high relative CU YoY / short low — "
        "cyclical strength → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_cap_util_*.csv`, "
        "`scholarly_fx_cap_util_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_cap_util_wave.md").write_text("\n".join(lines))
