#!/usr/bin/env python3
"""Causal daily loss-streak cool consistency wave on locked sleeve.
...
STREAK_NS = [2, 3, 4, 5]
COOL_SCALES = [0.25, 0.35, 0.5, 0.65]
...
def tag_ls(n: int, cool: float) -> str:
    return f"lstreak_n{n}_c{cool}"
...
def pick_lstreak_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for n, cool in itertools.product(STREAK_NS, COOL_SCALES):
        params = {"streak_n": int(n), "cool_scale": float(cool)}
        st = cor.stats_from_port(
            apply_daily_loss_streak_cool(
                port24,
                streak_n=params["streak_n"],
    ...
    seen = set()
    for item in ranked:
        key = (item[1]["streak_n"], round(item[1]["cool_scale"], 4))
        if key in seen:
            continue
        seen.add(key)
        uniq.append(item)
...
def pick_lstreak_on_transformed(port24_xf, base_st_xf):
    best = None
    for n, cool in itertools.product(STREAK_NS, COOL_SCALES):
        params = {"streak_n": int(n), "cool_scale": float(cool)}
        st = cor.stats_from_port(
...
def main() -> None:
    ...
    # --- lstreak alone: full grid on locked ---
    lstreak_soft_ports = {}
    for n, cool in itertools.product(STREAK_NS, COOL_SCALES):
        params = {"streak_n": int(n), "cool_scale": float(cool)}
        ports = apply_ports(
        ...
        name = f"lock+{tag_ls(n, cool)}"
        row = cor.eval_idea(name, "lstreak", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"lstreak: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            lstreak_soft_ports[name] = ports
    ...
    # --- wrebal + nested lstreak on 2024 ---
    soft_structures = {}
    w24 = wrebal_ports.get("2024")
    if w24 is not None:
        wst = cor.stats_from_port(w24)
        picked = pick_lstreak_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ...
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_ls(params['streak_n'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_lstreak", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"wrebal+lstreak: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                soft_structures[name] = ports

    # --- wrebal + mtd + nested lstreak ---
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        st24 = cor.stats_from_port(p24)
        picked = pick_lstreak_on_transformed(p24, st24)
        ...
        lports = apply_ports(
            apply_daily_loss_streak_cool,
            ports,
            streak_n=lparams["streak_n"],
            cool_scale=lparams["cool_scale"],
            lo=0.25,
        )
        iname = f"{name}+{tag_ls(lparams['streak_n'], lparams['cool_scale'])}"
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_lstreak", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+lstreak: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(lstreak_soft_ports.items())[:2]:
        soft_structures[n] = p
    ...
    out_csv = ROOT / "reports" / "quest_loss_streak_board.csv"
    out_md = ROOT / "reports" / "quest_loss_streak_consistency.md"
    out_json = ROOT / "configs" / "quest_loss_streak_selected.json"
    ...
    selected = {
        "wave": "causal_daily_loss_streak_cool",
        "data_source": ds,
    ...
    lines = [
        "# Daily loss-streak cool consistency wave",
        "",
