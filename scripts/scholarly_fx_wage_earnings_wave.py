#!/usr/bin/env python3
"""Scholarly FX OECD/FRED manufacturing wage/earnings FX differential wave (§55).

Fixed priors: FRED OECD MEI LCEAMN01 manufacturing earnings YoY
(USD/CAD *M657S; EUR=*EZQ657S; NZD=*NZQ657S; GBP/JPY *M661S→YoY;
AUD *AUQ661S→YoY; CHF unmapped), pub_lag_months=2 + signal_lag=1m +
signal_lag_days=1. Primary low_wage_growth_xs (competitiveness —
slower wage growth → appreciate). Scores on **wage YoY %**. No HO
tuning. Locked sleeve untouched. Distinct from ULC §48, employment §41,
UR §54, LP §49, CU §50, PPI/CPI.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED manufacturing wage/earnings wave (§{SECTION}) ===")
    print(
        "FRED LCEAMN wage YoY (pub_lag=2) — low_wage_growth XS; competitiveness "
        "prior; CHF unmapped; EUR EZ Q through ~2025-07; distinct from ULC §48, "
        "employment §41, UR §54. Locked sleeve untouched."
    )

    try:
        lp = load_wage_earnings_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL wage load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_wage_earnings_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_wage_earnings_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED manufacturing wage/earnings\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"wage panel too thin: cols={list(lp.columns)}. "
    ...
    cov = wage_earnings_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_wage_earnings_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_wage_earnings_panel.csv")
    print(
        f"wage panel cols={list(lp.columns)} "
    ...
    us_wage = None
    try:
        us_wage = load_us_wage_earnings(download=True, force=False)
        print(
            f"US LCEAMN01USM657S [{us_wage.dropna().index.min().date()}.."
            f"{us_wage.dropna().index.max().date()}] n={us_wage.dropna().shape[0]} "
            f"series={us_wage.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US wage load failed: {exc}")
        us_wage = None
    ...
    factors = wage_earnings_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_wage_override=us_wage,
    )
    if not factors:
        print("FATAL: no wage factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_wage_earnings_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_wage_earnings_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_wage_earnings_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_wage_earnings_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_manufacturing_earnings_lceamn",
        "data_tag": data_tag,
        ...
        "normalisation": (
            ...
            "XS on wage YoY %; also 5y z / −Δ12 of YoY boarded; "
            "low_wage_growth primary (competitiveness → appreciate); "
            "n_long=n_short=2 on available foreign panel"
        ),
        "eur_mapping": "LCEAMN01EZQ657S EA manufacturing earnings YoY (ends ~2025-07; DE/FR alts)",
        "coverage_g10": "USD/EUR/GBP/JPY/AUD/CAD/NZD mapped; CHF unmapped on FRED (404)",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "yoy_growth_pct",
        "distinct_from": [
        ...
        "citations": [
            "Balassa–Samuelson wage / relative labour-cost competitiveness channel",
            "Dahlquist & Hasseltoft (2020), JFE — economic momentum labour companion",
            "OECD MEI LCEAMN01 manufacturing earnings (ULC §48 documented fallback — now dedicated)",
            "Honesty: high wage growth / labour-cost stress debtor premium",
        ],
    ...
    (REPORTS / "scholarly_fx_wage_earnings_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED manufacturing wage/earnings differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **manufacturing earnings YoY** "
        "(`LCEAMN01*`; CHF unmapped 404) — wage YoY XS — "
        "**distinct** from ULC (§48), employment persons (§41), UR levels (§54), "
        "LP (§49), CU (§50), CPI (§52), PPI (§51).",
        f"**Data:** `{data_tag}` + free FRED LCEAMN YoY after pub_lag. "
        f"EUR EA EZQ657S ends ~2025-07; CHF unmapped; GBP/JPY/AUD index→YoY. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (earnings lag UR/CPI a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **wage YoY %**.",
        f"**Primary:** `{PRIMARY}` (long low relative wage growth / short high — "
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_wage_earnings_*.csv`, "
        "`scholarly_fx_wage_earnings_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_wage_earnings_wave.md").write_text("\n".join(lines))
