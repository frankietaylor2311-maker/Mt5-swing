#!/usr/bin/env python3
"""Scholarly FX global-imbalances / current-account wave (GR / DCRS style).

Fixed priors: IMF BOP CA/GDP via FRED, pub_lag_quarters=2 + signal_lag=1m.
...
INITIAL = 100_000.0
PRIMARY = "ca_debtor_xs"
PUB_LAG_Q = DEFAULT_PUB_LAG_QUARTERS
...
def main() -> int:
    ...
    print("=== scholarly FX global-imbalances / current-account wave ===")
    print(
        "Gourinchas–Rey / Della Corte–Riddiough–Sarno — IMF BOP CA/GDP via FRED; "
        "distinct from PPP/BS/macro-diff. Locked sleeve untouched."
    )

    try:
        ca = load_ca_gdp_panel(pub_lag_quarters=PUB_LAG_Q, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL CA load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False}
        (REPORTS / "scholarly_fx_ca_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_ca_wave.md").write_text(
            f"# Scholarly FX: current-account / global imbalances\n\n**FAILED:** {exc}\n"
        )
        return 1

    cov = ca_coverage(ca)
    cov.to_csv(REPORTS / "scholarly_fx_ca_coverage.csv", index=False)
    ca.to_csv(REPORTS / "scholarly_fx_ca_gdp_panel.csv")
    print(
        f"CA panel cols={list(ca.columns)} "
        f"[{ca.dropna(how='all').index.min().date()}..{ca.dropna(how='all').index.max().date()}] "
        f"pub_lag_q={PUB_LAG_Q}"
    )
    ...
    factors = current_account_factor_returns(pair_ret, ca, cfg=cfg)
    if not factors:
        print("FATAL: no CA factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_ca_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_ca_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_ca_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_ca_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "path": "global_imbalances_current_account",
        "data_tag": data_tag,
        "pub_lag_quarters": PUB_LAG_Q,
        ...
        "distinct_from": ["PPP", "Balassa_Samuelson", "macro_diff_CPI_IP_UR", "Hau_Rey_equity"],
        "citations": [
            "Della Corte, Riddiough & Sarno (2016), Currency Premia and Global Imbalances, RFS",
            "Gourinchas & Rey (2007), International Financial Adjustment, JPE",
        ],
        "coverage": cov.to_dict(orient="records"),
    }
    (REPORTS / "scholarly_fx_ca_meta.json").write_text(json.dumps(meta, indent=2, default=str))

    lines = [
        "# Scholarly FX: global imbalances / current-account wave",
        "",
        "**Path:** Gourinchas–Rey / Della Corte–Riddiough–Sarno external-adjustment "
        "/ NFA–CA channel — **distinct** from PPP, BS, macro-diff, Hau–Rey equity.",
        f"**Data:** `{data_tag}` + free FRED IMF BOP `{{ISO3}}B6BLTT02STSAQ` (CA/GDP %). "
        f"**PIT:** pub_lag_quarters={PUB_LAG_Q} (=6m) + signal_lag={cfg.signal_lag}m + 1d weight lag.",
        f"**Primary:** `{PRIMARY}` (long deficit / short surplus). Locked sleeve untouched.",
        "",
