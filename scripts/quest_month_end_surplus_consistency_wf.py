#!/usr/bin/env python3
"""Causal month-end MTD-surplus cool consistency wave on locked sleeve.

Distinct from apply_mtd_pace_cool (continuous day_frac×target×pace_mult) and
apply_mtd_gain_clip (fixed absolute tau all month). Month-end surplus cool
fires only when day_of_month >= start_day AND lag-1 MTD >= surplus.
...
START_DAYS = [15, 18, 20, 22]
SURPLUS = [0.005, 0.008, 0.01, 0.012]
...
def tag_surplus(sd: int, sur: float, cool: float) -> str:
    return f"mesurp_sd{sd}_s{sur}_c{cool}"
...
def _surplus_grid():
    return itertools.product(START_DAYS, SURPLUS, COOL_SCALES)


def pick_surplus_2024(port24, base_st24, top_k: int = 3):
    ranked = []
    for sd, sur, cool in _surplus_grid():
        params = {
            "start_day": int(sd),
            "surplus": float(sur),
        ...
        st = cor.stats_from_port(
            apply_month_end_surplus_cool(port24, **params, lo=0.25)
        )
...
def pick_surplus_on_transformed(port24_xf, base_st_xf):
    best = None
    for sd, sur, cool in _surplus_grid():
        params = {
            "start_day": int(sd),
        ...
        st = cor.stats_from_port(
            apply_month_end_surplus_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    surplus_soft_ports = {}
    # Sparse board: start_days x mid surplus x cool extremes
    sparse_surplus = list(itertools.product(START_DAYS, [0.008], COOL_SCALES))
    for sd, sur, cool in sparse_surplus:
        params = {
            "start_day": int(sd),
            "surplus": float(sur),
            "cool_scale": float(cool),
        }
        ports = apply_ports(apply_month_end_surplus_cool, base_ports, **params, lo=0.25)
        name = f"lock+{tag_surplus(sd, sur, cool)}"
        row = cor.eval_idea(name, "month_end_surplus", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"surplus: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
            f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% "
            f"top3HO={row.top3_holdout*100:.0f}% note={row.note}",
            flush=True,
        )
        if row.soft_pass:
            surplus_soft_ports[name] = ports

    # Also nest top-k surplus alone on locked (2024-only pick → board)
    if base24 is not None and base_st24 is not None:
        for sc, params, _ in pick_surplus_2024(base24, base_st24, top_k=3):
            ports = apply_ports(
                apply_month_end_surplus_cool, base_ports, **params, lo=0.25
            )
            name = (
                f"lock+{tag_surplus(params['start_day'], params['surplus'], params['cool_scale'])}"
                f"_nested"
            )
            # skip if already on sparse board under same params without _nested
            row = cor.eval_idea(
                name, "month_end_surplus_nested", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"surplus_nested: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    if w24 is not None:
        ...
        for sc, params, _ in pick_surplus_2024(w24, wst, top_k=2):
            ports = apply_ports(
                apply_month_end_surplus_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_surplus(params['start_day'], params['surplus'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(
                name, "frozen_wrebal_surplus", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"wrebal+surplus: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        picked = pick_surplus_on_transformed(p24, st24)
        if picked is None or picked[0] == float("-inf"):
            continue
        _, sparams, _ = picked
        lports = apply_ports(
            apply_month_end_surplus_cool, ports, **sparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_surplus(sparams['start_day'], sparams['surplus'], sparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_surplus", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+surplus: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(surplus_soft_ports.items())[:3]:
        soft_structures[n] = p

    blend_targets = []
    for n, p in soft_structures.items():
        if "wrebal" in n and ("mesurp_" in n or "mtd_" in n):
    ...
    out_csv = ROOT / "reports" / "quest_month_end_surplus_board.csv"
    out_md = ROOT / "reports" / "quest_month_end_surplus_consistency.md"
    out_json = ROOT / "configs" / "quest_month_end_surplus_selected.json"
    ...
    lines = [
        "# Quest wave — month-end MTD-surplus cool",
        "",
        f"**data_source:** {ds}",
