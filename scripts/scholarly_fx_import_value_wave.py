#!/usr/bin/env python3
"""Scholarly FX OECD/FRED merchandise import-value YoY (XTIMVA01) FX differential wave (§62).

Fixed priors: FRED OECD MEI XTIMVA01 merchandise import-value YoY %
(full G10 *M657S; EUR=XTIMVA01DEM657S Germany proxy — EZM657S 404,
EZM667S stale ~2023-04; unit=VALUE not volume),
pub_lag_months=2 + signal_lag=1m + signal_lag_days=1. Primary high_ival_xs
(growth-channel / domestic-absorption/import-demand). Scores on **merchandise import-value
YoY %**. No HO tuning. Locked sleeve untouched. Distinct from export-value
§61, trade-balance §30, CA §21, ToT, retail §46, IP §42, cars §60, construction §59, GDP §58.

Fall-through: CIP/cross-currency basis Missing free G10; hours/vacancies
thin/stale; services PRSVTT* / mfg turnover ODMN* 404.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED merchandise import-value wave (§{SECTION}) ===")
    print(
        "FRED XTIMVA01 merchandise import-value YoY (M657S; pub_lag=2) — high_ival XS; "
        "growth-channel / domestic-absorption/import-demand prior; EUR=DEM Germany proxy; "
        "full G10; distinct from export §61, TB §30, CA §21, retail §46, IP §42, cars §60, construction §59, GDP §58. "
        "Locked sleeve untouched. Fall-through: CIP/basis Missing; hours/vacancies thin; PRSVTT*/ODMN* 404. "
        "Unit=VALUE not volume."
    )

    try:
        lp = load_import_value_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL import_value load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_import_value_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_import_value_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED merchandise import-value\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"import_value panel too thin: cols={list(lp.columns)}. "
    ...
    cov = import_value_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_import_value_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_import_value_panel.csv")
    print(
        f"import_value panel cols={list(lp.columns)} "
    ...
    us_ival = None
    try:
        us_ival = load_us_import_value(download=True, force=False)
        print(
            f"US XTIMVA01USM657S YoY [{us_ival.dropna().index.min().date()}.."
            f"{us_ival.dropna().index.max().date()}] n={us_ival.dropna().shape[0]} "
            f"series={us_ival.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US import_value load failed: {exc}")
        us_ival = None
    ...
    factors = import_value_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_ival_override=us_ival,
    )
    if not factors:
        print("FATAL: no import_value factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_import_value_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_import_value_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_import_value_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_import_value_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_merchandise_import_value_xtimva01",
        "data_tag": data_tag,
        ...
        "score_unit": "import_value_yoy_pct",
        "z_low": cfg.z_low,
        ...
        "normalisation": (
            "FRED OECD MEI XTIMVA01 merchandise import-value YoY % "
            "(full G10 *M657S; EUR=XTIMVA01DEM657S Germany proxy — EZM657S 404, "
            "EZM667S stale ~2023-04; unit=VALUE not volume; *M667S levels not mixed); "
            "XS on import-value YoY %; also 5y z / +Δ12 of YoY boarded; "
            "high_ival primary (growth-channel / domestic absorption / import demand → appreciate); "
            "n_long=n_short=2 on available foreign panel (full G10)"
        ),
        "eur_mapping": "XTIMVA01DEM657S Germany proxy (EZM657S 404; EZM667S stale ~2023-04)",
        "coverage_g10": "full G10 mapped (USD/EUR/GBP/JPY/CAD/AUD/CHF/NZD)",
        "series_unit_caveat": "value_not_volume",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "import_value_yoy_pct",
        "distinct_from": [
            "export_value_61",
            "trade_balance_30",
            "current_account_gdp_21",
            "terms_of_trade_uv_404",
            "retail_sales_46",
            "industrial_production_42",
            "passenger_car_regs_60",
            "construction_production_59",
            "real_gdp_growth_58",
            "building_permits_43",
            "oecd_cli_37",
            "oecd_bci_39",
        ...
        "citations": [
            "Dahlquist & Hasseltoft (2020), JFE — economic momentum / growth differentials",
            "Growth-channel FX on merchandise import-value YoY (domestic absorption / import demand)",
            "OECD MEI XTIMVA01 merchandise import-value YoY (M657S; value not volume)",
            "Honesty: low import-value growth / import-demand-stress premium",
            "Fall-through: CIP/basis Missing free G10; hours/vacancies thin/stale; PRSVTT*/ODMN* 404",
        ],
        "fallthrough_from": [
    ...
    (REPORTS / "scholarly_fx_import_value_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED merchandise import-value differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **merchandise import-value YoY** "
        "(`XTIMVA01` M657S; EUR=Germany DEM proxy; full G10; unit=VALUE not volume) — import-value YoY XS — "
        "**distinct** from trade-balance (§30), CA (§21), ToT (UV 404), retail (§46), IP (§42), "
        "passenger-cars (§60), construction (§59), GDP (§58).",
        f"**Data:** `{data_tag}` + free FRED XTIMVA01 YoY after pub_lag. "
        f"EUR=XTIMVA01DEM657S Germany proxy (EZM657S 404); full G10 live. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (trade/import ~1–2m lag a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **merchandise import-value YoY %** (value not volume).",
        f"**Primary:** `{PRIMARY}` (long high relative import-value YoY / short low — "
        "growth-channel / domestic absorption / import demand → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_import_value_*.csv`, "
        "`scholarly_fx_import_value_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_import_value_wave.md").write_text("\n".join(lines))
