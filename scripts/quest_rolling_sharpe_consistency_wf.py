#!/usr/bin/env python3
"""Causal rolling Sharpe cool consistency wave on locked sleeve.
...
LOOKBACKS = [21, 42, 63]
HIGH_THRESH = [1.0, 1.5, 2.0, 2.5]
LOW_THRESH = [-1.0, -0.5, 0.0]
...
def tag_rs(lb: int, hi: float, lo_th: float, cool: float) -> str:
    return f"rsharpe_lb{lb}_hi{hi}_lo{lo_th}_c{cool}"
...
def _rs_params(lb, hi, lo_th, cool):
    return {
...
def pick_rsharpe_2024(port24, base_st24, top_k: int = 5):
    ranked = []
    for lb, hi, lo_th, cool in itertools.product(
        LOOKBACKS, HIGH_THRESH, LOW_THRESH, COOL_SCALES
    ):
        params = _rs_params(lb, hi, lo_th, cool)
        st = cor.stats_from_port(
            apply_rolling_sharpe_cool(port24, **params, lo=0.25)
        )
...
def pick_rsharpe_on_transformed(port24_xf, base_st_xf):
    best = None
    for lb, hi, lo_th, cool in itertools.product(
        LOOKBACKS, HIGH_THRESH, LOW_THRESH, COOL_SCALES
    ):
        params = _rs_params(lb, hi, lo_th, cool)
        st = cor.stats_from_port(
            apply_rolling_sharpe_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    # --- rsharpe alone: top-k 2024 picks on locked (prune full 108-cell grid) ---
    rsharpe_soft_ports = {}
    if base24 is not None and base_st24 is not None:
        picked = pick_rsharpe_2024(base24, base_st24, top_k=6)
        print(f"rsharpe alone: confirming top {len(picked)} / 108 2024 picks", flush=True)
        for sc, params, _ in picked:
            ports = apply_ports(apply_rolling_sharpe_cool, base_ports, **params, lo=0.25)
            name = (
                f"lock+{tag_rs(params['lookback_days'], params['high_thresh'], params['low_thresh'], params['cool_scale'])}"
            )
            row = cor.eval_idea(name, "rsharpe", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"rsharpe: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
                f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                rsharpe_soft_ports[name] = ports
    ...
    # --- wrebal + nested rsharpe on 2024 ---
    soft_structures = {}
    w24 = wrebal_ports.get("2024")
    if w24 is not None:
        wst = cor.stats_from_port(w24)
        picked = pick_rsharpe_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ports = apply_ports(apply_rolling_sharpe_cool, wrebal_ports, **params, lo=0.25)
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_rs(params['lookback_days'], params['high_thresh'], params['low_thresh'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_rsharpe", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"wrebal+rsharpe: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                soft_structures[name] = ports

    # --- wrebal + mtd + nested rsharpe ---
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        st24 = cor.stats_from_port(p24)
        picked = pick_rsharpe_on_transformed(p24, st24)
        if picked is None or picked[0] == float("-inf"):
            continue
        _, rsparams, _ = picked
        lports = apply_ports(
            apply_rolling_sharpe_cool, ports, **rsparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_rs(rsparams['lookback_days'], rsparams['high_thresh'], rsparams['low_thresh'], rsparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_rsharpe", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+rsharpe: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(rsharpe_soft_ports.items())[:2]:
        soft_structures[n] = p
    ...
    out_csv = ROOT / "reports" / "quest_rolling_sharpe_board.csv"
    out_md = ROOT / "reports" / "quest_rolling_sharpe_consistency.md"
    out_json = ROOT / "configs" / "quest_rolling_sharpe_selected.json"
    ...
    selected = {
        "wave": "causal_rolling_sharpe_cool",
        "data_source": ds,
