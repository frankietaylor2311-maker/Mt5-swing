...
Multi-factor structure: Rogoff-style PPP / real exchange-rate cross-sectional
value gated / cooled by VIX (Menkhoff global FX-vol proxy) and aggregate
Caldara–Iacoviello GPR.
Literature: Rogoff PPP / real FX + Menkhoff et al. (2012 JF) + Caldara–
Iacoviello (2022) / Liu–Zhang (2024).

Fixed priors: VIX/GPR bar_lag=1 + signal_lag=1d on trailing z (z_window=252,
min_periods=60), gate z≤0 / cool at z≥1 (§71–§74 symmetry — lit GPR often
1.5; documented), PPP lookback=60m (honesty 120m in raw ppp wave), CPI
pub_lag + ppp_signal_lag=1m + weight lag 1d, costs 1.5 bps/side.
Primary ``value_low_vix``. Honesty: ``value_raw`` + ``value_high_vix``.
No HO tuning. Locked sleeve untouched.

Distinct from: raw ppp_wave / bis_reer, soft EW §66, VIX/GPR soft §72,
VIX/GPR carry §73, VIX/GPR mom §74, CIP waves, capital-sleeve §53/§70,
combo §8.
...
PREFIX = "scholarly_fx_vix_gpr_conditioned_value"
...
def write_locked_verify() -> dict:
    ...
    lines = [
        f"# One-pct month quest — locked_verify_vix_gpr_conditioned_value",
        "",
    ...
    lines += [
        "",
        "Config unmodified vs §74 mom / §73 carry / §72 soft "
        "— canonical windowed PASS.",
        "",
    ]
    (REPORTS / "quest_locked_verify_vix_gpr_conditioned_value.md").write_text(
        "\n".join(lines)
    )
    log = (
        f"Locked sleeve re-verify (§{SECTION} vix_gpr_conditioned_value): "
        ...
        f"Wrote {REPORTS / 'quest_locked_verify_vix_gpr_conditioned_value.md'}\n"
    )
    (REPORTS / "quest_locked_verify_vix_gpr_conditioned_value.log").write_text(log)
    print(log, end="")
...
def main() -> int:
    ...
    print(
        f"=== scholarly FX VIX/GPR-conditioned Rogoff PPP / real-FX value wave "
        f"(§{SECTION}) ==="
    )
    print(
        f"VIX + aggregate GPR stress gate/cool on Rogoff PPP real-FX value — primary {PRIMARY}; "
        "Rogoff PPP / real FX + Menkhoff et al. (2012 JF) + "
    ...
    try:
        ...
        (REPORTS / f"{PREFIX}_wave.md").write_text(
            f"# Scholarly FX: VIX/GPR-conditioned value\n\n**FAILED:** {exc}\n"
        )
        return 1
    ...
    try:
        ...
        print(
            f"CPI levels cols={list(cpi.columns)} pub_lag={pub_lag} "
            f"[{cpi.dropna(how='all').index.min().date()}.."
            f"{cpi.dropna(how='all').index.max().date()}]"
        )
    except Exception as exc:  # noqa: BLE001
    ...
    factors = vix_gpr_conditioned_value_factor_returns(
        pair_close, pair_ret, cpi, vix=vix, gpr=gpr, cfg=cfg
    )
    if not factors or PRIMARY not in factors:
        print("FATAL: no vix_gpr_conditioned_value factors produced")
        return 1
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "rogoff_ppp_vix_caldara_gpr_conditioned_real_fx_value",
        "data_tag": data_tag,
        "vix_bar_lag": 1,
        "gpr_bar_lag": 1,
        "signal_lag_days": cfg.signal_lag,
        "ppp_signal_lag_months": cfg.ppp_signal_lag,
        "ppp_lookback_months": cfg.lookback,
        "ppp_honesty_lookback_months": cfg.honesty_lookback,
        "ppp_weight_lag_days": cfg.ppp_weight_lag_days,
        "cpi_pub_lag_months": DEFAULT_PUB_LAGS["cpi"],
        ...
        "primary": PRIMARY,
        "primary_sign_prior": (
            "trade Rogoff PPP / real-FX value only when lagged VIX z ≤ 0 "
            "— value weakens when global FX-vol elevated "
        ...
        "data_caveat": (
            ...
            "VIX/GPR gate z≤0 / cool z≥1 fixed a priori (§71–§74 symmetry; "
            "lit GPR often 1.5 — documented, not HO-tuned). "
            "z_window=252 daily (NOT monthly CIP 60). "
            "PPP lookback=60m (honesty 120m in raw ppp wave); CPI pub_lag + "
            "ppp_signal_lag=1m. AU/NZ CPI quarterly ffilled — documented."
        ),
        "normalisation": (
            f"VIX/GPR trailing z {cfg.z_window}d min_periods={cfg.min_periods}; "
            f"bar_lag=1 + signal_lag={cfg.signal_lag}d; gate z≤0 / cool z≥"
            f"{cfg.z_high}; PPP lookback={cfg.lookback}m n_long=n_short={cfg.n_long}"
        ),
        "cpi_start": str(cpi.dropna(how="all").index.min().date()),
        "cpi_end": str(cpi.dropna(how="all").index.max().date()),
    ...
    lines = [
        f"# Scholarly FX: VIX/GPR-conditioned Rogoff PPP / real-FX value wave (§{SECTION})",
        "",
        "**Path:** Menkhoff **VIX** (global FX-vol proxy) + Caldara–Iacoviello "
        "**aggregate GPR** stress × Rogoff PPP / real-FX cross-sectional value "
        "gate/cool — **distinct** from raw ppp_wave / bis_reer, combo (§8), "
        "soft EW (§66), VIX/GPR soft (§72), VIX/GPR carry (§73), VIX/GPR mom (§74), "
        "CIP waves, capital-sleeve (§53/§70).",
        f"**Data:** `{data_tag}` + `vix_yahoo.csv` + `gpr_daily.csv` (bar_lag=1) + "
        f"FRED CPI levels (pub_lag={DEFAULT_PUB_LAGS['cpi']}m). "
        f"G10 trade: EUR/GBP/JPY/AUD/CAD/CHF/NZD. "
        f"`lookback={cfg.lookback}m` `n_long=n_short={cfg.n_long}`. **PIT:** "
        f"VIX/GPR bar_lag=1 + signal_lag={cfg.signal_lag}d on trailing z "
        f"({cfg.z_window}d); CPI pub_lag + ppp_signal_lag={cfg.ppp_signal_lag}m + "
        f"weight lag {cfg.ppp_weight_lag_days}d.",
        f"**Primary:** `{PRIMARY}` (trade value only when lagged VIX z ≤ 0). "
        "Honesty `value_raw` + `value_high_vix`. Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        f"CPI ends **{cpi.dropna(how='all').index.max().date()}**. "
        "Gate z≤0 / cool z≥1 fixed (§71–§74 symmetry; lit GPR often 1.5 — "
        "documented, not HO-tuned). z_window=252 daily (NOT monthly CIP 60). "
        f"PPP lookback={cfg.lookback}m (honesty {cfg.honesty_lookback}m in raw "
        "ppp wave). AU/NZ CPI quarterly ffilled.",
        "",
        f"Artifacts: `reports/{PREFIX}_*.csv`, "
        f"`{PREFIX}_meta.json`, `quest_locked_verify_vix_gpr_conditioned_value.md`.",
        "",
    ]
    (REPORTS / f"{PREFIX}_wave.md").write_text("\n".join(lines))
