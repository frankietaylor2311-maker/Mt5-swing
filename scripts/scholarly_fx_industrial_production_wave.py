#!/usr/bin/env python3
"""Scholarly FX industrial-production (OECD MEI PRINTO01) differential wave (§42).

Fixed priors: FRED OECD MEI {ISO3}PRINTO01GYSAM industry YoY (monthly) +
AUD/NZD/CHF PRMNTO01GYSAQ manufacturing YoY (quarterly → monthly after
pub_lag_months=2), + signal_lag_days=1 (no extra month lag).
Primary high_ip_xs (high relative IP YoY → appreciate). Scores on **YoY
growth as reported**. No HO tuning. Locked sleeve untouched.
Distinct from macro-diff EW CPI/IP/UR, OECD CLI/CCI/BCI, employment §41,
WUI, EPU/TPU, CA/TB, fiscal/debt, BIS, reserves, house-price, money,
...
def main() -> int:
    ...
    print(f"=== scholarly FX Industrial-production (PRINTO01) wave (§{SECTION}) ===")
    print(
        "OECD MEI PRINTO01GYSAM industry YoY via FRED (monthly + AUD/NZD/CHF "
        "PRMNTO01GYSAQ quarterly → monthly after pub_lag=2) — IP YoY XS; distinct from "
        "macro-diff EW, OECD CLI/CCI/BCI, employment §41, WUI, EPU/TPU, IG OAS. "
        "Locked sleeve untouched."
    )

    try:
        ip = load_industrial_production_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL IP load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_industrial_production_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_industrial_production_wave.md").write_text(
            f"# Scholarly FX: Industrial-production\n\n**FAILED:** {exc}\n"
        )
        return 1

    if ip.empty or ip.shape[1] < 4:
        msg = (
            f"IP panel too thin: cols={list(ip.columns)}. "
    ...
    cov = industrial_production_coverage(ip)
    cov.to_csv(REPORTS / "scholarly_fx_industrial_production_coverage.csv", index=False)
    ip.to_csv(REPORTS / "scholarly_fx_industrial_production_panel.csv")
    print(
        f"IP panel cols={list(ip.columns)} "
    ...
    try:
        ...
        print(
            f"US USAPRINTO01GYSAM [{us_ip.dropna().index.min().date()}.."
            f"{us_ip.dropna().index.max().date()}] n={us_ip.dropna().shape[0]} "
            f"series={us_ip.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US IP load failed: {exc}")
        us_ip = None
    ...
    factors = industrial_production_factor_returns(
        pair_ret,
        ip,
        cfg=cfg,
        us_ip_override=us_ip,
    )
    if not factors:
        print("FATAL: no IP factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_industrial_production_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_industrial_production_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_industrial_production_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_industrial_production_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_industrial_production_printo01",
        "data_tag": data_tag,
        ...
        "score_unit": "ip_yoy_pct_as_reported",
        "z_low": cfg.z_low,
        ...
        "normalisation": (
            "OECD MEI {ISO3}PRINTO01GYSAM industry YoY monthly + "
            "AUD/NZD/CHF PRMNTO01GYSAQ manufacturing YoY quarterly → "
            "monthly ffill after pub_lag=2); XS on YoY as reported; also 5y z / "
            "Δ12 of YoY boarded; high_ip primary (growth-channel → appreciate); "
            "n_long=n_short=2 full G10 foreign"
        ),
        "eur_mapping": "FRAPRINTO01GYSAM France proxy (EA19/DEU GYSAM stale; ITA/ESP/NLD alts)",
        "coverage_g10": "full G10 mapped (USD/GBP/JPY/CAD/EUR monthly PRINTO01GYSAM; AUD/NZD/CHF quarterly PRMNTO01GYSAQ)",
        ...
        "citations": [
            "Molodtsova–Papell / growth-channel: high relative IP growth → subsequent appreciation",
            "Dahlquist–Hasseltoft-style macro–FX differentials applied to coincident IP",
            "Honesty: low IP / activity-stress debtor premium",
        ],
    ...
    (REPORTS / "scholarly_fx_industrial_production_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: Industrial-production (PRINTO01) differential wave (§{SECTION})",
        "",
        "**Path:** OECD MEI industry YoY via FRED (`{ISO3}PRINTO01GYSAM`) + "
        "AUD/NZD/CHF manufacturing YoY (`{ISO3}PRMNTO01GYSAQ`) — IP YoY XS — "
        "**distinct** from macro-diff EW CPI/IP/UR, OECD CLI (§37) / CCI (§38) / "
        "BCI (§39), employment (§41), WUI (§40), EPU/TPU, IG OAS (§36), CA/TB, "
        "fiscal/debt, BIS, reserves, house-price, money-growth, equity, commodity.",
        f"**Data:** `{data_tag}` + free FRED PRINTO01/PRMNTO01 YoY (monthly + "
        f"quarterly → monthly after pub_lag). EUR = `FRAPRINTO01GYSAM` (France "
        f"proxy; EA19/DEU stale). Full G10 mapped. `n_long=n_short=2` a priori. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (IP a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **YoY growth as reported**.",
        f"**Primary:** `{PRIMARY}` (long high relative IP YoY / short low — "
        "growth-channel → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_industrial_production_*.csv`, "
        "`scholarly_fx_industrial_production_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_industrial_production_wave.md").write_text("\n".join(lines))
