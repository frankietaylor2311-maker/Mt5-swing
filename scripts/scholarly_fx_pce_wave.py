#!/usr/bin/env python3
"""Scholarly FX OECD/FRED real private final consumption (NAEXKP02) FX differential wave (§65).

Fixed priors: FRED OECD MEI NAEXKP02*Q657S real private final consumption QoQ % → YoY %
(USD/GBP/JPY/CAD/AUD/CHF/NZD *Q657S; EUR=DEQ657S Germany
proxy — EZQ657S stale ~2023-01), pub_lag_months=3 + signal_lag=1m +
signal_lag_days=1. Primary high_pce_xs (consumption boom / economic-momentum —
high relative private-consumption YoY → appreciate). Scores on **PCE YoY %**.
PCE here = OECD private final consumption, not US BEA PCE. No HO tuning.
Locked sleeve untouched. Distinct from GDP §58, GFCF §64, retail §46, cars §60,
household credit §63, construction §59, IP §42, CLI/CCI/BCI, emp/labour §41/54–57.
Literature: Lustig–Verdelhan (2007) consumption-growth risk + Dahlquist–Hasseltoft
(2020) applied to private consumption.
"""
...
from mt5_swing.data.fred_pce import (
...
from mt5_swing.strategies.pce_fx import (
    PceFxConfig,
    pce_factor_returns,
)
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED real private final consumption wave (§{SECTION}) ===")
    print(
        "FRED NAEXKP02 real PCE YoY (from Q657S QoQ; pub_lag=3) — high_pce XS; "
        "consumption boom / economic-momentum prior; full G10; EUR=DE proxy (EZQ657S stale ~2023-01); "
        "distinct from GDP §58, GFCF §64, retail §46, cars §60, hh-credit §63, "
        "construction §59, IP §42, CLI/CCI/BCI, emp/labour §41/54–57. "
        "Locked sleeve untouched."
    )

    try:
        lp = load_pce_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL pce load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_pce_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_pce_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED real private final consumption\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"pce panel too thin: cols={list(lp.columns)}. "
    ...
    cov = pce_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_pce_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_pce_panel.csv")
    print(
        f"pce panel cols={list(lp.columns)} "
    ...
    us_pce = None
    try:
        us_pce = load_us_pce(download=True, force=False)
        print(
            f"US NAEXKP02USQ657S PCE YoY [{us_pce.dropna().index.min().date()}.."
            f"{us_pce.dropna().index.max().date()}] n={us_pce.dropna().shape[0]} "
            f"series={us_pce.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US pce load failed: {exc}")
        us_pce = None
    ...
    cfg = PceFxConfig()
    factors = pce_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_pce_override=us_pce,
    )
    if not factors:
        print("FATAL: no pce factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_pce_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_pce_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_pce_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_pce_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_private_consumption_naexkp02",
        "data_tag": data_tag,
        ...
        "normalisation": (
            "FRED OECD MEI NAEXKP02*Q657S real private final consumption QoQ % → compounded YoY % "
            "(USD/GBP/JPY/CAD/AUD/CHF/NZD *Q657S; "
            "EUR=NAEXKP02DEQ657S Germany proxy — EZQ657S stale ~2023-01); "
            "XS on private-consumption YoY %; also 5y z / +Δ12 of YoY boarded; "
            "high_pce primary (consumption boom / Lustig–Verdelhan + economic-momentum → appreciate); "
            "n_long=n_short=2 on available foreign panel (full G10)"
        ),
        "eur_mapping": "NAEXKP02DEQ657S Germany proxy (EA EZQ657S stale ~2023-01)",
        "coverage_g10": "USD/EUR/GBP/JPY/AUD/CAD/CHF/NZD all mapped (full G10)",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "pce_yoy_pct",
        "distinct_from": [
            "gdp_58",
            "gfcf_64",
            "retail_sales_46",
        ...
        "citations": [
            "Lustig & Verdelhan (2007), AER — consumption-growth risk",
            "Dahlquist & Hasseltoft (2020), JFE — economic momentum applied to private consumption",
            "OECD MEI NAEXKP02*Q657S real private final consumption growth (QoQ→YoY %)",
            "Honesty: low PCE / consumption stress",
        ],
    ...
    (REPORTS / "scholarly_fx_pce_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED real private final consumption differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **real private final consumption YoY** "
        "(`NAEXKP02*Q657S` QoQ→YoY; full G10; EUR=Germany DEQ proxy; not BEA PCE) — "
        "PCE YoY XS — **distinct** from GDP (§58), GFCF (§64), retail (§46), cars (§60), "
        "household credit (§63), construction (§59), IP (§42), CLI/CCI/BCI, "
        "emp/labour (§41/§54–57).",
        f"**Data:** `{data_tag}` + free FRED NAEXKP02 YoY (from Q657S QoQ) after pub_lag. "
        f"EUR EA EZQ657S stale ~2023-01 → DEQ657S Germany proxy; full G10 mapped. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (quarterly PCE/NA lag a priori) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **real private final consumption YoY %** (OECD NAEXKP02, not BEA PCE).",
        f"**Primary:** `{PRIMARY}` (long high relative PCE YoY / short low — "
        "consumption boom / economic-momentum → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_pce_*.csv`, "
        "`scholarly_fx_pce_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_pce_wave.md").write_text("\n".join(lines))
