#!/usr/bin/env python3
"""Causal rolling hit-rate cool consistency wave on locked sleeve.
...
LOOKBACKS = [21, 42, 63]
HIGH_THRESH = [0.55, 0.60, 0.65, 0.70]
LOW_THRESH = [0.30, 0.35, 0.40]
...
def tag_hr(lb: int, hi: float, lo_th: float, cool: float) -> str:
    return f"hitrate_lb{lb}_hi{hi}_lo{lo_th}_c{cool}"
...
def _hr_params(lb, hi, lo_th, cool):
    return {
        "lookback_days": int(lb),
        "high_thresh": float(hi),
        "low_thresh": float(lo_th),
...
def _hr_key(params):
    return (
        ...
        round(params["high_thresh"], 4),
        round(params["low_thresh"], 4),
        round(params["cool_scale"], 4),
    )


def pick_hitrate_2024(port24, base_st24, top_k: int = 5):
    ranked = []
    for lb, hi, lo_th, cool in itertools.product(
        LOOKBACKS, HIGH_THRESH, LOW_THRESH, COOL_SCALES
    ):
        params = _hr_params(lb, hi, lo_th, cool)
        st = cor.stats_from_port(
            apply_rolling_hitrate_cool(port24, **params, lo=0.25)
        )
...
def pick_hitrate_on_transformed(port24_xf, base_st_xf):
    best = None
    for lb, hi, lo_th, cool in itertools.product(
        LOOKBACKS, HIGH_THRESH, LOW_THRESH, COOL_SCALES
    ):
        params = _hr_params(lb, hi, lo_th, cool)
        st = cor.stats_from_port(
            apply_rolling_hitrate_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    # --- hitrate alone: top-k 2024 picks on locked (prune full 108-cell grid) ---
    hitrate_soft_ports = {}
    if base24 is not None and base_st24 is not None:
        picked = pick_hitrate_2024(base24, base_st24, top_k=6)
        print(f"hitrate alone: confirming top {len(picked)} / 108 2024 picks", flush=True)
        for sc, params, _ in picked:
            ports = apply_ports(apply_rolling_hitrate_cool, base_ports, **params, lo=0.25)
            name = (
                f"lock+{tag_hr(params['lookback_days'], params['high_thresh'], params['low_thresh'], params['cool_scale'])}"
            )
            row = cor.eval_idea(name, "hitrate", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"hitrate: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
                f"HO={row.mo_holdout*100:.2f}%/{row.pos_holdout*100:.0f}% note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                hitrate_soft_ports[name] = ports
    ...
    # --- wrebal + nested hitrate on 2024 ---
    soft_structures = {}
    w24 = wrebal_ports.get("2024")
    if w24 is not None:
        wst = cor.stats_from_port(w24)
        picked = pick_hitrate_2024(w24, wst, top_k=2)
        for sc, params, _ in picked:
            ports = apply_ports(apply_rolling_hitrate_cool, wrebal_ports, **params, lo=0.25)
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_hr(params['lookback_days'], params['high_thresh'], params['low_thresh'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(name, "frozen_wrebal_hitrate", ports, is_labels, conf_labels)
            board.append(row)
            print(
                f"wrebal+hitrate: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
                soft_structures[name] = ports

    # --- wrebal + mtd + nested hitrate ---
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        p24 = ports.get("2024")
        if p24 is None:
            continue
        st24 = cor.stats_from_port(p24)
        picked = pick_hitrate_on_transformed(p24, st24)
        if picked is None or picked[0] == float("-inf"):
            continue
        _, hrparams, _ = picked
        lports = apply_ports(
            apply_rolling_hitrate_cool, ports, **hrparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_hr(hrparams['lookback_days'], hrparams['high_thresh'], hrparams['low_thresh'], hrparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_hitrate", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+hitrate: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in list(hitrate_soft_ports.items())[:2]:
        soft_structures[n] = p
    ...
    out_csv = ROOT / "reports" / "quest_rolling_hitrate_board.csv"
    out_md = ROOT / "reports" / "quest_rolling_hitrate_consistency.md"
    out_json = ROOT / "configs" / "quest_rolling_hitrate_selected.json"
    ...
    selected = {
        "wave": "causal_rolling_hitrate_cool",
        "data_source": ds,
        ...
        "hitrate_grid": {
            ...
            "lo": 0.25,
            "hitrate": "mean(r_d > 0) over lookback_days",
        },
    }
