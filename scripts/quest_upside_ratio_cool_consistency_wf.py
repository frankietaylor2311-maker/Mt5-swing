#!/usr/bin/env python3
"""Causal rolling upside/downside vol-ratio cool consistency wave on locked sleeve.

Distinct from apply_trailing_downside_vol_scale (downside vs target),
apply_rolling_burst_cool (single fat day), apply_rolling_return_pctile_cool
(return level), apply_trailing_gain_concentration_dampen (month HHI), and
weekly/month mean cool overlays.

Lag-1 rolling upside_rms/downside_rms ratio; if ratio >= ratio_thresh →
cool_scale else 1.0. Clip [lo, 1], hi<=1.
...
from mt5_swing.portfolio.overlays import (
    apply_mtd_gain_clip,
    apply_rolling_upside_ratio_cool,
)
...
LOOKBACKS = [21, 42, 63]
RATIO_THRESH = [1.1, 1.25, 1.5, 1.75]
...
def tag_cool(lb: int, thr: float, cool: float) -> str:
    return f"urc_lb{lb}_t{thr}_c{cool}"
...
def _cool_grid():
    return itertools.product(LOOKBACKS, RATIO_THRESH, COOL_SCALES)
...
def pick_cool_2024(port24, base_st24, top_k: int = 3):
    ...
    for lb, thr, cool in _cool_grid():
        params = {
            ...
            "ratio_thresh": float(thr),
            "cool_scale": float(cool),
        }
        st = cor.stats_from_port(
            apply_rolling_upside_ratio_cool(port24, **params, lo=0.25)
        )
...
def pick_cool_on_transformed(port24_xf, base_st_xf):
    ...
    for lb, thr, cool in _cool_grid():
        ...
        st = cor.stats_from_port(
            apply_rolling_upside_ratio_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    sparse_cool = list(itertools.product(LOOKBACKS, [1.25], COOL_SCALES))
    for lb, thr, cool in sparse_cool:
        ...
        ports = apply_ports(apply_rolling_upside_ratio_cool, base_ports, **params, lo=0.25)
        name = f"lock+{tag_cool(lb, thr, cool)}"
        row = cor.eval_idea(name, "upside_ratio_cool", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"upside_ratio_cool: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    if base24 is not None and base_st24 is not None:
        for sc, params, _ in pick_cool_2024(base24, base_st24, top_k=3):
            ports = apply_ports(
                apply_rolling_upside_ratio_cool, base_ports, **params, lo=0.25
            )
            name = (
                f"lock+{tag_cool(params['lookback_days'], params['ratio_thresh'], params['cool_scale'])}"
                f"_nested"
            )
            row = cor.eval_idea(
                name, "upside_ratio_cool_nested", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"upside_ratio_cool_nested: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    if w24 is not None:
        ...
        for sc, params, _ in pick_cool_2024(w24, wst, top_k=2):
            ports = apply_ports(
                apply_rolling_upside_ratio_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
                f"{tag_cool(params['lookback_days'], params['ratio_thresh'], params['cool_scale'])}_frozen"
            )
            row = cor.eval_idea(
                name, "frozen_wrebal_upside_ratio_cool", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"wrebal+upside_ratio_cool: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        _, cparams, _ = picked
        lports = apply_ports(
            apply_rolling_upside_ratio_cool, ports, **cparams, lo=0.25
        )
        iname = (
            f"{name}+"
            f"{tag_cool(cparams['lookback_days'], cparams['ratio_thresh'], cparams['cool_scale'])}"
        )
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_upside_ratio_cool", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+upside_ratio_cool: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in soft_structures.items():
        if "wrebal" in n and ("urc_" in n or "mtd_" in n):
    ...
    out_csv = ROOT / "reports" / "quest_upside_ratio_cool_board.csv"
    out_md = ROOT / "reports" / "quest_upside_ratio_cool_consistency.md"
    out_json = ROOT / "configs" / "quest_upside_ratio_cool_selected.json"
    ...
    lines = [
        "# Quest wave — rolling upside/downside vol-ratio cool",
        "",
        f"**data_source:** {ds}",
