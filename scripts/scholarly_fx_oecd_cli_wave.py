#!/usr/bin/env python3
"""Scholarly FX OECD Composite Leading Indicator (CLI) differential wave (§37).

Fixed priors: FRED OECD *LOLITOAASTSAM amplitude-adjusted CLI, pub_lag_months=2
+ signal_lag_days=1 (no extra month lag). Primary high_cli_xs (leading-activity
strength → appreciate). No HO tuning. Locked sleeve untouched.
Distinct from coincident macro-diff CPI/IP/UR, house-price, money-growth, IG OAS.
"""
...
from mt5_swing.strategies.oecd_cli_fx import OecdCliFxConfig, oecd_cli_factor_returns
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD CLI leading-indicator wave (§{SECTION}) ===")
    print(
        "OECD amplitude-adjusted CLI via FRED *LOLITOAASTSAM — Estrella–Mishkin / "
        "leading-activity XS; distinct from coincident macro-diff CPI/IP/UR, "
        "house-price, money-growth, IG OAS. Locked sleeve untouched."
    )

    try:
        cli = load_oecd_cli_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL OECD CLI load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_oecd_cli_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_oecd_cli_wave.md").write_text(
            f"# Scholarly FX: OECD CLI\n\n**FAILED:** {exc}\n"
        )
        return 1

    if cli.empty or cli.shape[1] < 4:
        msg = (
            f"OECD CLI panel too thin: cols={list(cli.columns)}. "
        ...
        meta = {
            ...
            "promote": False,
            "section": SECTION,
            "fallback": "pmi_ism_or_consumer_confidence",
        }
        (REPORTS / "scholarly_fx_oecd_cli_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_oecd_cli_wave.md").write_text(
            f"# Scholarly FX: OECD CLI\n\n**FAILED data:** {msg}\n"
    ...
    cov = cli_coverage(cli)
    cov.to_csv(REPORTS / "scholarly_fx_oecd_cli_coverage.csv", index=False)
    cli.to_csv(REPORTS / "scholarly_fx_oecd_cli_panel.csv")
    print(
        f"CLI panel cols={list(cli.columns)} "
        f"[{cli.dropna(how='all').index.min().date()}.."
        f"{cli.dropna(how='all').index.max().date()}] "
        f"pub_lag_m={PUB_LAG_M}"
    )
    print(cov.to_string(index=False))
    print(f"Series source note: {cli.attrs.get('source')}")
    print(f"Failed/alt documented: {cli.attrs.get('failed_or_alt')}")

    us_cli = None
    try:
        us_cli = load_us_oecd_cli(download=True, force=False)
        print(
            f"US USALOLITOAASTSAM [{us_cli.dropna().index.min().date()}.."
            f"{us_cli.dropna().index.max().date()}] n={us_cli.dropna().shape[0]} "
            f"series={us_cli.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US OECD CLI load failed: {exc}")
        us_cli = None
    ...
    cfg = OecdCliFxConfig()
    factors = oecd_cli_factor_returns(
        pair_ret,
        cli,
        cfg=cfg,
        us_cli_override=us_cli,
    )
    if not factors:
    ...
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_oecd_cli_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_oecd_cli_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_oecd_cli_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_oecd_cli_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_cli_leading_indicator",
        "data_tag": data_tag,
        ...
        "normalisation": (
            "OECD MEI *LOLITOAASTSAM amplitude-adjusted CLI levels; "
            "XS on YoY diff CLI_t−CLI_{t−12}; also 5y z / Δ12 of YoY boarded; "
            "high-CLI-growth primary (leading-activity strength → appreciate)"
        ),
        "eur_mapping": "DEULOLITOAASTSAM Germany proxy (EA19 ends 2022-11; EZ19 404)",
        "nzd_chf": "unmapped on primary (NZL ends 2019-11; CHE ends 2022-11)",
        "series_map": cli.attrs.get("series_map", {}),
        "distinct_from": [
        ...
        "citations": [
            "OECD Composite Leading Indicators methodology (amplitude-adjusted CLI)",
            "Estrella & Mishkin (1998) leading indicators / recession prediction",
            "Dahlquist–Hasseltoft-style macro–FX differential framing for leading activity",
        ],
        "coverage": cov.to_dict(orient="records"),
        "unmapped": cli.attrs.get("unmapped", []),
        "failed_or_alt": cli.attrs.get("failed_or_alt", {}),
        "soft_best": soft_best,
        "primary_sweep": prim_sweep.iloc[0].to_dict() if len(prim_sweep) else None,
    }
    (REPORTS / "scholarly_fx_oecd_cli_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD CLI leading-indicator differential wave (§{SECTION})",
        "",
        "**Path:** OECD amplitude-adjusted CLI via FRED `*LOLITOAASTSAM` — "
        "Estrella–Mishkin / leading-activity XS — **distinct** from coincident "
        "macro-diff CPI/IP/UR, house-price (§35), money-growth (§33), IG OAS (§36), "
        "CA/TB, BIS REER/credit, reserves, equity-diff, GPR/EPU.",
        f"**Data:** `{data_tag}` + free FRED `*LOLITOAASTSAM`. "
        f"EUR = `DEULOLITOAASTSAM` (Germany proxy; EA19 ends 2022-11). "
        f"NZD/CHF **unmapped** on primary (NZL 2019 / CHE 2022 stale). "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (conservative monthly OECD CLI) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag.",
        f"**Primary:** `{PRIMARY}` (long high relative CLI YoY growth / short low — "
        "leading-activity strength → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_oecd_cli_*.csv`, "
        "`scholarly_fx_oecd_cli_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_oecd_cli_wave.md").write_text("\n".join(lines))
