#!/usr/bin/env python3
"""Scholarly FX OECD/FRED labour-force participation / activity-rate FX differential wave (§56).

Fixed priors: FRED OECD MEI LRAC64TT activity rate ages 15–64 (%)
(USD/JPY/CAD/AUD *M156S; GBP/CHF/NZD *Q156S; EUR=DEQ156S Germany
proxy — EZQ156S stale ~2022-10), pub_lag_months=2 + signal_lag=1m +
signal_lag_days=1. Primary high_lfp_xs (growth / labour-engagement —
high relative LFP → appreciate). Scores on **LFP level %**. No HO
tuning. Locked sleeve untouched. Distinct from employment §41, UR §54,
wages §55, ULC/LP/CU.
"""
...
def main() -> int:
    ...
    print(f"=== scholarly FX OECD/FRED labour-force participation / activity-rate wave (§{SECTION}) ===")
    print(
        "FRED LRAC64TT LFP/activity levels (pub_lag=2) — high_lfp XS; growth/"
        "engagement prior; full G10; EUR=DE proxy (EZQ stale); distinct from "
        "employment §41, UR §54, wages §55. Locked sleeve untouched."
    )

    try:
        lp = load_lfp_panel(pub_lag_months=PUB_LAG_M, download=True, force=False)
    except Exception as exc:  # noqa: BLE001
        print(f"FATAL lfp load: {exc}")
        meta = {"ok": False, "error": str(exc), "promote": False, "section": SECTION}
        (REPORTS / "scholarly_fx_lfp_meta.json").write_text(json.dumps(meta, indent=2))
        (REPORTS / "scholarly_fx_lfp_wave.md").write_text(
            f"# Scholarly FX: OECD/FRED labour-force participation / activity-rate\n\n**FAILED:** {exc}\n"
        )
        return 1

    if lp.empty or lp.shape[1] < 4:
        msg = (
            f"lfp panel too thin: cols={list(lp.columns)}. "
    ...
    cov = lfp_coverage(lp)
    cov.to_csv(REPORTS / "scholarly_fx_lfp_coverage.csv", index=False)
    lp.to_csv(REPORTS / "scholarly_fx_lfp_panel.csv")
    print(
        f"lfp panel cols={list(lp.columns)} "
    ...
    us_lfp = None
    try:
        us_lfp = load_us_lfp(download=True, force=False)
        print(
            f"US LRAC64TTUSM156S [{us_lfp.dropna().index.min().date()}.."
            f"{us_lfp.dropna().index.max().date()}] n={us_lfp.dropna().shape[0]} "
            f"series={us_lfp.attrs.get('series_id')}"
        )
    except Exception as exc:  # noqa: BLE001
        print(f"WARN US lfp load failed: {exc}")
        us_lfp = None
    ...
    cfg = LfpFxConfig()
    factors = lfp_factor_returns(
        pair_ret,
        lp,
        cfg=cfg,
        us_lfp_override=us_lfp,
    )
    if not factors:
        print("FATAL: no lfp factors produced")
        return 1

    board = dict(factors)
    summaries = [factor_summary(r, name) for name, r in board.items()]
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_lfp_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_lfp_{name}_monthly.csv", header=[name]
        )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_lfp_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_lfp_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "oecd_mei_lfp_activity_rate_lrac64tt",
        "data_tag": data_tag,
        ...
        "normalisation": (
            ...
            "XS on LFP level %; also 5y z / +Δ12 of level boarded; "
            "high_lfp primary (growth/engagement → appreciate); "
            "n_long=n_short=2 on available foreign panel (full G10)"
        ),
        "eur_mapping": "LRAC64TTDEQ156S Germany proxy (EA EZQ156S stale ~2022-10)",
        "coverage_g10": "USD/EUR/GBP/JPY/AUD/CAD/CHF/NZD all mapped (full G10)",
        "series_map": lp.attrs.get("series_map", {}),
        "score_basis": "lfp_level_pct",
        "distinct_from": [
        ...
        "citations": [
            ...
            "Labour-force participation / activity rate as growth / extensive labour-supply margin",
            "OECD MEI LRAC64TT activity rate ages 15–64 (%)",
            "Honesty: low LFP / labour-disengagement debtor premium",
        ],
    ...
    (REPORTS / "scholarly_fx_lfp_meta.json").write_text(
        json.dumps(meta, indent=2, default=str)
    )

    lines = [
        f"# Scholarly FX: OECD/FRED labour-force participation / activity-rate differential wave (§{SECTION})",
        "",
        "**Path:** FRED OECD MEI **activity / LFP rate levels** "
        "(`LRAC64TT*`; full G10; EUR=Germany DEQ proxy) — LFP level XS — "
        "**distinct** from employment persons (§41), UR levels (§54), wages (§55), "
        "ULC (§48), LP (§49), CU (§50).",
        f"**Data:** `{data_tag}` + free FRED LRAC64TT levels after pub_lag. "
        f"EUR EA EZQ156S stale ~2022-10 → DEQ156S Germany proxy; full G10 mapped. "
        f"`n_long=n_short=2` on available foreign panel. "
        f"**PIT:** pub_lag_months={PUB_LAG_M} (participation lag a priori; mixed M+Q) + "
        f"signal_lag_months={cfg.signal_lag} + 1d weight lag. "
        f"Score basis = **LFP / activity rate level %**.",
        f"**Primary:** `{PRIMARY}` (long high relative LFP / short low — "
        "growth/engagement → appreciate). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        "Artifacts: `reports/scholarly_fx_lfp_*.csv`, "
        "`scholarly_fx_lfp_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_lfp_wave.md").write_text("\n".join(lines))
