#!/usr/bin/env python3
"""Causal trailing completed-month mean cool consistency wave on locked sleeve.

Distinct from apply_prior_month_win_throttle (binary last-month win),
apply_equity_curve_target (trailing monthly VOL), and apply_mtd_gain_clip /
pace / month-end surplus (intra-month MTD).

Uses ONLY completed months: lag-1 trailing mean of last `lookback` month
...
from mt5_swing.portfolio.overlays import (
    apply_mtd_gain_clip,
    apply_trailing_month_mean_cool,
)
...
LOOKBACKS = [3, 4, 6]
MEAN_THRESH = [0.01, 0.012, 0.015, 0.02]
...
def tag_cool(lb: int, thr: float, cool: float) -> str:
    return f"mmc_lb{lb}_t{thr}_c{cool}"
...
def pick_cool_2024(port24, base_st24, top_k: int = 3):
    ...
    for lb, thr, cool in _cool_grid():
        ...
        st = cor.stats_from_port(
            apply_trailing_month_mean_cool(port24, **params, lo=0.25)
        )
...
def pick_cool_on_transformed(port24_xf, base_st_xf):
    ...
    for lb, thr, cool in _cool_grid():
        ...
        st = cor.stats_from_port(
            apply_trailing_month_mean_cool(port24_xf, **params, lo=0.25)
        )
...
def main() -> None:
    ...
    sparse_cool = list(itertools.product(LOOKBACKS, [0.012], COOL_SCALES))
    for lb, thr, cool in sparse_cool:
        ...
        ports = apply_ports(apply_trailing_month_mean_cool, base_ports, **params, lo=0.25)
        name = f"lock+{tag_cool(lb, thr, cool)}"
        row = cor.eval_idea(name, "month_mean_cool", ports, is_labels, conf_labels)
        board.append(row)
        print(
            f"mean_cool: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    if base24 is not None and base_st24 is not None:
        for sc, params, _ in pick_cool_2024(base24, base_st24, top_k=3):
            ports = apply_ports(
                apply_trailing_month_mean_cool, base_ports, **params, lo=0.25
            )
            name = (
            ...
            row = cor.eval_idea(
                name, "month_mean_cool_nested", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"mean_cool_nested: {name} soft={row.soft_pass} 2024={row.mo_2024*100:.2f}% "
    ...
    if w24 is not None:
        ...
        for sc, params, _ in pick_cool_2024(w24, wst, top_k=2):
            ports = apply_ports(
                apply_trailing_month_mean_cool, wrebal_ports, **params, lo=0.25
            )
            name = (
                f"{tag_w(frozen_w)}+"
            ...
            row = cor.eval_idea(
                name, "frozen_wrebal_mean_cool", ports, is_labels, conf_labels
            )
            board.append(row)
            print(
                f"wrebal+mean_cool: {name} soft={row.soft_pass} note={row.note}",
                flush=True,
            )
            if row.soft_pass:
    ...
    for name, (mtd, ports) in list(wrebal_mtd_ports.items()):
        ...
        _, cparams, _ = picked
        lports = apply_ports(
            apply_trailing_month_mean_cool, ports, **cparams, lo=0.25
        )
        iname = (
            f"{name}+"
        ...
        row = cor.eval_idea(
            iname, "frozen_wrebal_mtd_mean_cool", lports, is_labels, conf_labels
        )
        board.append(row)
        print(
            f"wrebal+mtd+mean_cool: {iname} soft={row.soft_pass} note={row.note} "
    ...
    for n, p in soft_structures.items():
        if "wrebal" in n and ("mmc_" in n or "mtd_" in n):
    ...
    out_csv = ROOT / "reports" / "quest_month_mean_cool_board.csv"
    out_md = ROOT / "reports" / "quest_month_mean_cool_consistency.md"
    out_json = ROOT / "configs" / "quest_month_mean_cool_selected.json"
    ...
    lines = [
        "# Quest wave — trailing completed-month mean cool",
        "",
        f"**data_source:** {ds}",
