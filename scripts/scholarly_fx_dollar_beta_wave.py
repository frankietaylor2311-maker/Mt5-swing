...
Sort currencies by rolling β of monthly currency returns on the dollar factor
RX = EW foreign excess returns vs USD (typically 60-month window). Long high-$β
/ short low-$β. Optional AFD sign conditioning; dollar RX TSMOM companion.

Fixed priors: β window 60m/36m, skip=1m, signal_lag=1m; monthly XS rebalance;
costs 1.5 bps/side. No HO tuning. Optional IS→OOS risk sweep if any positive
IS mean. Locked sleeve untouched.
"""
...
from mt5_swing.strategies.dollar_beta_fx import DollarBetaConfig, dollar_beta_factor_returns
...
def main() -> int:
    ...
    print("=== scholarly FX Lustig–Verdelhan dollar-factor beta wave ===")
    print(
        "Verdelhan / Lustig–Roussanov–Verdelhan — rolling $β on RX dollar factor; "
        "HML high−low β; AFD-conditioned; RX TSMOM. Distinct from Lustig carry, "
        "Menkhoff FX-RV, Hau–Rey, BNP crash-skew, AI-GPR. Locked sleeve untouched."
    )
    ...
    try:
        ...
        print(f"WARN rates: {exc}; dollar_beta_afd skipped")
        rates = None

    cfg = DollarBetaConfig()
    factors = dollar_beta_factor_returns(pair_ret, rates=rates, cfg=cfg)
    if not factors:
        print("FATAL: no dollar-beta factors produced")
        return 1
    ...
    pd.DataFrame(summaries).to_csv(
        REPORTS / "scholarly_fx_dollar_beta_factor_summary.csv", index=False
    )
    for name, r in board.items():
        monthly_returns(r).to_csv(
            REPORTS / f"scholarly_fx_dollar_beta_{name}_monthly.csv", header=[name]
        )

    # Daily panel for reference
    pd.DataFrame({k: v for k, v in board.items()}).to_csv(
        REPORTS / "scholarly_fx_dollar_beta_panel.csv"
    )
    ...
    win_df.to_csv(REPORTS / "scholarly_fx_dollar_beta_window_stats.csv", index=False)
    ...
    if len(sweep_df):
        sweep_df.to_csv(REPORTS / "scholarly_fx_dollar_beta_risk_sweep.csv", index=False)
    ...
    meta = {
        "ok": True,
        "path": "lustig_verdelhan_dollar_factor_beta",
        "section": 27,
        "data_tag": data_tag,
        "signal_lag_months": cfg.signal_lag,
        "skip_months": cfg.skip_months,
        "beta_window_months": cfg.beta_window_months,
        "beta_window_months_short": cfg.beta_window_months_short,
        "tsmom_lookback_months": cfg.tsmom_lookback_months,
        "estimation": "monthly_ols_verdelhan_60m",
    ...
    lines = [
        "# Scholarly FX: Lustig–Verdelhan dollar-factor beta wave",
        "",
        "**Path:** Rolling β of currency returns on the dollar factor RX "
        "(EW foreign excess returns vs USD) — long high-$β / short low-$β. "
        "**Distinct** from Lustig carry (rate sort), Menkhoff FX-RV (§15), "
        "Hau–Rey equity-diff, BNP crash-skew (§26), AI-GPR (§25).",
        f"**Data:** `{data_tag}` Yahoo D1 USD majors + free FRED short rates for AFD. "
        f"**PIT:** skip={cfg.skip_months}m + signal_lag={cfg.signal_lag}m; "
        f"β window {cfg.beta_window_months}/{cfg.beta_window_months_short}m "
        f"(monthly OLS); RX TSMOM {cfg.tsmom_lookback_months}m; "
        f"costs {cfg.cost_bps_per_side} bps/side.",
        f"**Primary:** `{PRIMARY}` (unconditional high−low $β HML). "
    ...
    for _, row in win_df[
        ...
        top3 = row["top3"] * 100 if np.isfinite(row["top3"]) else float("nan")
        lines.append(
            f"| {row['strategy']} | {row['window']} | {row['mean_mo']*100:+.2f}% | "
            f"{row['pct_pos']*100:.0f}% | {top3:.0f}% | "
            f"{'PASS' if row['gates_pass'] else 'FAIL'} | "
            f"{'yes' if row['clears_1pct_bar'] else 'no'} |"
        )
    lines += [
        "",
        f"**Risk sweep run:** {'YES' if any_pos_is else 'NO (no positive IS mean)'}.",
        f"**Unscaled promote:** {'YES' if promote else 'NO'}. "
        f"**Scaled primary promote:** {'YES' if scaled_promote else 'NO'}.",
        "",
        "Artifacts: `reports/scholarly_fx_dollar_beta_*.csv`, "
        "`scholarly_fx_dollar_beta_meta.json`.",
        "",
    ]
    (REPORTS / "scholarly_fx_dollar_beta_wave.md").write_text("\n".join(lines))
