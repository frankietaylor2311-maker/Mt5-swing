#!/usr/bin/env python3
"""Scholarly FX VIX/GPR-conditioned Dahlquist soft-signal EW wave (§72).

Multi-factor structure: §66 soft_ew_macro5 (SOFT_LEGS EW) gated / cooled by
VIX (Menkhoff global FX-vol proxy) and aggregate Caldara–Iacoviello GPR.
Literature: Dahlquist–Hasseltoft (2020) + Menkhoff et al. (2012 JF) +
Caldara–Iacoviello (2022) / Liu–Zhang (2024).

Fixed priors: VIX/GPR bar_lag=1 + signal_lag=1d on trailing z, gate z≤0 /
cool at z≥1 (same as §71 — lit GPR often 1.5; documented symmetry), soft
legs source-wave PIT, costs 1.5 bps/side inside source factors.
Primary ``soft_low_vix``. Honesty: ``soft_raw`` + ``soft_high_vix``.
No HO tuning. Locked sleeve untouched.

Distinct from: soft_ew §66, soft CIP §69, CIP×carry §68, CIP XS §67,
CIP-conditioned soft §71, capital-sleeve §53/§70, funding_liq §20, combo §8,
gpr_regime standalone, AI-GPR §25, country-GPR §45.
...
PREFIX = "scholarly_fx_vix_gpr_conditioned_soft"
...
def write_locked_verify() -> dict:
    ...
    lines = [
        "# One-pct month quest — locked_verify_vix_gpr_conditioned_soft",
        "",
    ...
    lines += [
        "",
        "Config unmodified vs §71 cip_conditioned_soft / §70 capital_sleeve_soft — "
        "canonical windowed PASS.",
        "",
    ]
    (REPORTS / "quest_locked_verify_vix_gpr_conditioned_soft.md").write_text(
        "\n".join(lines)
    )
    log = (
        f"Locked sleeve re-verify (§{SECTION} vix_gpr_conditioned_soft): "
        f"{LOCKED_CFG.relative_to(ROOT)}\n"
        f"basket_tag={LOCKED_TAG} untouched\n"
        f"sha256={sha}\n"
        f"Canonical windowed PASS (same as §71 cip_conditioned_soft — config unmodified):\n"
        ...
        f"Wrote {REPORTS / 'quest_locked_verify_vix_gpr_conditioned_soft.md'}\n"
    )
    (REPORTS / "quest_locked_verify_vix_gpr_conditioned_soft.log").write_text(log)
    print(log, end="")
...
def main() -> int:
    ...
    print(f"=== scholarly FX VIX/GPR-conditioned soft-signal EW wave (§{SECTION}) ===")
    print(
        f"VIX + aggregate GPR stress gate/cool on soft_ew_macro5 — primary {PRIMARY}; "
        "Dahlquist–Hasseltoft + Menkhoff et al. + Caldara–Iacoviello / Liu–Zhang; "
        "distinct from soft_ew §66, soft CIP §69, CIP×carry §68, CIP XS §67, "
        "CIP-conditioned soft §71, capital-sleeve §53/§70, funding_liq §20, combo §8, "
        "gpr_regime / AI-GPR §25 / country-GPR §45. Locked sleeve untouched."
    )
    ...
    try:
        ...
        (REPORTS / f"{PREFIX}_wave.md").write_text(
            f"# Scholarly FX: VIX/GPR-conditioned soft\n\n**FAILED:** {exc}\n"
        )
        return 1

    vix_d = vix.dropna()
    gpr_d = gpr.dropna()
    if len(vix_d) < 252 or len(gpr_d) < 252:
        msg = f"VIX/GPR too thin: vix_n={len(vix_d)} gpr_n={len(gpr_d)}"
        print(f"FATAL: {msg}")
    ...
    print(
        f"VIX [{vix_d.index.min().date()}..{vix_d.index.max().date()}] n={len(vix_d)}"
    )
    print(
        f"GPR [{gpr_d.index.min().date()}..{gpr_d.index.max().date()}] n={len(gpr_d)}"
    )
    ...
    factors = vix_gpr_conditioned_soft_factor_returns(
        soft_ew, vix=vix, gpr=gpr, cfg=cfg
    )
    if not factors or PRIMARY not in factors:
        print("FATAL: no vix_gpr_conditioned_soft factors / primary missing")
        return 1
    ...
    meta = {
        "ok": True,
        "section": SECTION,
        "path": "menkhoff_vix_caldara_gpr_conditioned_dahlquist_soft_ew",
        "data_tag": data_tag,
        "stress_a": "VIX (Yahoo ^VIX / vix_yahoo.csv) — Menkhoff global FX-vol proxy",
        "stress_b": "aggregate Caldara–Iacoviello GPR daily (gpr_daily.csv)",
        "bar_lag": 1,
        "signal_lag_days": cfg.signal_lag,
        "z_window": cfg.z_window,
        "z_high": cfg.z_high,
        "z_high_note": (
            "cool z≥1.0 for §71 symmetry; literature GPR often uses 1.5 "
            "(macro_regimes.apply_gpr_risk_gate default) — documented, not HO-tuned"
        ),
        "cool": cfg.cool,
        ...
        "primary": PRIMARY,
        "primary_sign_prior": (
            "trade Dahlquist soft_ew_macro5 only when lagged VIX z ≤ 0 — "
            "sit out when global FX-vol / equity-vol stress elevated (Menkhoff)"
        ),
        "n_factors": len(board),
        "soft_nw_pos": soft,
        "hard_nw_pos": hard,
        ...
        "vix_start": str(vix_d.index.min().date()),
        "vix_end": str(vix_d.index.max().date()),
        "gpr_start": str(gpr_d.index.min().date()),
        "gpr_end": str(gpr_d.index.max().date()),
        "data_caveat": (
            "approximate_non_ftmo Yahoo D1. Soft legs rebuilt with §66 source-wave PIT; "
            "costs inside source factors. VIX/GPR live daily (bar_lag=1 + signal_lag=1). "
            "Cool z_high=1.0 for both VIX and GPR (§71 symmetry; lit GPR often 1.5)."
        ),
        "normalisation": (
            f"VIX/GPR trailing z {cfg.z_window}d after bar_lag=1; signal_lag="
            f"{cfg.signal_lag}d; gate z≤0 / cool z≥{cfg.z_high}; "
            "soft_ew_macro5 = EW SOFT_LEGS (≥2 legs/day); "
            "soft_vix_gpr_stack = product of VIX×GPR cool scales"
        ),
        ...
        "distinct_from": [
            ...
            "funding_liq_20",
            "combo_8",
            "gpr_regime_standalone",
            "ai_gpr_25",
            "country_gpr_45",
        ],
        "citations": [
            "Dahlquist & Hasseltoft (2020), JFE — economic momentum (EW of macro signals)",
            "Menkhoff, Sarno, Schmeling & Schrimpf (2012), JF — global FX vol / carry crashes (VIX proxy)",
            "Caldara & Iacoviello (2022), AER — Geopolitical Risk (GPR) index",
    ...
    lines = [
        f"# Scholarly FX: VIX/GPR-conditioned soft-signal EW wave (§{SECTION})",
        "",
        "**Path:** Menkhoff **VIX** (global FX-vol proxy) + Caldara–Iacoviello "
        "**aggregate GPR** stress × Dahlquist–Hasseltoft ``soft_ew_macro5`` "
        "gate/cool — **distinct** from soft_ew (§66 ungated), soft-stack CIP "
        "enrichment (§69), CIP-conditioned carry (§68), CIP XS (§67), "
        "CIP-conditioned soft (§71 CIP stress), capital-sleeve (§53/§70), "
        "funding_liq (§20), combo (§8), gpr_regime standalone, AI-GPR (§25), "
        "country-GPR (§45). Explicit: do **not** overlay coolers on locked fx4plus.",
        f"**Data:** `{data_tag}` + `vix_yahoo.csv` + `gpr_daily.csv` + FRED/OECD "
        f"soft legs (§66 PIT). G10 trade: EUR/GBP/JPY/AUD/CAD/CHF/NZD. "
        f"**PIT:** VIX/GPR bar_lag=1 + signal_lag={cfg.signal_lag}d on trailing z; "
        f"soft legs source-wave pub/signal lags. Costs 1.5 bps/side inside source factors.",
        f"**Primary:** `{PRIMARY}` (trade soft only when lagged VIX z ≤ 0). "
        "Companions: soft_vix_cool / soft_low_gpr / soft_gpr_cool / soft_raw / "
        "soft_high_vix / soft_vix_gpr_stack / soft_vix_gpr_ew. "
        f"Cool z_high={cfg.z_high} for both VIX and GPR (§71 symmetry; lit GPR often "
        f"{cfg.gpr_lit_z_high_note}). Locked sleeve untouched.",
        "",
    ...
    lines += [
        ...
        "",
        f"**Data caveat:** VIX [{vix_d.index.min().date()}.."
        f"{vix_d.index.max().date()}]; GPR [{gpr_d.index.min().date()}.."
        f"{gpr_d.index.max().date()}]. Gate z≤0 / cool z≥1 fixed for both "
        "(§71 symmetry; lit GPR often 1.5 — documented, not HO-tuned).",
        "",
        f"Artifacts: `reports/{PREFIX}_*.csv`, "
        f"`{PREFIX}_meta.json`, `quest_locked_verify_vix_gpr_conditioned_soft.md`.",
        "",
    ]
    (REPORTS / f"{PREFIX}_wave.md").write_text("\n".join(lines))
