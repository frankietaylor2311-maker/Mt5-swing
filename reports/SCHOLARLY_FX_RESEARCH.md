...
**Status:** Active (2026-09-24 BST, **EPU/TPU-conditioned BIS REER HML-FX value §77 boarded** (reer_cheap_xs × EPU/TPU gate/cool; soft=0 hard=0 promote=0) after EPU/TPU-conditioned soft EW §76 boarded (soft_ew_macro5 × EPU/TPU gate/cool; soft=8 hard=8 promote=0) after VIX/GPR-conditioned Rogoff PPP / real-FX value §75 boarded (value × VIX/GPR gate/cool; soft=0 hard=0 promote=0) after VIX/GPR-conditioned Menkhoff FX momentum §74 / Lustig–Verdelhan carry §73 / soft EW §72 / CIP-conditioned soft §71 boarded; free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + CIP-conditioned soft + VIX/GPR-conditioned soft/carry/mom/value + EPU/TPU soft + EPU/TPU×REER value boarded; FX IV/RR + news-sentiment still blocked). Replaces the technical **overlay hunt** as the primary path toward FTMO-consistent ~1%/month.
**Data tag:** `approximate_non_ftmo` (Yahoo D1) + free FRED short rates / OECD IR3M money-market + Chicago NFCI/ANFCI + TED/CPFF/BAA + yfinance VIX + Caldara–Iacoviello GPR + free CFTC TFF/Legacy COT + Baker–Bloom–Davis EPU/TPU + IMF BOP CA/GDP (`{ISO3}B6BLTT02STSAQ`) + Fed/ECB/BoJ CB assets (`WALCL` / `ECBASSETSW` / `JPNASSETS`) + US TIPS/BE (`DFII10` / `T10YIE`) + Caldara–Iacoviello AI-GPR daily roles (`ai_gpr_daily.csv` threats/acts/oil-region) + IMF WEO fiscal balance (`GGNLBA*188N`) + US MTS (`MTSDS133FMS`) + IMF WEO gross debt (`GGGDTA*188N`; `GGXWDG*` 404) + US federal debt/GDP Q (`GFDEGDQ188S`) + BIS private credit/GDP (`Q*PAM770A`; `CRDQ*APABIS` absolute unused) + OECD broad-money growth (`MABMM301*M657S`; NZD/CHF 657S stale) + IMF IFS total reserves excl. gold (`TRESEG*M052N`; EUR=`TRESEGDEM052N`; NZD/CHF 404) + BIS real residential HPI (`Q*R628BIS`; EUR=`QDER628BIS`, `QEUR628BIS` 404) + ICE BofA OAS (`BAMLC0A0CM` IG / `BAMLH0A0HYM2` HY / `BAMLC0A4CBBB` BBB; public CSV ~3y ICE truncation). + OECD CLI amplitude-adjusted (`*LOLITOAASTSAM`; EUR=`DEULOLITOAASTSAM`; NZD/CHF stale unmapped). + OECD CCI balances (`CSCICP02*M460S`; USD=`USACSCICP02STSAM`; EUR=`CSCICP02EZM460S`; CAD/NZD/CHF unmapped; `CSCICP03*M665S` amplitude stale ~2024-01). + OECD BCI balances (`BSCICP02*M460S`; EUR=`BSCICP02EZM460S`; CHF live; AUD/CAD/NZD/JPY unmapped; `BSCICP03*M665S` amplitude stale ~2023-11..2024-01; NAPM/ISM 404). + Ahir–Bloom–Furceri country WUI (`WUIUSA`/`WUIDEU`/`WUIGBR`/`WUIJPN`/`WUICAN`/`WUIAUS`/`WUINZL`/`WUICHE`; EUR=Germany proxy; full G10 live ~2026-04; quarterly→monthly after pub_lag=4). + OECD MEI employment persons LFEMTTTT (`LFEMTTTTUSM647S`/`JPM647S`/`CAM647S`/`AUM647S` monthly; `LFEMTTTTDEQ647S` EUR Germany proxy — EZ Q647S stale 2022-10; `GBQ647S`/`NZQ647S`/`CHQ647S` quarterly; M657S/Q657S YoY stale/404 — not primary; YoY of levels; pub_lag=3). + OECD MEI industrial production YoY (`USAPRINTO01GYSAM`/`GBR`/`JPN`/`CAN` monthly; EUR=`FRAPRINTO01GYSAM` France proxy — EA19/DEU GYSAM stale; AUD/NZD/CHF=`*PRMNTO01GYSAQ` manufacturing quarterly — industry PRINTO01 404; PROINDMISMEI stale ~2024-03; pub_lag=2). + OECD MEI building permits YoY (`CAN`/`AUS`/`NZL`/`DEU ODCNPI03GYSAM`; EUR=Germany — EA19 stale 2023-08; USD=`PERMIT` levels→YoY — `USAODCNPI03GYSAM` 404; GBP/JPY/CHF unmapped; ODCNPI02 starts 404; ACM `THREEFYTP10` live fallback not used; pub_lag=2). + NY Fed ACM term premium (`THREEFYTP10` / `THREEFYTP5`; `ACMTP10` 404; pub_lag=1d). + Country-GPR bilateral rel-to-US (`gpr_country_monthly.csv` GPRC_*; EUR=EW DEU/FRA/ITA/ESP/NLD/BEL; NZD gap no GPRC_NZL; pub_lag=1m). + OECD MEI retail volume growth / US RSAFS (`SLRTTO01*Q657S` Q; EUR=`SLRTTO01DEQ657S` DEU proxy; USD=`RSAFS` levels→YoY; CAD=`CANWSCNDW01IXOBSAM` index→YoY ends~2024-04; `USAWSCNDW`/`SARTM*` stale — not primary; pub_lag=2). + BIS SDMX WS_DSR PNFS debt-service (`Q.{ISO2}.P`; FRED Q*PNFDSR/BISDSR* **404**; EUR=`Q.DE.P` Germany proxy — XM/EA missing; NZD unmapped; pub_lag=5). + OECD MEI early-estimate ULC YoY (`ULQEUL01{ISO2}Q657S`; EUR=`ULQEUL01DEQ657S` Germany — EZ ends 2024-01; full G10 incl NZD/CHF; `*Q661S`/`{ISO3}ULQEUL01GYSAQ`/`ULQULC01*` stale/404; LCEAMN wage fallback not primary; pub_lag=3). + OECD MEI early-estimate labour-productivity YoY (`ULQELP01{ISO2}Q657S`; EUR=`ULQELP01DEQ657S`; full G10; **STALE** ends ~2023-04..07; `{ISO3}ULQELP01GYSAQ` 404; IMF IFS UV/ToT fallback 404; OPHNFB/PRS85006092 US alts; pub_lag=3). + OECD MEI early-estimate capacity-utilization YoY (`ULQECU01{ISO2}Q657S`; EUR=`ULQECU01DEQ657S`; full G10; **STALE** ends ~2023-04..07; `{ISO3}ULQECU01GYSAQ` 404; TCU US national level alt live not mixed; pub_lag=3). + OECD MEI manufacturing PPI YoY (`{ISO3}PIEAMP01GYM`; EUR=`DEUPIEAMP01GYM`; JPY=`JPNPPDMMINMEI`→YoY — PIEAMP Japan 404; AUD/NZD=`PIEAMP01*Q661N`→YoY; **STALE** ~2022-12; PPIACO/PPIFIS US alts not mixed; pub_lag=2). + OECD MEI / national CPI YoY (`CPIAUCSL`; `CP0000EZ19M086NEST` EA19 HICP; GBP/JPY/CAD/CHF `*CPIALLMINMEI`; AUD/NZD `*CPIALLQINMEI` Q→YoY; JPY **STALE** ~2021-06; pub_lag=1). + OECD MEI / national unemployment rate levels (`LRHUTTTT*M156S`; CHF=`LMUNRRTTCHM156S` registered; NZD unmapped; EUR sparse ~2023-01; pub_lag=1). + OECD MEI manufacturing earnings YoY (`LCEAMN01*`; USD/CAD `*M657S`; EUR=`LCEAMN01EZQ657S` ends ~2025-07; NZD=`*NZQ657S`; GBP/JPY `*M661S`→YoY; AUD `*AUQ661S`→YoY; CHF unmapped 404; ULC §48 fallback now dedicated §55; pub_lag=2). + OECD MEI activity / LFP rate levels (`LRAC64TT*`; USD/JPY/CAD/AUD `*M156S`; GBP/CHF/NZD `*Q156S`; EUR=`LRAC64TTDEQ156S` Germany proxy — EZQ156S stale ~2022-10; full G10; vacancies/hours alts documented not boarded; pub_lag=2). + OECD MEI employment-rate levels (`LREM64TT*`; USD/JPY/CAD/AUD `*M156S`; GBP/CHF/NZD `*Q156S`; EUR=`LREM64TTDEQ156S` Germany proxy — EZQ156S stale ~2022-10; full G10; EMRATIO/CE16OV US-only + vacancies/hours alts documented not boarded; pub_lag=2). + OECD MEI real GDP YoY (`NAEXKP01*Q657S` QoQ→compounded YoY %; EUR=`NAEXKP01DEQ657S` Germany proxy — EZQ657S stale ~2023-01; full G10; Q659S 404; Q652S levels partial stale/404; GDPC1/JPNRGDPEXP alts not mixed; pub_lag=3). + OECD MEI construction production YoY (`USAPRCNTO01GYSAM`/`DEU`/`GBR`/`CAN` monthly; JPY=`JPNPRCNTO01GYSAQ` Q YoY — monthly GYSAM stale ~2020-07; AUD/NZD/CHF=`PRCNTO01*Q657S` QoQ→YoY; EUR=Germany DEU proxy; `CHEPRCNTO01GYSAM` 404; `USAPRCNTO01IXOBSAM` stale ~2024-03; pub_lag=2). + OECD MEI passenger-car registrations YoY (`USASLRTCR03GYSAM`/`GBR`/`JPN`/`CAN`/`AUS` monthly; EUR=`ESPSLRTCR03GYSAM` Spain proxy — DEU/FRA GYSAM 404, EA19 stale ~2018-12; NZD/CHF unmapped stale ~2021-04/2018-12; housing starts ODCNPI02/04 + mfg orders ODMN* 404 thin fall-through; pub_lag=2). + OECD MEI merchandise export-value YoY (`XTEXVA01{ISO2}M657S`; EUR=`XTEXVA01DEM657S` Germany proxy — EZM657S 404, EZM667S stale ~2023-04; full G10; unit=VALUE not volume; *M667S levels not mixed; CIP/basis Missing + hours/vacancies thin + PRSVTT*/ODMN* 404 fall-through; pub_lag=2). + OECD MEI merchandise import-value YoY (`XTIMVA01{ISO2}M657S`; EUR=`XTIMVA01DEM657S` Germany proxy — EZM657S 404, EZM667S stale ~2023-04; full G10; unit=VALUE not volume; *M667S levels not mixed; mirrors export §61; pub_lag=2). + BIS household credit abs→YoY (`CRDQ{ISO2}AHABIS`; EUR=`CRDQXMAHABIS` euro-area — CRDQEAAHABIS 404; NZD CRDQNZAHABIS 404 unmapped; distinct from Q*PAM770A §32; trade-volume/saving-rate/inventories/order-books fallthrough; pub_lag=5). + OECD MEI real GFCF / investment YoY (`NAEXKP04*Q657S` QoQ→compounded YoY %; EUR=`NAEXKP04DEQ657S` Germany proxy — EZQ657S stale ~2023-01; full G10; Q659S live≡compound not mixed; PNFS CRDQ*APABIS skip §32 companion; NFC ANFBIS 404; pub_lag=3). + OECD MEI real private final consumption YoY (`NAEXKP02*Q657S` QoQ→compounded YoY %; EUR=`NAEXKP02DEQ657S` Germany proxy — EZQ657S stale ~2023-01; full G10; Q659S live≡compound not mixed; not US BEA PCE; pub_lag=3). + Du–Keerati–Schreger gov CIP / UST premium (`cip_dataset_v4` `cip_govt` bps; tenor=**5y** a priori; pub_lag_days=1; slim `cip_g10_5y_govt_panel.csv`; §67) + **CIP-conditioned Lustig–Verdelhan carry** (§68; stress=−mean cip_govt gate/cool) + **soft-stack CIP enrichment** (§69; SOFT_LEGS_CIP=macro5+cip2; primary soft_ew7_cip) + **capital-sleeve soft-stack** (§70; locked fx4plus core + soft_ew_macro5/soft_ew7_cip satellites). + **CIP-conditioned soft EW** (§71; soft_ew_macro5 × CIP-stress gate/cool; primary soft_low_cip_stress) + **VIX/GPR-conditioned soft EW** (§72; soft_ew_macro5 × VIX/GPR gate/cool; primary soft_low_vix)
...
### 1.35 Building-permits / housing-starts activity differentials → FX (OECD MEI ODCNPI03)

- **Claim:** Currencies with *high* relative building-permits (housing-*activity* / construction flow) YoY growth subsequently appreciate vs low-activity peers (growth-channel / Taylor-rule FX; leading real-activity differential). Honesty alternate: long low housing / activity-stress. **Distinct from house-price §35** (BIS real HPI levels/growth — wealth/collateral channel).
- **Key refs:** Growth-channel / Taylor-rule FX literature applied to construction activity; housing starts/permits as leading real-activity indicators (distinct from Aoki–Proudman–Vlieghe house-*price* collateral).
- **Free data:** FRED `{ISO3}ODCNPI03GYSAM` permits YoY monthly (CAD/AUD/NZD/DEU live ~2026-04..07). EUR=`DEUODCNPI03GYSAM` Germany proxy (`EA19ODCNPI03GYSAM` ends 2023-08). USD=`PERMIT` levels→YoY (`USAODCNPI03GYSAM` **404**). GBP/JPY/CHF **unmapped** (ODCNPI03/WSCNDW 404 or stale). `*ODCNPI02*` starts **404**. ACM `THREEFYTP10` live — fallback **not used** (foreign panel = 4 ≥ threshold).
- **What we implement (wave §43):** `data/fred_building_permits.py` + `strategies/building_permits_fx.py` + `scripts/scholarly_fx_building_permits_wave.py` — legs `high_housing_xs` (primary), `low_housing_xs`, `high_housing_z_xs`, `housing_chg_xs`, `us_housing_stress_fx`, `us_housing_haven_usd`, `housing_ew`. Scores on **YoY growth**. PIT `pub_lag=2m` (housing a priori) + 1d weight lag. **Distinct** from house-price §35, OECD CLI/CCI/BCI, IP §42, employment §41. **Not** overlaid on the locked sleeve.


### 1.36 NY Fed ACM Treasury term premium → USD risk-appetite

- **Claim:** Elevated ACM 10y Treasury term premium (compensation for duration / interest-rate risk) coincides with USD safe-haven / risk-off demand; long USD when lagged z(THREEFYTP10) is elevated. Honesty alternate: long risk FX when TP elevated (wrong-signed check). Related carry conditioning: cool or gate cash-rate carry by TP z.
- **Key refs:** Adrian, Crump & Moench (2013), "Pricing the Term Structure with Linear Regressions," *JF*; Lustig, Stathopoulos & Verdelhan (2019), "The Term Structure of Currency Carry Trade Risk Premia," *AER*; Hofmann, Shim & Shin (2020), "Bond risk premia and the exchange rate," *JME*.
- **Free data:** FRED `THREEFYTP10` (ACM 10y TP, ~1990–present live) + companion `THREEFYTP5` (5y). `ACMTP10` / `ACMTP05` **404** — skipped.
- **What we implement (wave §44):** `data/fred_acm_term_premium.py` + `strategies/acm_term_premium_fx.py` + `scripts/scholarly_fx_acm_tp_wave.py` — legs `acm_tp_usd` (primary), `acm_tp_chg_usd`, `acm_tp_lvl_usd`, `acm_tp_stress_fx` (honesty), `carry_acm_tp_cool`, `carry_acm_tp_loose`, `acm_tp_ew`, optional `acm_tp5_usd`. PIT daily `pub_lag=1d` + `signal_lag=1d`. **Distinct** from yield-curve slope §16, real-rate/TIPS §23, funding-liq §20, IG OAS §36. **Not** overlaid on the locked sleeve.

### 1.37 Country-GPR bilateral (relative-to-US)

- **Claim:** Currencies with *low relative* geopolitical risk (home country GPR − US GPR) subsequently appreciate vs high-rel-GPR peers (Liu–Zhang / Caldara–Iacoviello bilateral refinement of country GPRC_*).
- **Key refs:** Caldara & Iacoviello (2022) AER country indexes; relative (home−US) differential as bilateral risk-premium state variable.
- **Free data:** Caldara–Iacoviello `gpr_country_monthly.csv` GPRC_* (live ~2026-08). EUR = EW of DEU/FRA/ITA/ESP/NLD/BEL. **NZD gap** (no `GPRC_NZL`).
- **What we implement (wave §45):** `data/country_gpr_bilateral.py` + `strategies/country_gpr_bilateral_fx.py` + `scripts/scholarly_fx_country_gpr_bilateral_wave.py` — legs `low_rel_gpr_xs` (primary), `high_rel_gpr_xs`, `low_rel_gpr_lvl_xs`, `rel_gpr_chg_xs`, `us_gpr_stress_fx`, `us_gpr_haven_usd`, `gpr_bilat_ew`. PIT `pub_lag=1m` + 1d weight lag; `signal_lag` months=0 a priori (modern XS). **Distinct** from absolute country-GPR §9, aggregate GPR §8, AI-GPR §25. **Not** overlaid on the locked sleeve.

### 1.38 Retail-sales / consumer-sales volume differentials → FX (OECD MEI SLRTTO / RSAFS)

- **Claim:** Currencies with *high* relative retail-sales / consumer-sales volume growth subsequently appreciate vs low-growth peers (Dahlquist–Hasseltoft 2020 economic-momentum channel remaining after IP §42 / employment §41 / permits §43). Honesty alternate: long low retail / consumer-stress.
- **Key refs:** Dahlquist & Hasseltoft (2020), "Economic Momentum and Currency Returns," *JFE*.
- **Free data:** FRED `SLRTTO01*Q657S` retail volume growth (GBP/JPY/AUD/NZD/CHF live ~2025-04..2026-04). EUR=`SLRTTO01DEQ657S` Germany proxy (`EA19WSCNDW` 404; `FRAWSCNDW` ends 2018; `SLRTTO01EZQ657S` ends 2023-07). USD=`RSAFS` Advance Retail Sales levels→YoY (`USAWSCNDW01GYSAM` ends 2023-11; `SLRTTO01USQ657S` ends 2018). CAD=`CANWSCNDW01IXOBSAM` index→YoY (ends ~2024-04; `SLRTTO01CAQ657S` ends 2022). `SARTM*` / monthly `SLRTTO*M657S` mostly 404/stale — not primary.
...
### 1.40 Unemployment-rate differentials → FX (OECD MEI / national UR)

- **Claim:** Currencies with *low* relative unemployment rates (strong labour markets) subsequently appreciate vs high-UR peers (Dahlquist–Hasseltoft 2020 economic-momentum labour channel). Honesty alternate: long high UR / labour-stress debtor premium. **Distinct from employment persons growth §41** (LFEMTTTT YoY) and from macro-diff §12 EW CPI+IP+UR *blend* (UR only as one EW leg — no dedicated UR XS until this wave).
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic momentum incl. unemployment; OECD MEI / national labour-force survey UR.
- **Free data:** FRED `LRHUTTTT*M156S` UR levels (USD/EUR/GBP/JPY/AUD/CAD); CHF=`LMUNRRTTCHM156S` registered UR; **NZD unmapped** on FRED. EUR EA UR sparse tail ~2023-01.
- **What we implement (wave §54):** `data/fred_unemployment.py` (wraps `fred_macro_diff.UR_SERIES` / `load_ur_panel`) + `strategies/unemployment_fx.py` + `scripts/scholarly_fx_unemployment_wave.py` — legs `low_ur_xs` (primary), `high_ur_xs`, `low_ur_z_xs`, `ur_chg_xs` (−Δ12 of level), `us_ur_stress_fx` / `us_ur_haven_usd` (US UR z ≥ +1.0), `ur_ew`. Scores on **UR level %**. PIT `pub_lag=1m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.


### 1.41 Manufacturing wage / earnings growth differentials → FX (OECD MEI LCEAMN01)

- **Claim:** Currencies with *low* relative manufacturing wage / earnings YoY growth subsequently appreciate vs high-wage peers (competitiveness / REER-adjacent labour-cost; Balassa–Samuelson wage side; Dahlquist–Hasseltoft labour companion). Honesty alternate: long high wage growth / labour-cost stress debtor. **Distinct from ULC §48** (ULQEUL01 = wages/productivity unit labour cost — LCEAMN was documented there as fallback "not primary"), employment persons §41, UR levels §54, LP §49.
- **Key refs:** Balassa–Samuelson relative wage / productivity; Dahlquist & Hasseltoft (2020) labour-momentum companion; OECD MEI manufacturing earnings.
- **Free data:** FRED `LCEAMN01*` — USD/CAD `*M657S` YoY live; EUR=`LCEAMN01EZQ657S` (ends ~2025-07; DE/FR alts); NZD=`*NZQ657S`; GBP/JPY `*M661S`→YoY; AUD `*AUQ661S`→YoY; **CHF unmapped** (404).
- **What we implement (wave §55):** `data/fred_wage_earnings.py` + `strategies/wage_earnings_fx.py` + `scripts/scholarly_fx_wage_earnings_wave.py` — legs `low_wage_growth_xs` (primary), `high_wage_growth_xs`, `low_wage_z_xs`, `wage_chg_xs` (−Δ12 of YoY), `us_wage_stress_fx` / `us_wage_haven_usd` (US wage YoY z ≥ +1.0), `wage_ew`. Scores on **wage YoY %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.





### 1.42 Labour-force participation / activity-rate differentials → FX (OECD MEI LRAC64TT)

- **Claim:** Currencies with *high* relative labour-force participation / activity rates (ages 15–64) subsequently appreciate vs low-LFP peers (growth / extensive labour-supply margin; Dahlquist–Hasseltoft 2020 labour companion). Honesty alternate: long low LFP / labour-disengagement debtor premium. **Distinct from employment persons §41** (LFEMTTTT YoY), **UR levels §54** (jobless rate), **wages §55** (LCEAMN earnings YoY), ULC/LP/CU.
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic momentum labour companion; OECD MEI activity rate ages 15–64.
- **Free data:** FRED `LRAC64TT*` — USD/JPY/CAD/AUD `*M156S` live; GBP/CHF/NZD `*Q156S`; EUR=`LRAC64TTDEQ156S` Germany proxy (`EZQ156S` stale ~2022-10); full G10. Vacancies `LMJVTTUV*` / hours `HOHWMN02*` documented alts — not this wave.
- **What we implement (wave §56):** `data/fred_lfp.py` + `strategies/lfp_fx.py` + `scripts/scholarly_fx_lfp_wave.py` — legs `high_lfp_xs` (primary), `low_lfp_xs`, `high_lfp_z_xs`, `lfp_chg_xs` (+Δ12 of level), `us_lfp_stress_fx` / `us_lfp_haven_usd` (US LFP z ≤ −1.0), `lfp_ew`. Scores on **LFP level %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.

### 1.43 Employment-rate differentials → FX (OECD MEI LREM64TT)

- **Claim:** Currencies with *high* relative employment rates (employed/population ages 15–64) subsequently appreciate vs low-ER peers (labour-strength / engagement **stock** between participation and joblessness; Dahlquist–Hasseltoft 2020 labour companion). Honesty alternate: long low ER / labour-disengagement debtor premium. **Distinct from employment persons §41** (LFEMTTTT YoY), **UR levels §54** (LRHUTTTT), **LFP/activity §56** (LRAC64TT), **wages §55**, ULC/LP/CU.
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic momentum labour companion; OECD MEI employment rate ages 15–64.
- **Free data:** FRED `LREM64TT*` — USD/JPY/CAD/AUD `*M156S` live; GBP/CHF/NZD `*Q156S`; EUR=`LREM64TTDEQ156S` Germany proxy (`EZQ156S` stale ~2022-10); full G10. EMRATIO/CE16OV US-only; vacancies/hours alts documented — not this wave.
- **What we implement (wave §57):** `data/fred_employment_rate.py` + `strategies/employment_rate_fx.py` + `scripts/scholarly_fx_employment_rate_wave.py` — legs `high_er_xs` (primary), `low_er_xs`, `high_er_z_xs`, `er_chg_xs` (+Δ12 of level), `us_er_stress_fx` / `us_er_haven_usd` (US ER z ≤ −1.0), `er_ew`. Scores on **ER level %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.


### 1.44 Real GDP growth differentials → FX (OECD MEI NAEXKP01)

- **Claim:** Currencies with *high* relative real GDP YoY growth subsequently appreciate vs low-growth peers (growth / economic-momentum prior; Dahlquist–Hasseltoft 2020 macro–FX). Honesty alternate: long low GDP growth / activity-stress debtor premium. **Distinct from IP §42**, **CLI §37**, **BCI §39**, **emp persons §41**, **UR §54**, **debt/GDP §29**, **CA/GDP §21**, emp-rate §57, LFP §56, wages §55.
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic momentum; classic growth-differential FX channel.
- **Free data:** FRED `NAEXKP01*Q657S` — Growth rate previous period (QoQ %, SA) → compounded to YoY % (matches US `GDPC1` pct_change(4)); USD/GBP/JPY/CAD/AUD/CHF/NZD live ~2026-01..04; EUR=`NAEXKP01DEQ657S` Germany proxy (`EZQ657S` stale ~2023-01); full G10. `Q659S` YoY **404**; `Q652S` levels partial stale/404; national alts (`GDPC1`, `JPNRGDPEXP`, `CLVMNACSCAB1GQEA19`) documented not mixed.
- **What we implement (wave §58):** `data/fred_gdp.py` + `strategies/gdp_fx.py` + `scripts/scholarly_fx_gdp_wave.py` — legs `high_gdp_xs` (primary), `low_gdp_xs`, `high_gdp_z_xs`, `gdp_chg_xs` (+Δ12 of YoY = acceleration), `us_gdp_stress_fx` / `us_gdp_haven_usd` (US GDP z ≤ −1.0), `gdp_ew`. Scores on **GDP YoY %**. PIT `pub_lag=3m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.


### 1.45 Construction-production differentials → FX (OECD MEI PRCNTO01)

- **Claim:** Currencies with *high* relative construction-output YoY growth subsequently appreciate vs low-growth peers (growth-channel / Taylor-rule FX applied to coincident construction *production*). Honesty alternate: long low construction / activity-stress debtor premium. **Distinct from building-permits §43** (ODCNPI03 — leading housing *flow*), **IP §42** (PRINTO01 — industry *excl.* construction), **GDP §58** (NAEXKP01), CLI §37, BCI §39, retail §46.
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic-momentum spirit; classic growth-differential / Taylor-rule FX channel on real activity.
- **Free data:** FRED `PRCNTO01` — USD/EUR/GBP/CAD `*GYSAM` monthly YoY; JPY=`JPNPRCNTO01GYSAQ` quarterly YoY (monthly GYSAM stale ~2020-07); AUD/NZD/CHF=`PRCNTO01*Q657S` QoQ→YoY; EUR=`DEUPRCNTO01GYSAM` Germany proxy; full G10. `CHEPRCNTO01GYSAM` **404**; `USAPRCNTO01IXOBSAM` stale ~2024-03 — not primary.
- **What we implement (wave §59):** `data/fred_construction.py` + `strategies/construction_fx.py` + `scripts/scholarly_fx_construction_wave.py` — legs `high_cons_xs` (primary), `low_cons_xs`, `high_cons_z_xs`, `cons_chg_xs` (+Δ12 of YoY), `us_cons_stress_fx` / `us_cons_haven_usd` (US cons z ≤ −1.0), `cons_ew`. Scores on **construction YoY %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag (mirror GDP §58 / ER §57 — *not* IP §42 zero-month signal lag). **Not** overlaid on the locked sleeve.



### 1.46 Passenger-car registrations differentials → FX (OECD MEI SLRTCR03)

- **Claim:** Currencies with *high* relative passenger-car registration YoY growth subsequently appreciate vs low-growth peers (growth-channel / durable consumer-demand FX). Honesty alternate: long low car-regs / demand-stress debtor premium. **Distinct from retail-sales §46** (SLRTTO01 / RSAFS — broad retail volume), **building-permits §43**, **construction §59**, **IP §42**, CLI §37, BCI §39.
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic-momentum spirit on real-activity differentials; classic growth-channel / Taylor-rule FX on durable demand.
- **Free data:** FRED `{ISO3}SLRTCR03GYSAM` — USD/GBP/JPY/CAD/AUD live ~2026-06..08; EUR=`ESPSLRTCR03GYSAM` Spain proxy (`DEU`/`FRA` GYSAM **404**; `EA19` stale ~2018-12); NZD/CHF **unmapped** (stale). Fall-through: housing starts `*ODCNPI02*` / completions `*ODCNPI04*` G10 **404** + US-only `HOUST`/`COMPUTSA`; mfg orders `*ODMNNO*`/`*ODMNTO*` **404** + US-only `AMTMNO`.
- **What we implement (wave §60):** `data/fred_passenger_cars.py` + `strategies/passenger_cars_fx.py` + `scripts/scholarly_fx_passenger_cars_wave.py` — legs `high_cars_xs` (primary), `low_cars_xs`, `high_cars_z_xs`, `cars_chg_xs` (+Δ12 of YoY), `us_cars_stress_fx` / `us_cars_haven_usd` (US cars z ≤ −1.0), `cars_ew`. Scores on **passenger-car regs YoY %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.


### 1.47 Merchandise export-value differentials → FX (OECD MEI XTEXVA01)

- **Claim:** Currencies with *high* relative merchandise export-value YoY subsequently appreciate vs low-export-growth peers (growth-channel / external-demand FX; Dahlquist–Hasseltoft spirit on trade intensity). Honesty alternate: long low export YoY / external-demand-stress premium. **Distinct from trade-balance §30** (TB/exports ratio), **CA §21**, ToT (UV 404), **retail §46**, **IP §42**, **passenger-cars §60**, **construction §59**, **GDP §58**. Series unit = **value** (not volume).
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic-momentum / growth differentials; external-demand / trade-intensity channel.
- **Free data:** FRED `XTEXVA01{ISO2}M657S` — full G10 live ~2026-06..08; EUR=`XTEXVA01DEM657S` Germany proxy (`EZM657S` **404**; `EZM667S` stale ~2023-04). Companion `*M667S` levels = value units — **not** mixed into score panel. Fall-through: CIP/cross-currency basis Missing free G10; hours `HOHWMN*` US-only/404; vacancies `LMJVTTUV*` thin/stale; services `PRSVTT*` / mfg turnover `ODMN*` 404.
- **What we implement (wave §61):** `data/fred_export_value.py` + `strategies/export_value_fx.py` + `scripts/scholarly_fx_export_value_wave.py` — legs `high_xval_xs` (primary), `low_xval_xs`, `high_xval_z_xs`, `xval_chg_xs` (+Δ12 of YoY), `us_xval_stress_fx` / `us_xval_haven_usd` (US xval z ≤ −1.0), `xval_ew`. Scores on **export-value YoY %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.





### 1.48 Merchandise import-value differentials → FX (OECD MEI XTIMVA01)

- **Claim:** Currencies with *high* relative merchandise import-value YoY subsequently appreciate vs low-import-growth peers (growth-channel / domestic-absorption / import-demand FX; Dahlquist–Hasseltoft spirit on trade intensity). Honesty alternate: long low import YoY / import-demand-stress premium. **Distinct from export-value §61** (XTEXVA01 external demand), **trade-balance §30**, **CA §21**, ToT (UV 404), **retail §46**, **IP §42**, **passenger-cars §60**, **construction §59**, **GDP §58**. Series unit = **value** (not volume).
- **Key refs:** Dahlquist & Hasseltoft (2020), *JFE* — economic-momentum / growth differentials; domestic-absorption / import-demand channel (mirror of §61 external demand).
- **Free data:** FRED `XTIMVA01{ISO2}M657S` — full G10 live ~2026-05/06; EUR=`XTIMVA01DEM657S` Germany proxy (`EZM657S` **404**; `EZM667S` stale ~2023-04). Companion `*M667S` levels = value units — **not** mixed into score panel. Fall-through: CIP/cross-currency basis Missing free G10; hours `HOHWMN*` US-only/404; vacancies `LMJVTTUV*` thin/stale; services `PRSVTT*` / mfg turnover `ODMN*` 404.
- **What we implement (wave §62):** `data/fred_import_value.py` + `strategies/import_value_fx.py` + `scripts/scholarly_fx_import_value_wave.py` — legs `high_ival_xs` (primary), `low_ival_xs`, `high_ival_z_xs`, `ival_chg_xs` (+Δ12 of YoY), `us_ival_stress_fx` / `us_ival_haven_usd` (US ival z ≤ −1.0), `ival_ew`. Scores on **import-value YoY %**. PIT `pub_lag=2m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.


### 1.49 Household / consumer credit growth differentials → FX (BIS CRDQ*AHABIS)

- **Claim:** Currencies with *high* relative household-credit YoY subsequently appreciate vs low-growth peers (consumer-demand / credit-financed absorption; Dahlquist–Hasseltoft economic-momentum spirit on household credit). Honesty alternate: long low HH-credit YoY / household-leverage stress premium (Mian–Sufi / Borio–Drehmann household-sector application). **Distinct from private credit/GDP §32** (`Q*PAM770A` stock %GDP + credit-gap; absolute `CRDQ*APABIS` PNFS unused), **money-growth §33**, **DSR §47**, **debt/GDP §29**, **house-price §35**, **retail §46**, **cars §60**, **import-value §62**. Score unit = abs→YoY (not %GDP).
- **Key refs:** Mian & Sufi (household leverage); Borio & Drehmann / BIS financial-cycle literature (household sector); Dahlquist & Hasseltoft (2020), *JFE*.
- **Free data:** FRED `CRDQ{ISO2}AHABIS` household credit absolute → YoY %; EUR=`CRDQXMAHABIS` euro-area (`CRDQEAAHABIS` 404); NZD `CRDQNZAHABIS` **404** unmapped. Fall-through: merchandise trade VOLUME `XTINTVA`/`XTEXVO`/`XTIMVO` **404**; household saving `NASAVP*`/`HHSBRG*` **404** (PSAVERT US-only); inventories `STMNTO*` **404**; order books `BSOITE02*` <4 G10.
- **What we implement (wave §63):** `data/fred_household_credit.py` + `strategies/household_credit_fx.py` + `scripts/scholarly_fx_household_credit_wave.py` — legs `high_hhcred_xs` (primary), `low_hhcred_xs`, `high_hhcred_z_xs`, `hhcred_chg_xs` (+Δ12 of YoY), `us_hhcred_stress_fx` / `us_hhcred_haven_usd` (US hhcred z ≤ −1.0), `hhcred_ew`. Scores on **household-credit YoY %**. PIT `pub_lag=5m` + `signal_lag=1m` + 1d weight lag. **Not** overlaid on the locked sleeve.
...
- **What we implement:**
  ...
  - **Bilateral refinement (wave §45):** `data/country_gpr_bilateral.py` + `strategies/country_gpr_bilateral_fx.py` + `scripts/scholarly_fx_country_gpr_bilateral_wave.py` — *relative-to-US* score = home − US GPR (levels / 60m z / Δ12); primary `low_rel_gpr_xs`; US stress/haven tilts; NZD gap. Distinct from absolute §9 / aggregate §8 / AI-GPR §25.
...
20. Next scholarly candidates (not sleeve coolers): **FTMO MT5 CSV re-score** when exports arrive (all boards still `approximate_non_ftmo`; `data/ftmo/` empty except README); FX IV/RR **blocked**; news-based currency sentiment still blocked without free multi-year panel. (**Household-credit §63 done** after merchandise import-value §62 / export-value §61 — CIP/basis Missing free G10 + hours/vacancies thin + PRSVTT*/ODMN* 404 fall-through; Merchandise export-value §61 / Passenger-car registrations §60 / Construction-production §59 / Real GDP growth §58 / Employment-rate §57 / LFP/activity §56 / Wage/earnings §55 / Unemployment-rate §54 / Capital-sleeve mix §53 / OECD CPI §52 / PPI §51 / capacity-utilization §50 / labour-productivity §49 / ULC §48 / BIS DSR §47 / Retail-sales §46 / Country-GPR bilateral §45 / ACM TP §44 / Building-permits §43 / Industrial-production §42 / Employment §41 / WUI §40 / BCI §39 / CCI §38 / CLI §37 done.) Free scholarly queue **thinning** — dedicated UR + LCEAMN wage + LFP/activity + employment-rate + real-GDP-growth + construction-production + passenger-car-regs + merchandise-export-value + merchandise-import-value + household-credit gaps closed; credit-impulse already §32 companion (`credit_chg_xs`) — skip as primary. Hours/vacancies mostly 404/stale; CIP/basis still Missing. IMF IFS trade unit-value ToT **404** on FRED.
...
**Next structure (if promote=0):** Done as §43 (building-permits ODCNPI03). FX IV/RR + news-sentiment still **blocked**. Next free scholarly candidate: NY Fed ACM term premium → USD risk-appetite (`THREEFYTP10`), or FTMO CSV re-score when exports arrive.


## 43. Building-permits / housing-activity (OECD MEI ODCNPI03) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI `{ISO3}ODCNPI03GYSAM` permits YoY (monthly) + USD Census `PERMIT` levels→YoY (`USAODCNPI03GYSAM` 404). EUR = `DEUODCNPI03GYSAM` Germany proxy (`EA19ODCNPI03GYSAM` ends 2023-08). GBP/JPY/CHF **unmapped** (404). Primary `high_housing_xs`: long high relative permits YoY / short low (`n_long=n_short=2` on available foreign EUR/CAD/AUD/NZD). Companions: `low_housing_xs` (honesty / activity-stress), `high_housing_z_xs` (60m z of YoY), `housing_chg_xs` (Δ12 of YoY), `us_housing_stress_fx` / `us_housing_haven_usd` (US permits YoY z ≤ −1.0), `housing_ew` (EW of primary + chg + stress). PIT pub_lag=2m (housing a priori) + 1d weight lag (no extra month signal lag); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = YoY growth** (OECD GYSAM as reported; US PERMIT derived). ACM `THREEFYTP10` live fallback **not used** (foreign panel = 4 ≥ ~4–5 threshold).

**What is new vs house-price §35 / OECD CLI·CCI·BCI / IP §42 / employment:** House-price §35 is BIS *real HPI* wealth/collateral; CLI/CCI/BCI are leading/sentiment balances; IP §42 is coincident industry YoY; employment §41 is LFEMTTTT labour. This wave is *building-permits / construction-activity flow* XS (ODCNPI03) + US housing stress/haven tilts.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/CAD/AUD/NZD; unmapped: GBP/JPY/CHF.
...
| low_housing_xs | −0.025% | −0.48 | −0.50 | 44% | — | −0.13 |
| high_housing_z_xs | +0.015% | +0.26 | +0.27 | 55% | — | +0.09 |
| housing_chg_xs | −0.083% | −1.65 | −1.62 | 52% | — | −0.32 |
| us_housing_stress_fx | +0.027% | +1.13 | +1.19 | 11% | — | +0.26 |
| us_housing_haven_usd | −0.030% | −1.22 | −1.29 | 9% | — | −0.28 |
| housing_ew | −0.016% | −0.58 | −0.57 | 47% | — | −0.08 |

### Consistency windows (primary `high_housing_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.01% | 45% | PASS | no |
| 2025 | −0.26% | 27% | PASS | no |
| 2026 | −0.07% | 50% | PASS | no |
| holdout_365d | −0.04% | 50% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_housing_xs` scale≈**3.517** bind=**daily**; scaled HO mean≈−0.13%/mo %pos 50% — clears: **NO**. Soft-best-ish = `us_housing_stress_fx` (~+2.7 bp/mo, NW t≈+1.19 — below |t|≥1.5 soft gate).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS**.

**Next structure (if promote=0):** Done as §44 (ACM term-premium THREEFYTP10). FX IV/RR + news-sentiment still **blocked**. Next free scholarly candidate: FTMO CSV re-score when exports arrive, or country-GPR bilateral refinement / live OECD retail if found.

## 44. NY Fed ACM term premium → USD risk-appetite FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED NY Fed ACM `THREEFYTP10` (10y Treasury term premium) + companion `THREEFYTP5` (5y; `ACMTP10` 404 skipped). Primary `acm_tp_usd`: long USD when lagged z(THREEFYTP10) ≥ 1.0 (binary usd_tilt=0.5). Companions: `acm_tp_chg_usd`, `acm_tp_lvl_usd` (continuous intensity), `acm_tp5_usd`, `acm_tp_stress_fx` (honesty wrong-signed risk-FX tilt), `carry_acm_tp_cool`, `carry_acm_tp_loose`, `acm_tp_ew` (EW of primary + chg + tp5). PIT daily `pub_lag=1d` + `signal_lag=1d`; z_window=252, min_periods=60, cool=0.35; costs 1.5 bps/side.

**What is new vs yield-curve / real-rate / funding-liq / IG OAS:** Yield-curve §16 is IRLTLT−IRSTCI slope; real-rate §23 is TIPS/BE (`DFII10`/`T10YIE`); funding-liq §20 is NFCI/TED/CPFF/BAA10Y; IG OAS §36 is corporate credit. This wave is *Treasury term premium* (ACM) as USD risk-appetite / duration-risk state variable.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). THREEFYTP10/5 live ~1990–2026-09 (~9.2k daily rows after pub lag).
...
### Consistency windows (primary `acm_tp_usd`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.14% | 36% | PASS | no |
| 2025 | −0.08% | 0% | PASS | no |
| 2026 | +0.03% | 62% | PASS | no |
| holdout_365d | −0.13% | 42% | PASS | no |

**Board:** n=8 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `acm_tp_usd` scale≈**3.727** bind=**daily**; scaled HO mean≈−0.48%/mo %pos 42% — clears: **NO**. Soft-best-ish = `acm_tp_lvl_usd` (~+4.0 bp/mo, NW t≈+1.05 — below |t|≥1.5 soft gate); `acm_tp5_usd` similar (~+4.0 bp/mo, NW t≈+1.12).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS**.

**Next structure (if promote=0):** Done as §45 (country-GPR bilateral rel-to-US). FX IV/RR + news-sentiment still **blocked**. OECD retail still stale (skipped). Next: **FTMO MT5 CSV re-score** when exports arrive (all boards `approximate_non_ftmo`).

Artifacts: `reports/scholarly_fx_acm_tp_wave.md`, `scholarly_fx_acm_tp_*.csv`, `scholarly_fx_acm_tp_meta.json`, `quest_scholarly_fx_acm_tp.md`.

## 45. Country-GPR bilateral (relative-to-US) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** Caldara–Iacoviello country GPRC_* panel → score = home − US GPR. Primary `low_rel_gpr_xs`: long low (home−US) trailing z / short high (`n_long=n_short=2` on EUR/GBP/JPY/CAD/AUD/CHF; **NZD absent** — no `GPRC_NZL`). Companions: `high_rel_gpr_xs` (honesty), `low_rel_gpr_lvl_xs` (raw levels), `rel_gpr_chg_xs` (−Δ12 of rel levels), `us_gpr_stress_fx` / `us_gpr_haven_usd` (US GPR z ≥ 1.0), `gpr_bilat_ew` (EW of primary + chg + stress). PIT pub_lag=1m (same as §9) + 1d weight lag; **signal_lag months=0** a priori (modern XS §43-style — pub_lag already PIT); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. Carry cool skipped (no rate-panel dependency). EUR = EW of DEU/FRA/ITA/ESP/NLD/BEL.

**What is new vs §9 / §8 / §25:** Absolute home-GPRC sort+LP (§9) has no multi-leg board / no FTMO risk sweep; aggregate GPR regime (§8) is global; AI-GPR (§25) is threats/acts/oil daily roles. This wave is *bilateral relative-to-US* differentials + US geopolitics tilts with modern board + risk sweep.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Country panel live through ~2026-08 (500 months after pub_lag).
...
### Consistency windows (primary `low_rel_gpr_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.02% | 45% | PASS | no |
| 2025 | −0.06% | 45% | PASS | no |
| 2026 | −0.03% | 50% | PASS | no |
| holdout_365d | −0.01% | 50% | PASS | no |

**Board:** n=7 soft_nw_pos=**1** hard_nw_pos=**1** promote=**0**.

Sweep **run**. Primary `low_rel_gpr_xs` scale≈**1.771** bind=**static**; scaled HO mean≈−0.03%/mo %pos 50% — clears: **NO**. Soft/hard-best = `us_gpr_haven_usd` (~+7.6 bp/mo, NW t≈**+2.31** — hard |t|≥2.0 gate; elevated US GPR → long USD). Honesty: `us_gpr_stress_fx` wrong-signed (NW t≈−2.41) supports haven over stress for US GPR spikes on this sample.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS**.

**Next structure (if promote=0):** Done as §46 (retail-sales SLRTTO / RSAFS). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_cgpr_bilat_wave.md`, `scholarly_fx_cgpr_bilat_*.csv`, `scholarly_fx_cgpr_bilat_meta.json`, `quest_scholarly_fx_cgpr_bilat.md`, `quest_locked_verify_cgpr_bilat.md`.

## 46. Retail-sales / consumer-sales (OECD MEI SLRTTO / RSAFS) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI `SLRTTO01*Q657S` retail volume growth (quarterly → monthly after `pub_lag_months=2`) + USD Census `RSAFS` levels→YoY + CAD `CANWSCNDW01IXOBSAM` index→YoY (ends ~2024-04). EUR = `SLRTTO01DEQ657S` Germany proxy (`EA19WSCNDW` 404; `FRAWSCNDW` ends 2018; `SLRTTO01EZQ657S` ends 2023-07). Primary `high_retail_xs`: long high relative retail YoY / short low (`n_long=n_short=2` full G10 foreign). Companions: `low_retail_xs` (honesty / consumer-stress), `high_retail_z_xs` (60m z of YoY), `retail_chg_xs` (Δ12 of YoY), `us_retail_stress_fx` / `us_retail_haven_usd` (US retail YoY z ≤ −1.0), `retail_ew` (EW of primary + chg + stress). PIT pub_lag=2m (retail a priori — same band as IP §42) + 1d weight lag (no extra month signal lag); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = YoY / growth %** (SLRTTO as reported; RSAFS/CAD derived).

**What is new vs IP §42 / employment §41 / permits §43 / OECD CLI·CCI·BCI:** IP is coincident industry YoY; employment is LFEMTTTT labour persons; permits are construction-activity flow; CLI/CCI/BCI are leading/sentiment balances. This wave is *retail / consumer-sales volume growth* XS (Dahlquist–Hasseltoft economic-momentum channel) + US retail stress/haven tilts.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Full G10 mapped; CAD gap after ~2024-06 PIT; AUD raw SLRTTO ends ~2025-04 (panel ffill with peers).
...
### Consistency windows (primary `high_retail_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.25% | 36% | PASS | no |
| 2025 | +0.24% | 64% | PASS | no |
| 2026 | +0.06% | 50% | PASS | no |
| holdout_365d | +0.23% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_retail_xs` scale≈**1.071** bind=**static**; scaled HO mean≈+0.25%/mo %pos 67% — clears: **NO**. Soft-best-ish = `retail_chg_xs` (~+6.2 bp/mo, NW t≈+1.14 — below |t|≥1.5 soft gate); `high_retail_z_xs` similar (~+5.9 bp/mo, NW t≈+1.08).
...
**Next structure (if promote=0):** Done as §48 (OECD ULC / ULQEUL01). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.
...
## 48. OECD unit labour cost (ULC / ULQEUL01) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI early-estimate quarterly ULC YoY `ULQEUL01{ISO2}Q657S` (quarterly → monthly after `pub_lag_months=3`) + `signal_lag=1m` + 1d weight lag. EUR = `ULQEUL01DEQ657S` Germany proxy (`ULQEUL01EZQ657S` ends 2024-01 stale; FR/IT/ES/NL alts). Full G10 mapped incl NZD/CHF. Primary `low_ulc_xs`: long low relative ULC YoY / short high — competitiveness prior (`n_long=n_short=2`). Companions: `high_ulc_xs` (honesty reverse), `low_ulc_z_xs` (60m z), `ulc_chg_xs` (−Δ12 of YoY), `us_ulc_stress_fx` / `us_ulc_haven_usd` (US ULC z ≥ +1.0), `ulc_ew` (EW of primary + chg + stress). PIT pub_lag=3m (labour-adjacent a priori — same band as employment §41); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = ULC YoY % as reported**.

**What is new vs employment §41 / IP §42 / REER §31 / retail §46 / DSR §47:** Employment is *persons* growth; IP is industrial *volume*; REER is price-level misalignment; retail is consumer *sales*; DSR is debt-*service* flow. This wave is *unit labour cost* growth — competitiveness / REER-adjacent labour-cost channel (Balassa–Samuelson-adjacent). Classic `ULQULC01*` / `{ISO3}ULQEUL01GYSAQ` mostly **404**; `*Q661S` index stale ~2023-07 — not primary. LCEAMN wage fallback documented, not boarded as primary once full ULC YoY panel found.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: full G10 USD/EUR(DE)/GBP/JPY/CAD/AUD/NZD/CHF.
...
### Consistency windows (primary `low_ulc_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.44% | 18% | PASS | no |
| 2025 | +0.17% | 64% | PASS | no |
| 2026 | −0.12% | 38% | PASS | no |
| holdout_365d | −0.03% | 42% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `low_ulc_xs` scale≈**0.943** bind=**static**; scaled HO mean≈−0.03%/mo %pos 42% — clears: **NO**. Soft-best-ish = `high_ulc_xs` (~+2.9 bp/mo, NW t≈+0.52 — below |t|≥1.5 soft gate; honesty reverse wins on full sample).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_ulc.md`).

**Next structure (if promote=0):** Done as §49 (OECD labour-productivity / ULQELP). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_ulc_wave.md`, `scholarly_fx_ulc_*.csv`, `scholarly_fx_ulc_meta.json`, `quest_locked_verify_ulc.md`.

## 49. OECD labour-productivity (ULQELP01) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI early-estimate quarterly labour-productivity YoY `ULQELP01{ISO2}Q657S` (quarterly → monthly after `pub_lag_months=3`) + `signal_lag=1m` + 1d weight lag. EUR = `ULQELP01DEQ657S` Germany proxy (EZ also ends ~2023-07). Full G10 mapped incl NZD/CHF. Primary `high_lp_xs`: long high relative LP YoY / short low — productivity / Balassa–Samuelson-adjacent prior (`n_long=n_short=2`). Companions: `low_lp_xs` (honesty reverse), `high_lp_z_xs` (60m z), `lp_chg_xs` (+Δ12 of YoY), `us_lp_stress_fx` / `us_lp_haven_usd` (US LP z ≤ −1.0), `lp_ew` (EW of primary + chg + stress). PIT pub_lag=3m (labour-adjacent a priori — same band as ULC §48 / employment §41); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = LP YoY % as reported**.

**What is new vs employment §41 / IP §42 / ULC §48 / Balassa–Samuelson §14:** Employment is *persons* growth; IP is industrial *volume*; ULC is unit labour *cost*; BS §14 used IP *levels* as productivity proxy + real-FX residual. This wave is OECD MEI *labour productivity* YoY (output per worker) — competitiveness companion to ULC. **STALE:** all G10 `ULQELP01*Q657S` end ~**2023-04..07** on free FRED (same class as ULC Q661S). `{ISO3}ULQELP01GYSAQ` **404**. IMF IFS trade unit-value / ToT fallback (documented optional) also **404** on FRED (OECD LOCOTTOR only Korea; PIEAMP ends 2022-12). US `OPHNFB`/`PRS85006092` live alts — not mixed into OECD panel.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: full G10 USD/EUR(DE)/GBP/JPY/CAD/AUD/NZD/CHF — all stale.
...
### Consistency windows (primary `high_lp_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.06% | 55% | PASS | no |
| 2025 | −0.34% | 27% | PASS | no |
| 2026 | +0.05% | 50% | PASS | no |
| holdout_365d | −0.20% | 33% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_lp_xs` scale≈**0.918** bind=**static**; scaled HO mean≈−0.19%/mo %pos 33% — clears: **NO**. Soft-best-ish = `low_lp_xs` (~+4.4 bp/mo, NW t≈+0.82 — below |t|≥1.5 soft gate; honesty reverse wins on full sample).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_labour_prod.md`).

**Next structure (if promote=0):** Done as §50 (OECD capacity-utilization / ULQECU). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_labour_prod_wave.md`, `scholarly_fx_labour_prod_*.csv`, `scholarly_fx_labour_prod_meta.json`, `quest_locked_verify_labour_prod.md`.


## 50. OECD capacity-utilization (ULQECU01) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI early-estimate quarterly capacity-utilization YoY `ULQECU01{ISO2}Q657S` (quarterly → monthly after `pub_lag_months=3`) + `signal_lag=1m` + 1d weight lag. EUR = `ULQECU01DEQ657S` Germany proxy (EZ also ends ~2023-07). Full G10 mapped incl NZD/CHF. Primary `high_cu_xs`: long high relative CU YoY / short low — cyclical strength / demand-pressure prior (`n_long=n_short=2`). Companions: `low_cu_xs` (honesty reverse), `high_cu_z_xs` (60m z), `cu_chg_xs` (+Δ12 of YoY), `us_cu_stress_fx` / `us_cu_haven_usd` (US CU z ≤ −1.0), `cu_ew` (EW of primary + chg + stress). PIT pub_lag=3m (labour/activity quarterly band — same as ULC §48 / LP §49); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = CU YoY % as reported**.

**What is new vs IP §42 / employment §41 / ULC §48 / LP §49 / CLI/BCI / building-permits:** IP is industrial *volume*; employment is *persons*; ULC is unit labour *cost*; LP is output-per-worker; CLI/BCI are survey composites; building-permits are housing starts. This wave is OECD MEI *capacity utilization* YoY — Phillips-curve / output-gap adjacent cyclical strength. Completes ULC/LP trilogy. **STALE:** all G10 `ULQECU01*Q657S` end ~**2023-04..07** on free FRED. `{ISO3}ULQECU01GYSAQ` **404**. US `TCU` live national level alt through ~2026-08 — not mixed into OECD YoY panel.
...
### Consistency windows (primary `high_cu_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.00% | 64% | PASS | no |
| 2025 | −0.33% | 27% | PASS | no |
| 2026 | +0.09% | 50% | PASS | no |
| holdout_365d | +0.09% | 50% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_cu_xs` scale≈**0.816** bind=**static**; scaled HO mean≈+0.08%/mo %pos 50% — clears: **NO**. Soft-best-ish = `low_cu_xs` (~+4.5 bp/mo, NW t≈+0.78 — below |t|≥1.5 soft gate; honesty reverse wins on full sample).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.35%/55%/78%; 2025 2.44%/73%/60%; 2026 2.37%/75%/87%; HO 1.51%/67%/83% — see `quest_locked_verify_cap_util.md`).

**Next structure (if promote=0):** Done as §51 (OECD PPI / PIEAMP) then §52 (OECD CPI). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_cap_util_wave.md`, `scholarly_fx_cap_util_*.csv`, `scholarly_fx_cap_util_meta.json`, `quest_locked_verify_cap_util.md`.


## 51. OECD producer-price / PPI (PIEAMP01) FX wave results (2026-09-23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI manufacturing PPI YoY `{ISO3}PIEAMP01GYM` (monthly) + JPY `JPNPPDMMINMEI` levels→YoY + AUD/NZD `PIEAMP01*Q661N` quarterly index→YoY (→ monthly after `pub_lag_months=2`) + `signal_lag=1m` + 1d weight lag. EUR = `DEUPIEAMP01GYM` Germany proxy (EA19 GYM also ends 2022-12). Full G10 mapped. Primary `high_ppi_xs`: long high relative PPI YoY / short low — Dahlquist–Hasseltoft (2020) economic-momentum prior (`n_long=n_short=2`). Companions: `low_ppi_xs` (honesty reverse), `high_ppi_z_xs` (60m z), `ppi_chg_xs` (Δ12 of YoY), `us_ppi_stress_fx` / `us_ppi_haven_usd` (US PPI z ≤ −1.0), `ppi_ew` (EW of primary + chg + stress). PIT pub_lag=2m (PPI ~1–2m band — same as IP §42 / retail §46); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = PPI YoY %**.

**What is new vs IP §42 / retail §46 / employment §41 / ULC §48 / LP §49 / CU §50:** IP is industrial *volume*; retail is consumer *sales*; employment is *persons*; ULC/LP/CU are labour-cost / productivity / capacity. This wave is *producer-price* YoY differentials — remaining Dahlquist economic-momentum channel (PPI). **STALE:** GYM / M661N end ~**2022-12** (AUD Q ~2023-01; NZD Q ~2022-07). `{ISO3}PIEATI01GYSAM` / ISO2 `*M657S`/`*Q657S` / Japan `PIEAMP01JP*` **404**. US `PPIACO`/`PPIFIS` live alts — not mixed into OECD panel.
...
### Consistency windows (primary `high_ppi_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.06% | 73% | PASS | no |
| 2025 | −0.01% | 64% | PASS | no |
| 2026 | +0.03% | 50% | PASS | no |
| holdout_365d | +0.17% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**1** hard_nw_pos=**1** promote=**0**.

Sweep **run**. Primary `high_ppi_xs` scale≈**2.054** bind=**daily**; scaled HO mean≈+0.34%/mo %pos 67% — clears: **NO**. Soft/hard-best = `high_ppi_xs` itself (~+11.6 bp/mo, NW t≈**+2.00**). Honesty `low_ppi_xs` is significantly *negative* (sign confirms primary).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.40%/55%/75%; 2025 2.25%/82%/60%; 2026 2.22%/88%/86%; HO 1.50%/75%/80% — see `quest_locked_verify_ppi.md`).

**Next structure (if promote=0):** Done as §52 (OECD CPI / inflation YoY). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_ppi_wave.md`, `scholarly_fx_ppi_*.csv`, `scholarly_fx_ppi_meta.json`, `quest_locked_verify_ppi.md`.


## 52. OECD CPI / inflation YoY FX wave results (2026-09-24 BST)

**Design (fixed priors, no HO tuning):** FRED CPI/HICP index→YoY (`CPIAUCSL`; `CP0000EZ19M086NEST` EA19 HICP; GBP/JPY/CAD/CHF `*CPIALLMINMEI`; AUD/NZD `*CPIALLQINMEI` Q→YoY → monthly after `pub_lag_months=1`) + `signal_lag=1m` + 1d weight lag. Reuses `fred_macro_diff.CPI_SERIES`. Full G10 mapped. Primary `high_cpi_xs`: long high relative CPI YoY / short low — Dahlquist–Hasseltoft (2020) economic-momentum prior (`n_long=n_short=2`). Companions: `low_cpi_xs` (PPP / high-inflation short honesty), `high_cpi_z_xs` (60m z), `cpi_chg_xs` (Δ12 of YoY), `us_cpi_stress_fx` / `us_cpi_haven_usd` (US CPI z ≤ −1.0), `cpi_ew` (EW of primary + chg + stress). PIT pub_lag=1m (matches macro_diff CPI default); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = CPI YoY %**.

**What is new vs PPI §51 / IP §42 / retail §46 / employment §41 / ULC §48 / LP §49 / CU §50 / macro_diff blend / PPP real FX:** PPI is *producer* prices; IP volume; retail sales; employment persons; ULC/LP/CU labour/capacity; macro_diff is EW CPI+IP+UR *blend*; PPP real FX is level ratio. This wave is dedicated *consumer-price inflation YoY* XS — Dahlquist economic-momentum CPI companion to PPI §51. **STALE:** JPY OECD MEI ends ~**2021-06** (hard); GBP/CAD ~2025-03, CHF ~2025-04, AUD/NZD Q ~2025-01 (mild); USD/EUR live mid-2026. `CPALTT01*M659N` same ends (not mixed); `CPHPTT01EZM659N` staler; Japan `CP0000JPM086NEST` **404**.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: full G10.
...
### Consistency windows (primary `high_cpi_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.02% | 55% | PASS | no |
| 2025 | −0.13% | 55% | PASS | no |
| 2026 | +0.15% | 75% | PASS | no |
| holdout_365d | +0.10% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_cpi_xs` scale≈**1.306** bind=**daily**; scaled HO mean≈+0.13%/mo %pos 67% — clears: **NO**. Soft-best-ish = `cpi_chg_xs` (~+7.0 bp/mo, NW t≈**+1.20**); honesty `low_cpi_xs` negative (weak sign confirm).
...
**Next structure (if promote=0):** Done as §54 (dedicated OECD/FRED unemployment-rate XS). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.
...
## 54. OECD/FRED unemployment-rate differential FX wave results (2026-09-24 ~01:00 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI / national unemployment **rate levels** (`LRHUTTTT*M156S`; CHF=`LMUNRRTTCHM156S` registered; NZD unmapped) via thin `fred_unemployment` wrapper on `fred_macro_diff.UR_SERIES` / `load_ur_panel` + `pub_lag_months=1` + `signal_lag=1m` + 1d weight lag. Primary `low_ur_xs`: long low relative UR / short high — Dahlquist–Hasseltoft (2020) labour-strength prior (`n_long=n_short=2` on available foreign panel). Companions: `high_ur_xs` (honesty reverse), `low_ur_z_xs` (60m z of level; long low z), `ur_chg_xs` (−Δ12 of level = falling UR), `us_ur_stress_fx` / `us_ur_haven_usd` (US UR z ≥ +1.0), `ur_ew` (EW of primary + chg + stress). PIT pub_lag=1m (matches macro_diff ur default); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = UR level %** (not YoY of persons). Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs employment §41 / macro_diff §12 EW blend / CPI §52 / PPI §51 / ULC/LP/CU:** Employment §41 is *persons growth* (LFEMTTTT); macro_diff uses UR only inside an EW CPI+IP+UR *blend* — **no dedicated UR XS board existed**. This wave closes that gap with a dedicated unemployment-rate level XS. EUR EA UR sparse ~2023-01; NZD unmapped on FRED; CHF registered UR.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/AUD/CAD/CHF; NZD unmapped.
...
### Consistency windows (primary `low_ur_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.10% | 45% | PASS | no |
| 2025 | −0.18% | 36% | PASS | no |
| 2026 | +0.18% | 62% | PASS | no |
| holdout_365d | +0.06% | 50% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `low_ur_xs` scale≈**2.014** bind=**static**; scaled HO mean≈+0.12%/mo %pos 50% — clears: **NO**. Soft-best-ish = `low_ur_z_xs` (~+3.8 bp/mo, NW t≈**+0.69**) / `us_ur_stress_fx` (~+2.3 bp/mo, NW t≈**+1.36**) — below soft |t|≥1.5 with positive mean. Honesty `high_ur_xs` weakly positive (weak reverse of primary).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_unemployment.md`). Config sha unchanged.

**Next structure (if promote=0):** Done as §55 (OECD/FRED manufacturing wage/earnings LCEAMN). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_unemployment_wave.md`, `scholarly_fx_unemployment_*.csv`, `scholarly_fx_unemployment_meta.json`, `quest_locked_verify_unemployment.md`.


## 55. OECD/FRED manufacturing wage/earnings (LCEAMN) FX wave results (2026-09-24 ~01:37 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI manufacturing earnings **YoY** (`LCEAMN01*`; USD/CAD `*M657S`; EUR=`LCEAMN01EZQ657S`; NZD=`*NZQ657S`; GBP/JPY `*M661S`→YoY; AUD `*AUQ661S`→YoY; CHF unmapped 404) via `fred_wage_earnings` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `low_wage_growth_xs`: long low relative wage YoY / short high — competitiveness prior (`n_long=n_short=2` on available foreign panel). Companions: `high_wage_growth_xs` (honesty reverse), `low_wage_z_xs` (60m z of YoY; long low z), `wage_chg_xs` (−Δ12 of YoY = decelerating wages), `us_wage_stress_fx` / `us_wage_haven_usd` (US wage YoY z ≥ +1.0), `wage_ew` (EW of primary + chg + stress). PIT pub_lag=2m (earnings lag UR/CPI a priori; mixed M+Q keeps 2 — not raised to 3); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = wage YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs ULC §48 / employment §41 / UR §54 / LP §49 / CU §50:** ULC is *unit* labour cost (wages/productivity); employment is *persons* growth; UR is *rate levels*; LP is output-per-worker. This wave is *manufacturing earnings / wage growth* — the LCEAMN channel documented as ULC §48 fallback "not primary", now a dedicated scholarly sleeve. EUR EA EZQ657S ends ~2025-07 (DE/FR alts); CHF LCEAMN **404** unmapped.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/AUD/CAD/NZD; CHF unmapped.
...
### Consistency windows (primary `low_wage_growth_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.11% | 36% | PASS | no |
| 2025 | +0.11% | 45% | PASS | no |
| 2026 | −0.13% | 62% | PASS | no |
| holdout_365d | −0.03% | 50% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `low_wage_growth_xs` scale≈**0.801** bind=**static**; scaled HO mean≈−0.03%/mo %pos 50% — clears: **NO**. Soft-best-ish = `high_wage_growth_xs` (~+4.6 bp/mo, NW t≈**+0.79**) / `us_wage_haven_usd` (~+4.2 bp/mo, NW t≈**+1.43**) — below soft |t|≥1.5 with positive mean. Honesty `high_wage_growth_xs` weakly positive (weak reverse of primary).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_wage_earnings.md`). Config sha unchanged.

**Next structure (if promote=0):** Done as §56 (OECD/FRED labour-force participation / activity-rate LRAC64TT). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive.

Artifacts: `reports/scholarly_fx_wage_earnings_wave.md`, `scholarly_fx_wage_earnings_*.csv`, `scholarly_fx_wage_earnings_meta.json`, `quest_locked_verify_wage_earnings.md`.


## 56. OECD/FRED labour-force participation / activity-rate (LRAC64TT) FX wave results (2026-09-24 ~02:08 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI activity / LFP rate **levels** (`LRAC64TT*`; USD/JPY/CAD/AUD `*M156S`; GBP/CHF/NZD `*Q156S`; EUR=`LRAC64TTDEQ156S` Germany proxy — EZQ156S stale ~2022-10; full G10) via `fred_lfp` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `high_lfp_xs`: long high relative LFP / short low — growth / labour-engagement prior (`n_long=n_short=2` on available foreign panel). Companions: `low_lfp_xs` (honesty reverse), `high_lfp_z_xs` (60m z of level; long high z), `lfp_chg_xs` (+Δ12 of level = rising participation), `us_lfp_stress_fx` / `us_lfp_haven_usd` (US LFP z ≤ −1.0), `lfp_ew` (EW of primary + chg + stress). PIT pub_lag=2m (participation lag a priori; mixed M+Q keeps 2 — not raised to 3); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = LFP / activity rate level %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs employment §41 / UR §54 / wages §55 / ULC §48 / LP §49 / CU §50:** Employment is *persons* growth; UR is *jobless rate levels*; wages are *earnings YoY*; ULC is *unit* labour cost. This wave is *participation / activity rate* — the extensive labour-supply margin. EUR EA EZQ156S stale ~2022-10 → Germany DEQ156S proxy; vacancies/hours alts documented not boarded.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/AUD/CAD/CHF/NZD (full G10).
...
### Consistency windows (primary `high_lfp_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.18% | 36% | PASS | no |
| 2025 | −0.07% | 36% | PASS | no |
| 2026 | −0.09% | 38% | PASS | no |
| holdout_365d | −0.17% | 33% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_lfp_xs` scale≈**0.761** bind=**static**; scaled HO mean≈−0.13%/mo %pos 33% — clears: **NO**. Soft-best-ish = `low_lfp_xs` (~+4.7 bp/mo, NW t≈**+1.01**) / `us_lfp_haven_usd` (~+1.5 bp/mo, NW t≈**+0.49**) — below soft |t|≥1.5 with positive mean. Honesty `low_lfp_xs` weakly positive (weak reverse of primary).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_lfp.md`). Config sha unchanged.

**Next structure (if promote=0):** Done as §57 (OECD/FRED employment-rate LREM64TT). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_lfp_wave.md`, `scholarly_fx_lfp_*.csv`, `scholarly_fx_lfp_meta.json`, `quest_locked_verify_lfp.md`.


## 57. OECD/FRED employment-rate (LREM64TT) FX wave results (2026-09-24 ~02:35 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI employment-rate **levels** (`LREM64TT*`; USD/JPY/CAD/AUD `*M156S`; GBP/CHF/NZD `*Q156S`; EUR=`LREM64TTDEQ156S` Germany proxy — EZQ156S stale ~2022-10; full G10) via `fred_employment_rate` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `high_er_xs`: long high relative ER / short low — labour-strength / engagement-stock prior (`n_long=n_short=2` on available foreign panel). Companions: `low_er_xs` (honesty reverse), `high_er_z_xs` (60m z of level; long high z), `er_chg_xs` (+Δ12 of level = rising ER), `us_er_stress_fx` / `us_er_haven_usd` (US ER z ≤ −1.0), `er_ew` (EW of primary + chg + stress). PIT pub_lag=2m (employment-rate lag a priori; mixed M+Q keeps 2 — not raised to 3); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = employment-rate level %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs employment §41 / UR §54 / LFP §56 / wages §55 / ULC §48 / LP §49 / CU §50:** Employment §41 is *persons* growth; UR is *jobless rate levels*; LFP is *participation / activity rate*; wages are *earnings YoY*. This wave is *employment rate* (employed/population) — the labour-engagement **stock** between participation and joblessness. EUR EA EZQ156S stale ~2022-10 → Germany DEQ156S proxy; EMRATIO/CE16OV US-only + vacancies/hours alts documented not boarded.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/AUD/CAD/CHF/NZD (full G10). Unmapped: none.
...
### Consistency windows (primary `high_er_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.04% | 27% | PASS | no |
| 2025 | −0.06% | 45% | PASS | no |
| 2026 | −0.09% | 38% | PASS | no |
| holdout_365d | −0.17% | 33% | PASS | no |
...
Sweep **run**. Primary `high_er_xs` scale≈**1.353** bind=**static**; scaled HO mean≈−0.23%/mo %pos 33% — clears: **NO**. Soft-best-ish = `us_er_stress_fx` (~+1.1 bp/mo, NW t≈**+0.78**) / `low_er_xs` (~+0.7 bp/mo, NW t≈**+0.13**) — below soft |t|≥1.5 with positive mean. Honesty `low_er_xs` near-flat (weak reverse of primary).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_employment_rate.md`). Config sha unchanged.

**Next structure (if promote=0):** Done as §58 (OECD/FRED real GDP growth NAEXKP01). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_employment_rate_wave.md`, `scholarly_fx_employment_rate_*.csv`, `scholarly_fx_employment_rate_meta.json`, `quest_locked_verify_employment_rate.md`.

## 58. OECD/FRED real GDP growth (NAEXKP01) FX wave results (2026-09-24 ~04:45 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI real GDP **YoY %** (`NAEXKP01*Q657S` QoQ % compounded to YoY; USD/GBP/JPY/CAD/AUD/CHF/NZD `*Q657S`; EUR=`NAEXKP01DEQ657S` Germany proxy — EZQ657S stale ~2023-01; full G10) via `fred_gdp` + `pub_lag_months=3` + `signal_lag=1m` + 1d weight lag. Primary `high_gdp_xs`: long high relative GDP YoY / short low — growth / economic-momentum prior (`n_long=n_short=2` on available foreign panel). Companions: `low_gdp_xs` (honesty reverse), `high_gdp_z_xs` (60m z of YoY; long high z), `gdp_chg_xs` (+Δ12 of YoY = acceleration), `us_gdp_stress_fx` / `us_gdp_haven_usd` (US GDP z ≤ −1.0), `gdp_ew` (EW of primary + chg + stress). PIT pub_lag=3m (quarterly GDP lag a priori); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = real GDP YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs IP §42 / CLI §37 / BCI §39 / emp §41 / UR §54 / debt/GDP §29 / CA/GDP §21:** IP is *industrial* coincident; CLI/BCI are *sentiment*; emp/UR are *labour*; debt/GDP and CA/GDP are *stock/flow imbalance*. This wave is *real GDP / output growth differentials* — the classic Dahlquist–Hasseltoft economic-momentum macro-FX channel. EUR EA EZQ657S stale ~2023-01 → Germany DEQ657S proxy; Q659S 404; Q652S levels partial stale/404; national GDPC1/JPNRGDPEXP alts documented not mixed.
...
### Consistency windows (primary `high_gdp_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | -0.13% | 45% | PASS | no |
| 2025 | +0.05% | 45% | PASS | no |
| 2026 | +0.03% | 62% | PASS | no |
| holdout_365d | +0.21% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**1** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_gdp_xs` scale≈**0.429** bind=**static**; scaled HO mean≈+0.09%/mo %pos 67% — clears: **NO**. Soft-best = `low_gdp_xs` (~+10.2 bp/mo, NW t≈**+1.55**) — honesty reverse clears soft |t|≥1.5 with positive mean; primary growth prior is wrong-signed (full-sample NW t ≈ −1.65).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_gdp.md`). Config sha unchanged.

**Next structure (if promote=0):** Done as §59 (OECD/FRED construction-production PRCNTO01). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_gdp_wave.md`, `scholarly_fx_gdp_*.csv`, `scholarly_fx_gdp_meta.json`, `quest_locked_verify_gdp.md`.


## 59. OECD/FRED construction-production (PRCNTO01) FX wave results (2026-09-24 ~05:00 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI construction production **YoY %** (`PRCNTO01`; USD/EUR/GBP/CAD `*GYSAM` monthly YoY; JPY=`JPNPRCNTO01GYSAQ` quarterly YoY — monthly GYSAM stale ~2020-07; AUD/NZD/CHF=`PRCNTO01*Q657S` QoQ→YoY; EUR=`DEUPRCNTO01GYSAM` Germany proxy; full G10) via `fred_construction` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `high_cons_xs`: long high relative construction YoY / short low — growth-channel / coincident construction prior (`n_long=n_short=2` on available foreign panel). Companions: `low_cons_xs` (honesty reverse), `high_cons_z_xs` (60m z of YoY; long high z), `cons_chg_xs` (+Δ12 of YoY = acceleration), `us_cons_stress_fx` / `us_cons_haven_usd` (US cons z ≤ −1.0), `cons_ew` (EW of primary + chg + stress). PIT pub_lag=2m (construction/IP-like lag a priori; mirror GDP §58 / ER §57 signal_lag=1 — *not* IP §42 zero-month signal lag); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = construction YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs building-permits §43 / IP §42 / GDP §58:** Building-permits are *leading housing flow*; IP is *industry excl. construction*; GDP is *aggregate output*. This wave is *coincident construction production / output* growth differentials — growth-channel / Taylor-rule FX on the construction sector. `CHEPRCNTO01GYSAM` 404 → CHF via Q657S QoQ→YoY; JPY monthly GYSAM stale ~2020-07 → GYSAQ; `USAPRCNTO01IXOBSAM` index stale ~2024-03 — not primary.
...
### Consistency windows (primary `high_cons_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | -0.01% | 55% | PASS | no |
| 2025 | +0.04% | 36% | PASS | no |
| 2026 | -0.14% | 25% | PASS | no |
| holdout_365d | -0.03% | 33% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run**. Primary `high_cons_xs` scale≈**0.739** bind=**static**; scaled HO mean≈-0.03%/mo %pos 33% — clears: **NO**. Soft-best-ish = `low_cons_xs` (~+5.7 bp/mo, NW t≈**+0.99**) — below soft |t|≥1.5 with positive mean. Honesty `low_cons_xs` weakly positive (primary growth prior wrong-signed / flat).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_construction.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §60 (OECD/FRED passenger-car registrations SLRTCR03 — after housing starts + mfg orders thin/404 fall-through). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_construction_wave.md`, `scholarly_fx_construction_*.csv`, `scholarly_fx_construction_meta.json`, `quest_locked_verify_construction.md`.

## 60. OECD/FRED passenger-car registrations (SLRTCR03) FX wave results (2026-09-24 ~05:30 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI passenger-car registrations **YoY %** (`SLRTCR03` GYSAM; USD/GBP/JPY/CAD/AUD live; EUR=`ESPSLRTCR03GYSAM` Spain proxy — DEU/FRA GYSAM 404, EA19 stale ~2018-12; NZD/CHF unmapped stale) via `fred_passenger_cars` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `high_cars_xs`: long high relative car-regs YoY / short low — growth-channel / durable auto-demand prior (`n_long=n_short=2` on available foreign panel). Companions: `low_cars_xs` (honesty reverse), `high_cars_z_xs` (60m z of YoY; long high z), `cars_chg_xs` (+Δ12 of YoY = acceleration), `us_cars_stress_fx` / `us_cars_haven_usd` (US cars z ≤ −1.0), `cars_ew` (EW of primary + chg + stress). PIT pub_lag=2m (auto/retail ~1–2m lag a priori; mirror GDP §58 / construction §59 signal_lag=1); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = passenger-car registrations YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**Fall-through (§60 probe order):** (1) Housing starts/completions — OECD `*ODCNPI02*` / `*ODCNPI04*` G10 **404**; US `HOUST`/`COMPUTSA` live but foreign panel <~4 → fall through (distinct from permits §43). (2) Manufacturing new orders/inventories — OECD `*ODMNNO*`/`*ODMNTO*`/`*ODMNIN*` **404**; US `AMTMNO`/`DGORDER` live only → fall through. (3) **Boarded:** passenger-car registrations SLRTCR03.

**What is new vs retail §46 / permits §43 / construction §59 / IP §42:** Retail is *broad sales volume*; permits are *leading housing flow*; construction is *coincident construction output*; IP is *industry excl. construction*. This wave is *durable consumer demand* (passenger-car registrations YoY) — growth-channel FX on auto demand. EUR = Spain proxy a priori (DEU/FRA 404).

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/AUD/CAD. Unmapped: NZD/CHF (stale).

### Full-sample factor summary

| Factor | mean_mo | t OLS | t NW | %pos | top3 | Sharpe |
|--------|--------:|------:|-----:|-----:|-----:|-------:|
| high_cars_xs | -0.138% | -2.14 | -2.18 | 45% | 12% | -0.54 |
| low_cars_xs | +0.120% | +1.87 | +1.93 | 54% | 11% | +0.47 |
| high_cars_z_xs | -0.091% | -1.39 | -1.43 | 47% | 12% | -0.33 |
| cars_chg_xs | -0.090% | -1.46 | -1.54 | 45% | 12% | -0.36 |
| us_cars_stress_fx | +0.012% | +0.38 | +0.38 | 14% | 34% | +0.09 |
| us_cars_haven_usd | -0.013% | -0.44 | -0.44 | 14% | 37% | -0.11 |
| cars_ew | -0.072% | -2.12 | -2.19 | 44% | 12% | -0.50 |

### Consistency windows (primary `high_cars_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.01% | 55% | PASS | no |
| 2025 | +0.18% | 64% | PASS | no |
| 2026 | -0.11% | 50% | PASS | no |
| holdout_365d | +0.02% | 58% | PASS | no |

**Board:** n=7 soft_nw_pos=**1** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `high_cars_xs` scale≈**0.351** bind=**static**; scaled HO mean≈+0.01%/mo %pos 58% — clears: **NO**. Soft-best = `low_cars_xs` (~+12.0 bp/mo, NW t≈**+1.93**) — honesty reverse clears soft |t|≥1.5 with positive mean (primary growth prior wrong-signed).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_passenger_cars.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §61 (OECD/FRED merchandise export-value XTEXVA01 — after CIP/basis Missing + hours/vacancies thin + PRSVTT*/ODMN* 404 fall-through). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_passenger_cars_wave.md`, `scholarly_fx_passenger_cars_*.csv`, `scholarly_fx_passenger_cars_meta.json`, `quest_locked_verify_passenger_cars.md`.

## 61. OECD/FRED merchandise export-value YoY (XTEXVA01) FX wave results (2026-09-24 ~05:59 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI merchandise export-value **YoY %** (`XTEXVA01{ISO2}M657S`; full G10 live; EUR=`XTEXVA01DEM657S` Germany proxy — EZM657S 404, EZM667S stale ~2023-04; unit=**VALUE** not volume; `*M667S` levels not mixed) via `fred_export_value` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `high_xval_xs`: long high relative export YoY / short low — growth-channel / external-demand prior (`n_long=n_short=2` on full G10 foreign panel). Companions: `low_xval_xs` (honesty reverse), `high_xval_z_xs` (60m z of YoY; long high z), `xval_chg_xs` (+Δ12 of YoY = acceleration), `us_xval_stress_fx` / `us_xval_haven_usd` (US xval z ≤ −1.0), `xval_ew` (EW of primary + chg + stress). PIT pub_lag=2m (trade ~1–2m lag a priori; mirror GDP §58 / construction §59 / cars §60 signal_lag=1); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = merchandise export-value YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**Fall-through (§61 probe order):** (1) CIP / cross-currency basis — still **Missing** free G10 panel (Du–Tepper–Verdelhan); rates+spot CIP-implied FD already §19 — do not fake basis → fall through. (2) Hours `HOHWMN*` — US-only / 404 abroad; vacancies `LMJVTTUV*` — partial + stale ~2023-24, JP/CA/NZ thin → fall through. (3) Services production `PRSVTT*` / mfg turnover `ODMN*` — **404**. (4) **Boarded:** merchandise export-value YoY `XTEXVA01*M657S`.

**What is new vs TB §30 / CA §21 / retail §46 / IP §42 / cars §60 / construction §59 / GDP §58:** Trade-balance is *TB/exports ratio*; CA is *current-account/GDP*; retail is *broad sales volume*; IP is *industry*; cars is *durable consumer demand*; construction is *construction output*; GDP is *aggregate real growth*. This wave is *external demand / export-growth differentials* (trade intensity) — growth-channel FX on merchandise export **value** YoY. EUR = Germany proxy a priori (EZM657S 404).

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: full G10 (USD/EUR/GBP/JPY/CAD/AUD/CHF/NZD).
...
### Consistency windows (primary `high_xval_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.55% | 64% | PASS | no |
| 2025 | -0.06% | 45% | PASS | no |
| 2026 | +0.12% | 50% | PASS | no |
| holdout_365d | +0.08% | 58% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `high_xval_xs` scale≈**0.492** bind=**static**; scaled HO mean≈+0.04%/mo %pos 58% — clears: **NO**. Soft-best-ish = `us_xval_haven_usd` (~+2.5 bp/mo, NW t≈**+1.22**) — below soft |t|≥1.5 with positive mean. Honesty `low_xval_xs` near-flat (~+0.8 bp/mo).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_export_value.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §62 (OECD/FRED merchandise import-value XTIMVA01 — domestic absorption / import demand; mirrors export §61). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_export_value_wave.md`, `scholarly_fx_export_value_*.csv`, `scholarly_fx_export_value_meta.json`, `quest_locked_verify_export_value.md`.


## 62. OECD/FRED merchandise import-value YoY (XTIMVA01) FX wave results (2026-09-24 ~06:23 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI merchandise import-value **YoY %** (`XTIMVA01{ISO2}M657S`; full G10 live; EUR=`XTIMVA01DEM657S` Germany proxy — EZM657S 404, EZM667S stale ~2023-04; unit=**VALUE** not volume; `*M667S` levels not mixed) via `fred_import_value` + `pub_lag_months=2` + `signal_lag=1m` + 1d weight lag. Primary `high_ival_xs`: long high relative import YoY / short low — growth-channel / domestic-absorption / import-demand prior (`n_long=n_short=2` on full G10 foreign panel). Companions: `low_ival_xs` (honesty reverse), `high_ival_z_xs` (60m z of YoY; long high z), `ival_chg_xs` (+Δ12 of YoY = acceleration), `us_ival_stress_fx` / `us_ival_haven_usd` (US ival z ≤ −1.0), `ival_ew` (EW of primary + chg + stress). PIT pub_lag=2m (trade ~1–2m lag a priori; mirror export §61 / GDP §58 / cars §60 signal_lag=1); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = merchandise import-value YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**Fall-through (§62 probe order):** (1) CIP / cross-currency basis — still **Missing** free G10 panel (Du–Tepper–Verdelhan); rates+spot CIP-implied FD already §19 — do not fake basis → fall through. (2) Hours `HOHWMN*` — US-only / 404 abroad; vacancies `LMJVTTUV*` — partial + stale ~2023-24, JP/CA/NZ thin → fall through. (3) Services production `PRSVTT*` / mfg turnover `ODMN*` — **404**. (4) **Boarded:** merchandise import-value YoY `XTIMVA01*M657S` (mirrors export-value §61).

**What is new vs export §61 / TB §30 / CA §21 / retail §46 / IP §42 / cars §60 / construction §59 / GDP §58:** Export-value is *external demand*; trade-balance is *TB/exports ratio*; CA is *current-account/GDP*; retail is *broad sales volume*; IP is *industry*; cars is *durable consumer demand*; construction is *construction output*; GDP is *aggregate real growth*. This wave is *domestic absorption / import-demand differentials* (trade intensity) — growth-channel FX on merchandise import **value** YoY. EUR = Germany proxy a priori (EZM657S 404).
...
### Consistency windows (primary `high_ival_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.03% | 64% | PASS | no |
| 2025 | +0.10% | 64% | PASS | no |
| 2026 | -0.02% | 62% | PASS | no |
| holdout_365d | +0.03% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `high_ival_xs` scale≈**0.371** bind=**static**; scaled HO mean≈+0.01%/mo %pos 67% — clears: **NO**. Soft-best-ish = `low_ival_xs` (~+7.3 bp/mo, NW t≈**+1.06**) — honesty reverse; below soft |t|≥1.5 with positive mean. Primary growth prior wrong-signed (negative full-sample).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_import_value.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** FX IV/RR + news-sentiment still **blocked**. Free scholarly queue **thinning** (UR + LCEAMN wage + LFP/activity + employment-rate + real-GDP-growth + construction-production + passenger-car-regs + merchandise-export-value + merchandise-import-value + household-credit gaps closed; CIP/basis still Missing free G10; hours/vacancies thin/stale; PRSVTT*/ODMN* 404; credit-impulse already §32 companion — skip). Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or free CIP/cross-currency basis if a multi-year public panel appears, or another free scholarly macro/GPR gap. No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_import_value_wave.md`, `scholarly_fx_import_value_*.csv`, `scholarly_fx_import_value_meta.json`, `quest_locked_verify_import_value.md`.


## 63. BIS/FRED household-credit YoY (CRDQ*AHABIS) FX wave results (2026-09-24 ~06:59 BST)

**Design (fixed priors, no HO tuning):** FRED BIS household credit absolute → **YoY %** (`CRDQ{ISO2}AHABIS`; EUR=`CRDQXMAHABIS` euro-area — CRDQEAAHABIS 404; NZD CRDQNZAHABIS **404** unmapped; unit=abs→YoY **not** %GDP) via `fred_household_credit` + `pub_lag_months=5` + `signal_lag=1m` + 1d weight lag. Primary `high_hhcred_xs`: long high relative HH-credit YoY / short low — consumer-demand / credit-financed-absorption prior (`n_long=n_short=2` on available foreign panel). Companions: `low_hhcred_xs` (honesty reverse), `high_hhcred_z_xs` (60m z of YoY), `hhcred_chg_xs` (+Δ12 of YoY), `us_hhcred_stress_fx` / `us_hhcred_haven_usd` (US hhcred z ≤ −1.0), `hhcred_ew`. PIT pub_lag=5m (same band as credit §32 / DSR §47); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = household-credit YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**Fall-through (§63 probe order):** (1) Merchandise trade **VOLUME** YoY — `XTINTVA*` / `XTEXVO*` / `XTIMVO*` **404**; OECD MEI XTEXVA/XTIMVA are VALUE (already §61/§62) → fall through. (2) Household saving rate — `NASAVP*` / `HHSBRG*` **404**; only US `PSAVERT` live → cannot map ≥4 G10 → fall through. (3) **Boarded:** BIS household credit `CRDQ*AHABIS` → YoY (distinct retail/consumer credit vs PNFS credit/GDP §32). (4–6) BOP services thin/US-heavy; inventories `STMNTO*` **404** (BUSINV US-only); order books `BSOITE02*` only US/DE/CH (<4 G10); `BSCICP03*` amplitude stale (BCI §39).

**What is new vs credit §32 / money §33 / DSR §47 / debt §29 / HPI §35 / retail §46 / cars §60 / import §62:** Private credit/GDP is *PNFS stock %GDP + gap*; money is *broad M growth*; DSR is *debt-service ratio*; debt is *gov debt/GDP*; HPI is *housing wealth*; retail/cars are *sales/registrations*; import-value is *merchandise trade value*. This wave is *household / consumer credit growth* (abs→YoY) — retail-demand channel on the credit side.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/CAD/AUD/CHF (NZD unmapped).
...
### Consistency windows (primary `high_hhcred_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | -0.09% | 55% | PASS | no |
| 2025 | -0.22% | 36% | PASS | no |
| 2026 | +0.28% | 75% | PASS | no |
| holdout_365d | +0.35% | 83% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `high_hhcred_xs` scale≈**0.586** bind=**static**; scaled HO mean≈+0.21%/mo %pos 83% — clears: **NO**. Soft-best-ish = `high_hhcred_z_xs` (~+6.7 bp/mo, NW t≈**+1.00**) — below soft |t|≥1.5 with positive mean. Honesty `low_hhcred_xs` ~+5.1 bp/mo NW t≈+0.77.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_household_credit.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** FX IV/RR + news-sentiment still **blocked**. Free scholarly queue **thinning** (UR + LCEAMN wage + LFP/activity + employment-rate + real-GDP + construction + passenger-cars + export-value + import-value + household-credit gaps closed; trade VOLUME 404; saving-rate 404 US-only; inventories 404; order books <4 G10; CIP/basis still Missing; credit-impulse already §32 companion — skip). Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or Dahlquist-style equal-weight soft-signal recombination scholarly stack if free single-series queue exhausted, or free CIP/cross-currency basis if a multi-year public panel appears. No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_household_credit_wave.md`, `scholarly_fx_household_credit_*.csv`, `scholarly_fx_household_credit_meta.json`, `quest_locked_verify_household_credit.md`.

## 64. OECD/FRED real GFCF / investment YoY (NAEXKP04) FX wave results (2026-09-24 ~07:26 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI real GFCF → **YoY %** (`NAEXKP04{ISO2}Q657S` QoQ→compounded YoY; EUR=`NAEXKP04DEQ657S` Germany proxy — EZQ657S stale ~2023-01; full G10 incl NZD/CHF; Q659S live≡compound Q657S — not mixed; same unit family as GDP NAEXKP01) via `fred_gfcf` + `pub_lag_months=3` + `signal_lag=1m` + 1d weight lag. Primary `high_gfcf_xs`: long high relative GFCF YoY / short low — investment boom / economic-momentum prior (`n_long=n_short=2` on available foreign panel). Companions: `low_gfcf_xs` (honesty reverse / capex stress), `high_gfcf_z_xs` (60m z of YoY), `gfcf_chg_xs` (+Δ12 of YoY), `us_gfcf_stress_fx` / `us_gfcf_haven_usd` (US GFCF z ≤ −1.0), `gfcf_ew`. PIT pub_lag=3m (same band as GDP §58); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = real GFCF YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**Fall-through (§64 probe order):** (1) PNFS absolute `CRDQ*APABIS` — too close to private credit-impulse companion of §32 (keepalive said skip). (2) NFC `ANFBIS` — **404**. (3) **Boarded:** OECD MEI GFCF `NAEXKP04*Q657S` QoQ→YoY (investment / capex channel; Dahlquist–Hasseltoft applied to GFCF not aggregate GDP).

**What is new vs GDP §58 / construction §59 / IP §42 / BCI §39 / building permits §43 / HPI §35 / CLI §37 / retail §46 / cars §60 / export §61 / import §62 / household credit §63 / private credit/GDP §32:** GDP is *aggregate output*; construction is *production index*; IP is *industrial output*; BCI is *business confidence*; permits/HPI are *housing leading/wealth*; CLI is *composite leading*; retail/cars are *consumer demand*; export/import are *trade value*; household credit is *consumer credit growth*; private credit/GDP is *PNFS stock %GDP*. This wave is *real gross fixed capital formation / investment growth* (QoQ→YoY).

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/CAD/AUD/CHF/NZD (full G10).
...
### Consistency windows (primary `high_gfcf_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | -0.23% | 45% | PASS | no |
| 2025 | +0.10% | 55% | PASS | no |
| 2026 | +0.03% | 62% | PASS | no |
| holdout_365d | +0.10% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `high_gfcf_xs` scale≈**1.779** bind=**static**; scaled HO mean≈+0.18%/mo %pos 67% — clears: **NO**. Soft-best-ish = `gfcf_chg_xs` (~+4.7 bp/mo, NW t≈**+0.85**) — below soft |t|≥1.5 with positive mean (also `gfcf_ew` ~+2.7 bp/mo NW t≈+1.02). Honesty `low_gfcf_xs` negative (no reverse edge).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_gfcf.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** FX IV/RR + news-sentiment still **blocked**. Free scholarly queue **thinning** (UR + LCEAMN wage + LFP/activity + employment-rate + real-GDP + construction + passenger-cars + export-value + import-value + household-credit + GFCF gaps closed; PNFS abs skip §32 companion; NFC ANFBIS 404; CIP/basis still Missing). Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or Dahlquist-style equal-weight soft-signal recombination scholarly stack if free single-series queue exhausted, or free CIP/cross-currency basis if a multi-year public panel appears. No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_gfcf_wave.md`, `scholarly_fx_gfcf_*.csv`, `scholarly_fx_gfcf_meta.json`, `quest_locked_verify_gfcf.md`.

## 65. OECD/FRED real private final consumption YoY (NAEXKP02) FX wave results (2026-09-24 ~08:07 BST)

**Design (fixed priors, no HO tuning):** FRED OECD MEI real **private final consumption** → **YoY %** (`NAEXKP02{ISO2}Q657S` QoQ→compounded YoY; EUR=`NAEXKP02DEQ657S` Germany proxy — EZQ657S stale ~2023-01; full G10 incl NZD/CHF; Q659S live≡compound Q657S — not mixed; same unit family as GDP NAEXKP01 / GFCF NAEXKP04; **not** US BEA PCE) via `fred_pce` + `pub_lag_months=3` + `signal_lag=1m` + 1d weight lag. Primary `high_pce_xs`: long high relative private-consumption YoY / short low — Lustig–Verdelhan (2007) consumption-growth risk + Dahlquist–Hasseltoft (2020) economic-momentum applied to private consumption (`n_long=n_short=2` on available foreign panel). Companions: `low_pce_xs` (honesty reverse / consumption stress), `high_pce_z_xs` (60m z of YoY), `pce_chg_xs` (+Δ12 of YoY), `us_pce_stress_fx` / `us_pce_haven_usd` (US PCE z ≤ −1.0), `pce_ew`. PIT pub_lag=3m (same band as GDP §58 / GFCF §64); z_window=60, min_periods=24, usd_tilt=0.5; costs 1.5 bps/side. **Score basis = real private final consumption YoY %**. Explicit: do **not** overlay on locked fx4plus. Prop sizing: utilise nearly full FTMO DD via risk sweep when IS mean>0; stay under 10% static / 5% daily.

**What is new vs GDP §58 / GFCF §64 / retail §46 / cars §60 / household credit §63 / construction §59 / IP §42 / CLI/CCI/BCI / emp/labour §41/54–57:** GDP is *aggregate output*; GFCF is *investment / capex*; retail/cars are *consumer volume / durable demand*; household credit is *consumer credit growth*; construction/IP are *production*; CLI/CCI/BCI are *survey composites*; emp/labour are *labour market stocks/flows*. This wave is *real private final consumption growth* (QoQ→YoY) — consumption-growth risk channel.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Mapped: USD/EUR/GBP/JPY/CAD/AUD/CHF/NZD (full G10). EUR EA EZQ657S stale ~2023-01 → DEQ Germany proxy.
...
### Consistency windows (primary `high_pce_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | -0.03% | 55% | PASS | no |
| 2025 | +0.23% | 73% | PASS | no |
| 2026 | +0.23% | 62% | PASS | no |
| holdout_365d | +0.33% | 75% | PASS | no |

**Board:** n=7 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `high_pce_xs` scale≈**0.710** bind=**static**; scaled HO mean≈+0.23%/mo %pos 75% — clears: **NO**. Soft-best-ish = `high_pce_z_xs` (~+4.4 bp/mo, NW t≈**+0.85**) — below soft |t|≥1.5 with positive mean (also `us_pce_stress_fx` ~+1.8 bp/mo NW t≈+0.94). Honesty `low_pce_xs` weakly positive (~+1.9 bp/mo) — no strong reverse edge.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_pce.md`). Config sha unchanged (`bafed8eff5aa7541…`).
...
## 67. Du–Schreger government-bond CIP / U.S. Treasury premium FX wave results (2026-09-24 ~09:06 BST)

**Design (fixed priors, no HO tuning):** Du, Keerati & Schreger (2025) public `cip_dataset_v4` government-bond CIP (`cip_govt` bps) + Du–Im–Schreger U.S. Treasury Premium (−cross-sectional mean). Literature also Du, Tepper & Verdelhan (2018) JF CIP / intermediary constraints.

**Tenor prior (a priori):** **5y** — Du–Im–Schreger Treasury-premium medium tenor; liquid CCS (≥1y); full multi-year G10 trade coverage. **Not** HO-tuned vs 1y/2y/10y.

**Sign prior (primary `low_cip_xs`):** long currencies with **low / more-negative** `cip_govt` / short high. Rationale: high `cip_govt` embeds a more-negative CCS basis (dollar funding stress) and a convenience discount vs UST → FX underperforms; low CIP = stronger relative foreign convenience / less funding stress. Honesty reverse `high_cip_xs`. Companions: `low_cip_z_xs`, `cip_chg_xs` (−Δ12), `ust_premium_haven_usd` / `ust_premium_stress_fx`, `cip_ew`.

**PIT:** `pub_lag_days=1` on daily → month-end panel + `signal_lag=1` month + 1 trading-day weight lag. `n_long=n_short=2`; costs 1.5 bps/side. G10 trade: EUR/GBP/JPY/AUD/CAD/CHF/NZD (EUR=Germany in dataset).

**Distinct from:** funding_liq §20 (NFCI/TED/CPFF), fwd_carry §19 (rate-implied CIP FD approximation — **not** observed basis), IG OAS §36, CB-BS §22. Explicit: do **not** overlay on locked fx4plus.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). Panel ~2000-01 → **2025-07** (raw vintage ends ~2025-06-30; 2026 FX window uses stale/ffilled CIP — documented caveat). Slim cache `data/macro/cip_g10_5y_govt_panel.csv`.
...
### Consistency windows (primary `low_cip_xs`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.12% | 64% | PASS | no |
| 2025 | +0.44% | 82% | PASS | no |
| 2026 | +0.14% | 62% | PASS | no |
| holdout_365d | +0.24% | 75% | PASS | no |

**Board:** n=7 soft_nw_pos=**2** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `low_cip_xs` scale≈**0.712** bind=**static**; scaled HO mean≈+0.17%/mo %pos 75% — clears: **NO**. Soft-best = `ust_premium_haven_usd` (~+7.6 bp/mo, NW t≈**+1.94**) — soft |t|≥1.5; also `cip_ew` ~+6.1 bp/mo NW t≈+1.68. Honesty `high_cip_xs` negative (sign prior not reversed).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_cip_basis.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §68 (CIP-conditioned Lustig–Verdelhan carry). FX IV/RR + news-sentiment still **blocked**. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or richer soft-stack / capital-sleeve refinements without overlaying coolers on locked fx4plus. No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_cip_basis_wave.md`, `scholarly_fx_cip_basis_*.csv`, `scholarly_fx_cip_basis_meta.json`, `quest_locked_verify_cip_basis.md`, `data/macro/cip_g10_5y_govt_panel.csv`.

## 68. CIP-conditioned Lustig–Verdelhan carry wave results (2026-09-24 ~09:33 BST)

**Design (fixed priors, no HO tuning):** Multi-factor structure — Lustig–Roussanov–Verdelhan (2011) IR3M / policy-rate carry **gated / cooled** by Du–Schreger aggregate CIP stress. Literature also Du, Tepper & Verdelhan (2018) JF (CIP / intermediary constraints → carry crashes) + Du, Keerati & Schreger (2025) `cip_govt` / U.S. Treasury Premium.

**CIP stress (a priori):** `cip_stress ≡ UST premium = −mean(G10 cip_govt)` at tenor **5y** (same §67 DEFAULT_TENOR — not HO-tuned). Elevated stress = more-negative mean CIP / high Treasury premium. Gate: trade carry only when lagged stress z ≤ 0; cool: risk_scale ∈ [0.35, 1] when z ≥ 1 (mirror funding_liq / ACM fixed z — **not** HO-tuned).

**Primary `carry_low_cip_stress`:** scholarly cash-rate carry × binary low-stress gate. Companions: `carry_cip_cool`, honesty `carry_raw` + `carry_high_cip_stress` (inverse), `cip_stress_haven_usd`, `cip_carry_ew`.

**PIT:** CIP `pub_lag_days=1` → month-end + `signal_lag_months=1` + 1d weight lag; rates FRED pub_lag_months=1 + carry `signal_lag=1`d. `n_long=n_short=2`; costs 1.5 bps/side. G10 USD majors.

**Distinct from:** fwd_carry §19 (rate-implied CIP FD — **not** observed basis), funding_liq §20 (NFCI/TED/CPFF), IG OAS §36, ACM TP §44, CIP XS §67 (`low_cip_xs` — pure cross-section, not carry×CIP), combo §8, capital-sleeve §53 / soft EW §66. Explicit: do **not** overlay on locked fx4plus.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). CIP panel ~2000-01 → **2025-07** (raw vintage ends ~2025-06-30; 2026 FX stale/ffill caveat).
...
### Consistency windows (primary `carry_low_cip_stress`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.15% | 27% | PASS | no |
| 2025 | +0.12% | 64% | PASS | no |
| 2026 | +0.13% | 62% | PASS | no |
| holdout_365d | +0.31% | 75% | PASS | no |

**Board:** n=6 soft_nw_pos=**2** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `carry_low_cip_stress` scale≈**2.602** bind=**daily**; scaled HO mean≈+0.80%/mo %pos 75% — clears: **NO**. Soft-best = `cip_stress_haven_usd` (~+7.3 bp/mo, NW t≈**+1.91**); also `cip_carry_ew` ~+5.7 bp/mo NW t≈**+1.89**. Honesty `carry_high_cip_stress` negative / `carry_raw` ~flat — low-stress gate direction not reversed. Primary %pos low (~23%) because gate is often flat (off) in stress regimes — expected for binary sit-out.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_cip_conditioned_carry.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** CIP×carry multi-factor boarded (§68). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP-conditioned carry queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or richer soft-stack membership / capital-sleeve refinements without overlaying coolers on locked fx4plus. No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_cip_conditioned_carry_wave.md`, `scholarly_fx_cip_conditioned_carry_*.csv`, `scholarly_fx_cip_conditioned_carry_meta.json`, `quest_locked_verify_cip_conditioned_carry.md`.

## 69. Soft-stack CIP enrichment wave results (2026-09-24 ~10:03 BST)

**Design (fixed priors, no HO tuning):** Sibling of §66 soft EW — richer soft-signal membership adding the two CIP soft legs that cleared soft |NW t|≥1.5 with positive full-sample mean on the §67 board. Literature: Dahlquist–Hasseltoft (2020) economic-momentum EW recombination + Du–Keerati–Schreger / Du–Tepper–Verdelhan CIP / Treasury-premium channel.

**SOFT_LEGS_CIP (a priori):** `bci_chg_xs` (§39), `high_ip_xs` (§42), `high_ppi_xs` (§51), `low_gdp_xs` (§58), `low_cars_xs` (§60), `ust_premium_haven_usd` (§67 soft-best — ONE haven seed; not `cip_stress_haven_usd`), `cip_ew` (§67 soft — ONE CIP XS EW seed; not `cip_carry_ew`).

**Primary `soft_ew7_cip`:** EW of all 7 SOFT_LEGS_CIP. Companions: `soft_ew_macro5` (§66 baseline), `soft_ew_cip2`, leave-one-out `soft_ew6_no_haven` / `soft_ew6_no_cip_ew`, `soft_ew_growth_cip`, `soft_ew_honesty_cip`.

**PIT:** same source-wave lags as §39/42/51/58/60 + CIP `pub_lag_days=1` / `signal_lag=1`m + 1d weight lag. Row-wise nanmean; min_legs=2. Costs 1.5 bps/side inside source factors. Does **not** mutate §66 `SOFT_LEGS`.

**Distinct from:** soft_ew §66 (macro5 only), CIP XS §67, CIP-conditioned carry §68, capital-sleeve §53, combo §8. Explicit: do **not** overlay on locked fx4plus.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). CIP panel ~2000-01 → **2025-07** (raw vintage ends ~2025-06-30; 2026 FX window uses stale/ffilled CIP — documented caveat).
...
### Consistency windows (primary `soft_ew7_cip`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.05% | 45% | PASS | no |
| 2025 | +0.05% | 73% | PASS | no |
| 2026 | +0.11% | 88% | PASS | no |
| holdout_365d | +0.05% | 67% | PASS | no |

**Board:** n=7 soft_nw_pos=**7** hard_nw_pos=**7** promote=**0**.

Sweep **run** (any IS mean>0 on board). Primary `soft_ew7_cip` scale≈**6.523** bind=**daily**; scaled HO mean≈+0.34%/mo %pos 67% — clears: **NO**. Soft-best = `soft_ew_macro5` (~+11.8 bp/mo, NW t≈**+3.63**) — macro5 alone still edges primary on mean; CIP enrichment lifts NW t on primary (+3.94 vs §66 soft_ew5 +3.63) but does not clear 1%/mo bar. All seven board factors hard |t|≥2 with positive mean — strong soft-stack confirmation; still far below 1%/mo consistency.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 + PORT_VOL_TARGET=0.0025: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_soft_signal_cip_stack.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §70 (capital-sleeve soft-stack: locked core + soft_ew_macro5 / soft_ew7_cip). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or further multi-factor recombination without overlaying coolers on locked fx4plus. No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_soft_signal_cip_stack_wave.md`, `scholarly_fx_soft_signal_cip_stack_*.csv`, `scholarly_fx_soft_signal_cip_stack_meta.json`, `quest_locked_verify_soft_signal_cip_stack.md`.


## 70. Capital-sleeve soft-stack enrichment wave results (2026-09-24 ~10:36 BST)

**Design (fixed priors, no HO tuning):** Keep locked `fx4plus_gbpcad_d1_voltarget_0025` as **core** capital sleeve (RF=0.08, VT=0.0025 — same as §53). Allocate smaller fixed capital shares to pre-registered soft-stack satellites that already cleared soft |NW t|≥1.5 with positive full-sample mean — **separate capital sleeves**, NOT overlays/coolers on locked equity. Satellite A = `soft_ew_macro5` (§66 / §69 ~+11.8 bp/mo NW t≈+3.63 — Dahlquist soft EW of SOFT_LEGS macro5). Satellite B = `soft_ew7_cip` (§69 ~+10.4 bp/mo NW t≈+3.94 — CIP-enriched soft stack). Mix grid (n=6, capital shares sum to 1): `core_100`, `core85_soft15`, `core70_soft30`, `core60_soft40`, `core80_soft10_cip10`, `core70_soft20_cip10`. Primary = **IS-only** argmax mean_mo (never HO). Combined series = Σ w_i r_i; `sweep_scale_to_ftmo_budget` on combined when IS mean>0. Soft satellites rebuilt via `soft_signal_cip_stack_fx` + source-wave PIT lags (costs 1.5 bps/side inside source factors). Does **not** mutate §53 `PRE_REGISTERED_MIXES` / locked config.

**What is new vs capital-sleeve §53 / soft_ew §66 / soft CIP §69 / CIP XS §67 / CIP×carry §68 / combo §8:** §53 used single-series BCI+PPI satellites. This wave mixes the locked FTMO basket with *recombined soft stacks* (macro5 / CIP7) as capital sleeves. Soft EW modules remain factor boards — this is capital allocation, not overlay coolers.
...
| core60_soft40 | +1.037% | +2.95 | +2.19 | 72% | 38% | +1.48 |
...
Sweep **run**. Primary `core_100` scale≈**1.341** bind=**daily**; scaled HO mean≈+2.59%/mo %pos 67% — clears: **NO** (%pos<70%). Soft/hard-best = `core_100` itself (IS + full-sample mean); soft-stack satellites **dilute** locked mean_mo on every pre-registered mix (IS: core_100 +1.52% > core85_soft15 +1.29% > … > core60_soft40 +0.92%) — same qualitative honesty as §53 BCI/PPI diversion. Soft stacks add NW t slightly on diluted mixes but do not lift the 1%/mo consistency bar vs core-only under FTMO gates.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 windowed verify: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_capital_sleeve_soft.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §71 (CIP-stress-conditioned soft-signal EW: soft_ew_macro5 × CIP gate/cool). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve + CIP-conditioned soft queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_capital_sleeve_soft_wave.md`, `scholarly_fx_capital_sleeve_soft_*.csv`, `scholarly_fx_capital_sleeve_soft_meta.json`, `quest_locked_verify_capital_sleeve_soft.md`.


## 71. CIP-stress-conditioned soft-signal EW FX wave results (2026-09-24 ~11:03 BST)

**Design (fixed priors, no HO tuning):** Trade §66 Dahlquist–Hasseltoft `soft_ew_macro5` (EW of SOFT_LEGS = bci_chg_xs / high_ip_xs / high_ppi_xs / low_gdp_xs / low_cars_xs) **only when** aggregate Du–Schreger CIP / U.S. Treasury Premium stress is low. `cip_stress ≡ UST premium = −mean(G10 cip_govt)` at tenor **5y** (DEFAULT_TENOR — same §67/§68 a priori). Gate: soft on when lagged stress z ≤ 0; cool: risk_scale ∈ [0.35, 1] when z ≥ 1 (mirror funding_liq / ACM / §68 — not HO-tuned). PIT: CIP `pub_lag_days=1` → month-end + `signal_lag_months=1` + 1d weight lag; soft legs keep source-wave PIT from §66. Costs 1.5 bps/side inside source factors. G10 USD majors. Primary = `soft_low_cip_stress`. Companions (pre-registered): `soft_cip_cool`, `soft_raw` (ungated honesty), `soft_high_cip_stress` (inverse), `cip_stress_haven_usd` (§68-style), `soft_cip_ew` (EW of low ⊕ cool ⊕ haven). Prop sizing via `sweep_scale_to_ftmo_budget` when IS mean>0.

**What is new vs soft_ew §66 / soft CIP §69 / CIP×carry §68 / CIP XS §67 / capital-sleeve §53/§70 / combo §8:** §66 is ungated soft EW. §69 *adds CIP soft legs into EW membership*. §68 gates Lustig–Verdelhan *carry*, not soft macro EW. This wave **gates/cools the soft macro EW itself** with the same CIP-stress definition as §68 — signal gate, not capital mix, not EW-membership enrichment. Explicit: do **not** overlay coolers on locked `fx4plus_gbpcad_d1_voltarget_0025`.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). CIP panel ~2000-01..**2025-07** (raw ends ~2025-06-30; 2026 FX stale/ffill caveat).

### Full-sample factor summary

| Factor | mean_mo | t OLS | t NW | %pos | top3 | Sharpe |
|--------|--------:|------:|-----:|-----:|-----:|-------:|
| **soft_low_cip_stress** (primary) | **+0.040%** | +2.66 | **+1.98** | **21%** | 30% | +0.67 |
...
### Consistency windows (primary `soft_low_cip_stress`)

| Window | mean_mo | %pos | top3 | gates | 1% bar |
|--------|--------:|-----:|-----:|:-----:|:------:|
| 2024 | -0.04% | 18% | 100% | PASS | no |
| 2025 | +0.04% | 64% | 69% | PASS | no |
| 2026 | +0.14% | 88% | 80% | PASS | no |
| holdout_365d | +0.06% | 67% | 75% | PASS | no |

**Board:** n=6 soft_nw_pos=**5** hard_nw_pos=**3** promote=**0**.

Sweep **run**. Primary `soft_low_cip_stress` scale≈**11.479** bind=**daily**; scaled HO mean≈+0.71%/mo %pos 67% — clears: **NO**. Soft-best = `soft_raw` (~+11.8 bp/mo, NW t≈**+3.63**) — ungated §66 soft_ew_macro5 still dominates; CIP low-stress gate **cuts** mean and %pos (sits out often → 21% positive months) while retaining soft |t|≈1.98. Cool companion (+8.3 bp/mo NW t +3.36) is between raw and gated. Honesty inverse `soft_high_cip_stress` weak (+1.9 bp/mo, soft miss). 2025/2026/HO primary ≡ soft_raw (stale CIP → gate stays on); 2024 gate actually hurts. Still far below 1%/mo consistency.

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 windowed verify: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_cip_conditioned_soft.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §72 (VIX/GPR-conditioned soft-signal EW: soft_ew_macro5 × VIX/GPR gate/cool). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve + CIP-conditioned soft + VIX/GPR-conditioned soft queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_cip_conditioned_soft_wave.md`, `scholarly_fx_cip_conditioned_soft_*.csv`, `scholarly_fx_cip_conditioned_soft_meta.json`, `quest_locked_verify_cip_conditioned_soft.md`.


## 72. VIX/GPR-conditioned soft-signal EW FX wave results (2026-09-24 ~11:35 BST)

**Design (fixed priors, no HO tuning):** Trade §66 Dahlquist–Hasseltoft `soft_ew_macro5` (EW of SOFT_LEGS = bci_chg_xs / high_ip_xs / high_ppi_xs / low_gdp_xs / low_cars_xs) **only when** global FX-vol / geopolitical stress is low. Stress A = **VIX** z (Menkhoff et al. 2012 JF global FX-vol proxy via `vix_yahoo.csv`). Stress B = aggregate **GPR** z (Caldara–Iacoviello 2022 / Liu–Zhang 2024 via `gpr_daily.csv`). Gate: soft on when lagged stress z ≤ 0; cool: risk_scale ∈ [0.35, 1] when z ≥ 1 (same as §71 — literature often uses **1.5** for pure GPR risk-off; for this wave keep z≥1 for §71 symmetry, documented, not HO-tuned). PIT: VIX/GPR `bar_lag=1` + `signal_lag=1`d on trailing z; soft legs keep source-wave PIT from §66. Costs 1.5 bps/side inside source factors. G10 USD majors. Primary = `soft_low_vix` (Menkhoff). Companions (pre-registered): `soft_vix_cool`, `soft_low_gpr`, `soft_gpr_cool`, `soft_raw` (ungated honesty), `soft_high_vix` (inverse), `soft_vix_gpr_stack` (sequential VIX×GPR cool product), `soft_vix_gpr_ew` (EW of low_vix ⊕ vix_cool ⊕ low_gpr ⊕ gpr_cool). Prop sizing via `sweep_scale_to_ftmo_budget` when IS mean>0.

**What is new vs soft_ew §66 / soft CIP §69 / CIP×carry §68 / CIP XS §67 / CIP-conditioned soft §71 / capital-sleeve §53/§70 / funding_liq §20 / combo §8 / gpr_regime / AI-GPR §25 / country-GPR §45:** §71 gates soft EW with *CIP* stress. This wave gates/cools the **same** soft macro EW with free **VIX** and **aggregate GPR** stress series already in the repo — natural parallel to §71, not a cooler overlay on locked fx4plus, not combo §8 (which cools carry+mom+dollar), not gpr_regime standalone. Explicit: do **not** overlay coolers on locked `fx4plus_gbpcad_d1_voltarget_0025`.

**Data tag:** `approximate_non_ftmo` (no FTMO CSVs under `data/ftmo/`). VIX ~1990-01..**2026-09-16**; GPR ~1985-01..**2026-09-14**.

### Full-sample factor summary

| Factor | mean_mo | t OLS | t NW | %pos | top3 | Sharpe |
|--------|--------:|------:|-----:|-----:|-----:|-------:|
| **soft_low_vix** (primary) | **+0.058%** | +2.92 | **+2.38** | **48%** | 14% | +0.69 |
...
### Consistency windows (primary `soft_low_vix`)

| Window | mean_mo | %pos | top3 | gates | 1% bar |
|--------|--------:|-----:|-----:|:-----:|:------:|
| 2024 | +0.09% | 55% | 81% | PASS | no |
| 2025 | −0.01% | 36% | 92% | PASS | no |
| 2026 | +0.04% | 50% | 98% | PASS | no |
| holdout_365d | −0.02% | 33% | 98% | PASS | no |

**Board:** n=8 soft_nw_pos=**8** hard_nw_pos=**8** promote=**0**.

Sweep **run**. Primary `soft_low_vix` scale≈**7.930** bind=**daily**; scaled HO mean≈−0.14%/mo %pos 33% — clears: **NO**. Soft-best = `soft_raw` (~+11.8 bp/mo, NW t≈**+3.63**) — ungated §66 soft_ew_macro5 still dominates; VIX low-stress gate cuts mean/%pos vs raw while keeping hard |t|≈2.38. Cool companions (~+9 bp/mo) sit between raw and gated. Stack / EW blends ~+7 bp/mo hard. Honesty inverse `soft_high_vix` weakest (+2.9 bp/mo) but still hard |t|≥2. All eight board factors hard |t|≥2 with positive mean — strong confirmation that VIX/GPR conditioning preserves soft-stack significance; still far below 1%/mo consistency (2025/HO primary negative mean).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 windowed verify: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_vix_gpr_conditioned_soft.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §73 (VIX/GPR-conditioned Lustig–Verdelhan carry: IR3M carry × VIX/GPR gate/cool). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve + CIP-conditioned soft + VIX/GPR-conditioned soft + VIX/GPR-conditioned carry queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_vix_gpr_conditioned_soft_wave.md`, `scholarly_fx_vix_gpr_conditioned_soft_*.csv`, `scholarly_fx_vix_gpr_conditioned_soft_meta.json`, `quest_locked_verify_vix_gpr_conditioned_soft.md`.

## 73. VIX/GPR-conditioned Lustig–Verdelhan carry FX wave results (2026-09-24 ~12:06 BST)

**Design (fixed priors, no HO tuning):** Trade Lustig–Verdelhan IR3M / policy-rate carry **only when** global FX-vol / geopolitical stress is low. Stress A = **VIX** z (Menkhoff et al. 2012 JF global FX-vol proxy via `vix_yahoo.csv`). Stress B = aggregate **GPR** z (Caldara–Iacoviello 2022 / Liu–Zhang 2024 via `gpr_daily.csv`). Parallel to §68 (CIP×carry) but with free VIX/GPR stress — **not** a cooler overlay on locked fx4plus. Gate: carry on when lagged stress z ≤ 0; cool: risk_scale ∈ [0.35, 1] when z ≥ 1 (§71/§72 symmetry; lit GPR often **1.5** — documented, not HO-tuned). PIT: VIX/GPR `bar_lag=1` + `signal_lag=1`d on trailing z (`z_window=252`, `min_periods=60` daily — NOT monthly CIP 60); rates FRED/OECD pub_lag_months=1 + `carry_signal_lag=1`d. Costs 1.5 bps/side. G10 USD majors. `n_long=n_short=2`. Primary = `carry_low_vix` (Menkhoff). Companions (pre-registered): `carry_vix_cool`, `carry_low_gpr`, `carry_gpr_cool`, `carry_raw` (ungated honesty), `carry_high_vix` (inverse), `carry_vix_gpr_stack` (sequential VIX×GPR cool product), `carry_vix_gpr_ew` (EW of low_vix ⊕ vix_cool ⊕ low_gpr ⊕ gpr_cool). Prop sizing via `sweep_scale_to_ftmo_budget` when IS mean>0.

**What is new vs fwd_carry §19 / funding_liq §20 / IG OAS §36 / ACM TP §44 / CIP XS §67 / CIP×carry §68 / soft EW §66 / soft CIP §69 / capital-sleeve §53/§70 / CIP-conditioned soft §71 / VIX/GPR-conditioned soft §72 / combo §8 / quest_fred_carry_gpr_study / gpr_regime / AI-GPR §25 / country-GPR §45:** §68 gates Lustig–Verdelhan *carry* with *CIP* stress. §72 gates *soft EW* with VIX/GPR. This wave gates/cools **IR3M carry itself** with free **VIX** and **aggregate GPR** — classic Menkhoff gap left after §72. Distinct from combo §8 (carry+mom+dollar × VIX/GPR), older informal `quest_fred_carry_gpr_study`, and gpr_regime standalone. Explicit: do **not** overlay coolers on locked `fx4plus_gbpcad_d1_voltarget_0025`.
...
| Factor | mean_mo | t OLS | t NW | %pos | top3 | Sharpe |
|--------|--------:|------:|-----:|-----:|-----:|-------:|
| **carry_low_vix** (primary) | **−0.091%** | −1.63 | **−1.59** | **43%** | 15% | −0.37 |
...
### Consistency windows (primary `carry_low_vix`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | +0.12% | 55% | PASS | no |
| 2025 | +0.12% | 55% | PASS | no |
| 2026 | +0.06% | 62% | PASS | no |
| holdout_365d | +0.19% | 75% | PASS | no |

**Board:** n=8 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **run** (any IS mean>0 on board — honesty inverse tiny-positive). Primary `carry_low_vix` scale≈**0.482** bind=**static**; scaled HO mean≈+0.09%/mo %pos 75% — clears: **NO**. Soft-best fallback (no soft |t|≥1.5): `carry_high_vix` (~+0.003%/mo NW t≈**+0.10**) — honesty inverse barely positive; all gated/cooled carry legs negative full-sample. Primary recent windows mildly positive but far below 1%/mo; full-sample primary negative / soft |t| < 1.5. Menkhoff low-VIX gate does **not** rescue IR3M carry on this Yahoo/FRED sample (carry_raw itself ~flat).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 windowed verify: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_vix_gpr_conditioned_carry.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** Done as §74 (VIX/GPR-conditioned Menkhoff FX momentum: Menkhoff mom × VIX/GPR gate/cool). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve + CIP-conditioned soft + VIX/GPR-conditioned soft + VIX/GPR-conditioned carry + VIX/GPR-conditioned mom queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_vix_gpr_conditioned_carry_wave.md`, `scholarly_fx_vix_gpr_conditioned_carry_*.csv`, `scholarly_fx_vix_gpr_conditioned_carry_meta.json`, `quest_locked_verify_vix_gpr_conditioned_carry.md`.


## 74. VIX/GPR-conditioned Menkhoff FX momentum wave results (2026-09-24 ~12:40 BST)

**Design (fixed priors, no HO tuning):** Trade Menkhoff–Sarno–Schmeling–Schrimpf (2012 JFE) cross-sectional FX momentum **only when** global FX-vol / geopolitical stress is low. Stress A = **VIX** z (Menkhoff et al. 2012 JF global FX-vol proxy via `vix_yahoo.csv`). Stress B = aggregate **GPR** z (Caldara–Iacoviello 2022 / Liu–Zhang 2024 via `gpr_daily.csv`). Parallel to §73 (carry × VIX/GPR) — classic scholarly gap left after carry×VIX/GPR — **not** a cooler overlay on locked fx4plus. Gate: mom on when lagged stress z ≤ 0; cool: risk_scale ∈ [0.35, 1] when z ≥ 1 (§71/§72/§73 symmetry; lit GPR often **1.5** — documented, not HO-tuned). PIT: VIX/GPR `bar_lag=1` + `signal_lag=1`d on trailing z (`z_window=252`, `min_periods=60` daily — NOT monthly CIP 60); Menkhoff `formation=63d` `skip=21d` `n_long=n_short=2` `mom_signal_lag=1` (fx_momentum defaults, not HO-tuned). Costs 1.5 bps/side. G10 USD majors. Primary = `mom_low_vix` (Menkhoff). Companions (pre-registered): `mom_vix_cool`, `mom_low_gpr`, `mom_gpr_cool`, `mom_raw` (ungated honesty), `mom_high_vix` (inverse), `mom_vix_gpr_stack` (sequential VIX×GPR cool product), `mom_vix_gpr_ew` (EW of low_vix ⊕ vix_cool ⊕ low_gpr ⊕ gpr_cool). Prop sizing via `sweep_scale_to_ftmo_budget` when IS mean>0.

**What is new vs combo §8 / raw fx_momentum / soft EW §66 / soft CIP §69 / CIP XS §67 / CIP×carry §68 / CIP-conditioned soft §71 / VIX/GPR-conditioned soft §72 / VIX/GPR-conditioned carry §73 / capital-sleeve §53/§70 / funding_liq §20 / gpr_regime / AI-GPR §25 / country-GPR §45:** §73 gates Lustig–Verdelhan *carry* with VIX/GPR. §72 gates *soft EW* with VIX/GPR. Combo §8 cools carry+mom+dollar *together*. This wave gates/cools **Menkhoff FX momentum alone** with free **VIX** and **aggregate GPR** — classic Menkhoff gap left after carry×VIX/GPR. Explicit: do **not** overlay coolers on locked `fx4plus_gbpcad_d1_voltarget_0025`.
...
| Factor | mean_mo | t OLS | t NW | %pos | top3 | Sharpe |
|--------|--------:|------:|-----:|-----:|-----:|-------:|
| **mom_low_vix** (primary) | **−0.017%** | −0.33 | **−0.30** | **42%** | 14% | −0.08 |
...
### Consistency windows (primary `mom_low_vix`)

| Window | mean_mo | %pos | gates | 1% bar |
|--------|--------:|-----:|:-----:|:------:|
| 2024 | −0.07% | 36% | PASS | no |
| 2025 | +0.27% | 64% | PASS | no |
| 2026 | +0.30% | 88% | PASS | no |
| holdout_365d | +0.31% | 83% | PASS | no |

**Board:** n=8 soft_nw_pos=**0** hard_nw_pos=**0** promote=**0**.

Sweep **skipped** (no positive IS mean on any board factor). Soft-best: **none** (all eight legs negative full-sample; least-bad = primary `mom_low_vix` ~−1.7 bp/mo). Menkhoff low-VIX gate slightly improves vs raw (−7.6 bp/mo) but does **not** rescue FX momentum on this Yahoo sample (mom_raw itself negative). Recent windows mildly positive (2025/2026/HO) but far below 1%/mo consistency; 2024 negative. GPR gate/cool legs weakest (hard |t| against).

Locked `fx4plus_gbpcad_d1_voltarget_0025` **untouched PASS** (Yahoo D1 windowed verify: 2024 0.42%/55%/74%; 2025 1.63%/73%/69%; 2026 2.30%/88%/87%; HO 1.52%/75%/81% — see `quest_locked_verify_vix_gpr_conditioned_mom.md`). Config sha unchanged (`bafed8eff5aa7541…`).

**Next structure (if promote=0):** VIX/GPR-conditioned Menkhoff FX momentum boarded (§74). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve + CIP-conditioned soft + VIX/GPR-conditioned soft + VIX/GPR-conditioned carry + VIX/GPR-conditioned mom queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.

Artifacts: `reports/scholarly_fx_vix_gpr_conditioned_mom_wave.md`, `scholarly_fx_vix_gpr_conditioned_mom_*.csv`, `scholarly_fx_vix_gpr_conditioned_mom_meta.json`, `quest_locked_verify_vix_gpr_conditioned_mom.md`.

## §75 — VIX/GPR-conditioned Rogoff PPP / real-FX value (2026-09-24)

**Path:** Menkhoff VIX + Caldara–Iacoviello GPR stress × Rogoff PPP / real-FX cross-sectional value gate/cool. Distinct from raw PPP / BIS REER, soft EW §66, VIX/GPR soft §72, carry §73, mom §74.

**Board:** n=8 soft=0 hard=0 **promote=0**
**Primary `value_low_vix`:** full-sample **−0.024%/mo**, NW t **−0.60**, %pos **31%**
- 2024 −0.05%/45%; 2025 −0.16%/27%; 2026 +0.13%/75%; HO +0.07%/58% — 1% bar **no**
**Risk sweep:** skipped (no positive IS mean on any factor)
**Locked** `fx4plus_gbpcad_d1_voltarget_0025` untouched **PASS**
**Data:** `approximate_non_ftmo`; VIX→2026-09-16, GPR→2026-09-14, CPI→2026-09-01
**Artifacts:** `reports/scholarly_fx_vix_gpr_conditioned_value_wave.md`, strategy + runner + tests
...
**Next:** Done as §77 (EPU/TPU-conditioned BIS REER HML-FX value). FX IV/RR + news-sentiment still blocked. No go-live under `approximate_non_ftmo`.

## §77 — EPU/TPU-conditioned BIS REER HML-FX value (2026-09-24)

**Path:** Baker–Bloom–Davis **EPU** (US USEPUINDXM) + **TPU** (categorical Trade policy) stress × BIS multilateral REER undervaluation / HML-FX value (``reer_cheap_xs`` §31) gate/cool — parallel to VIX/GPR-conditioned PPP value §75 and EPU/TPU-conditioned soft §76. Distinct from raw bis_reer §31, raw ppp, §75 VIX/GPR×PPP value, soft §66/§76, carry/mom §73/§74, CIP waves, capital-sleeve §53/§70, combo §8.

**Board:** n=8 soft=0 hard=0 **promote=0**
**Primary `reer_low_epu`:** full-sample **+0.065%/mo**, NW t **+1.32**, %pos **33%**
- 2024 −0.11%/36%; 2025 −0.09%/9%; 2026 +0.00%/0%; HO +0.00%/0% — 1% bar **no**
**Risk sweep:** primary scale≈4.287 bind=daily; scaled HO flat (EPU gate off — elevated EPU recent) — **not** clear
**Locked** `fx4plus_gbpcad_d1_voltarget_0025` untouched **PASS**
**Data:** `approximate_non_ftmo`; EPU→2026-09-01, TPU→2026-09-01, BIS REER→2026-09-01 (pub_lag=2); monthly z_window=60m (CIP §71 / EPU soft §76 mirror)
**Artifacts:** `reports/scholarly_fx_epu_tpu_conditioned_reer_wave.md`, strategy + runner + tests

**Takeaway:** Gating BIS REER cheap HML on low EPU/TPU did not clear the 1%/mo bar. Primary binary low-EPU gate is mildly positive full-sample (NW t +1.32) but flat/negative through 2024–2026/HO (elevated policy uncertainty → capital retention by design). Soft/hard NW boards empty. Completes EPU/TPU stress conditioning beside soft §76; classic value under EPU/TPU remains promote=0.

**Next:** FTMO MT5 CSV re-score when exports arrive, or other free multi-factor scholarly structures not yet boarded (without cooler-overlay on locked fx4plus). FX IV/RR + news-sentiment still blocked. No go-live under `approximate_non_ftmo`.
