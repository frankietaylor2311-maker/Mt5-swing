...
**Next structure (if promote=0):** CIP/basis gap **closed** (§67). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (e.g. richer soft-stack membership / capital-sleeve refinements without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.
