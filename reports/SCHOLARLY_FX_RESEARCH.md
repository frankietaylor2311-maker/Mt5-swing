...
**Status:** Active (2026-09-24 BST, **VIX/GPR-conditioned Lustig–Verdelhan carry §73 boarded** (IR3M carry × VIX/GPR gate/cool; soft=0 hard=0 promote=0) after VIX/GPR-conditioned soft EW §72 boarded (soft_ew_macro5 × VIX/GPR gate/cool; soft=8 hard=8 promote=0) after CIP-conditioned soft EW §71 boarded (soft_ew_macro5 × CIP-stress gate/cool; soft=5 hard=3 promote=0) after capital-sleeve soft-stack §70 (locked core + soft_ew_macro5/soft_ew7_cip; soft=6 hard=6 promote=0) / soft-stack CIP enrichment §69 / CIP-conditioned carry §68 / Du–Schreger CIP/basis §67 / soft-signal EW §66 / private consumption §65 / GFCF §64 / household-credit §63 / import-value §62 / export-value §61 / cars §60 / construction §59 / real GDP §58 / employment-rate §57 / LFP §56 / wage §55 / UR §54 / capital-sleeve §53 / CPI §52 / PPI §51; hours/vacancies thin/stale; PRSVTT*/ODMN* 404 fall-through; free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + CIP-conditioned soft + VIX/GPR-conditioned soft + VIX/GPR-conditioned carry boarded; FX IV/RR + news-sentiment still blocked). Replaces the technical **overlay hunt** as the primary path toward FTMO-consistent ~1%/month.
...
**Next structure (if promote=0):** VIX/GPR-conditioned Lustig–Verdelhan carry boarded (§73). FX IV/RR + news-sentiment still **blocked**. Free scholarly single-series + CIP XS + CIP×carry + soft-stack CIP + capital soft-sleeve + CIP-conditioned soft + VIX/GPR-conditioned soft + VIX/GPR-conditioned carry queue largely exhausted. Next: **FTMO MT5 CSV re-score** when exports arrive (`data/ftmo/` empty except README), or other multi-factor scholarly structures not yet boarded (without overlaying coolers on locked fx4plus). No go-live claim under `approximate_non_ftmo`.
