...
**Verdict:** Official locked tag **unchanged**. Rolling weekly-mean cool wave: board **n=29**, soft=**6**, hard=**0**, promote=**0**. Best soft `wrebal+wmc_lb6_t0.003_c0.35_frozen` (2024 1.29%/82%/52%; HO 1.46%/75%/79% holdout_fail on HO top3). Soft with MTD stack `wrebal+mtd+wmc_lb6_t0.003_c0.35` (2024 1.47%/82%/41%; HO 1.10%/75%/73% holdout_fail). Prior month-mean/surplus/pace/burst/DD-depth also no promote. **Promote: NO.** Still blocked on FTMO CSVs.
...
Re-verified this wave (`RF=0.08`, `PORT_VOL_TARGET=0.0025`, weights oos_sharpe; `scripts/eval_windowed_consistency.py` → `quest_locked_verify_keepalive_0549`):

| Window | Return | Mean mo | %pos | Top3 | Gates | P2T |
|--------|-------:|--------:|-----:|-----:|:-----:|----:|
| 2024 | +5.38% | **0.42%** | 55% | 74% | PASS | ~7% |
| 2025 | +22.04% | **1.63%** | 73% | 69% | PASS | ~4% |
| 2026 | +20.0%+ | **2.30%** | 88% | 87% | PASS | ~6% |
| holdout_365d | +21.5%+ | **1.52%** | 75% | 81% | PASS | ~7% |
| roll12_m6 | +16.4%+ | **1.13%** | 67% | 80% | PASS | ~4% |

## Wave: rolling weekly mean cool (this)

Scripts/helpers:
- `scripts/quest_weekly_mean_cool_consistency_wf.py`
- `apply_rolling_weekly_mean_cool` in `overlays.py` (completed-week lag-1 trailing mean ≥ thresh → cool; hi≤1)
- Soft mean floor 0.0095 both IS windows; HO never for selection; lb∈{4,6,8,12} × thresh∈{0.002,0.003,0.004,0.005} × cool∈{0.35,0.5,0.65}

### Board

| Family | Notes |
|--------|-------|
| baseline / weekly-cool alone | sparse+nested — is_fail / soft_mean_fail; 2024 mean ~0.42–0.84% |
| frozen_wrebal ± weekly-cool | **soft** holdout_fail (HO %pos 75% but top3 ~79%) |
| frozen_wrebal_mtd ± weekly-cool | **soft** holdout_fail; HO %pos 75% |
| is_blend | soft_mean_fail / holdout_fail after floor |

**Totals:** soft=**6** hard=**0** promote=**0** (board n=29).

### Best soft (not promote) — `wrebal+wmc_lb6_t0.003_c0.35_frozen`

| Window | Mean mo | %pos | Top3 | Role |
|--------|--------:|-----:|-----:|------|
| 2024 | **1.29%** | **82%** | **52%** | IS — soft OK |
| 2025_IS | **1.30%** | **88%** | **65%** | IS — soft OK |
| holdout | **1.46%** | **75%** | **79%** | confirm — holdout_fail (top3) |

Soft with MTD+weekly — `wrebal+mtd_t0.015_a0.5_frozen+wmc_lb6_t0.003_c0.35`: 2024 **1.47%/82%/41%**; HO **1.10%/75%/73%** holdout_fail.
...
Session: 2026-09-17 05:23 BST (wave: **causal trailing completed-month mean cool** on locked sleeve ± frozen HO-robust wrebal / MTD). Prior 04:54 month-end surplus; 04:22 MTD-pace+burst; 03:48 DD-depth — no promote.
...
**Verdict:** Official locked tag **unchanged**. Trailing month-mean cool wave: board **n=26**, soft=**4**, hard=**0**, promote=**0**. Best soft `wrebal+mtd_t0.015_a0.5_frozen` (2024 1.18%/82%/45%; HO 1.69%/67%/71% holdout_fail). Best with mean-cool `wrebal+mtd+mmc_lb6_t0.012_c0.65` (2024 1.10%/82%/45%; HO 1.44%/67%/72% holdout_fail). Prior surplus/pace/burst/DD-depth also no promote. **Promote: NO.** Still blocked on FTMO CSVs.
...
Re-verified this wave (`RF=0.08`, `PORT_VOL_TARGET=0.0025`, weights oos_sharpe; `scripts/eval_windowed_consistency.py` → `quest_locked_verify_keepalive_0523`):
...
## Wave: trailing completed-month mean cool (this)

Scripts/helpers:
- `scripts/quest_month_mean_cool_consistency_wf.py`
- `apply_trailing_month_mean_cool` in `overlays.py` (completed-month lag-1 trailing mean ≥ thresh → cool; hi≤1)
- Soft mean floor 0.0095 both IS windows; HO never for selection; lb∈{3,4,6} × thresh∈{0.01,0.012,0.015,0.02} × cool∈{0.35,0.5,0.65}

### Board

| Family | Notes |
|--------|-------|
| baseline / mean-cool alone | sparse+nested — all is_fail; 2024 mean ~0.41–0.52% |
| frozen_wrebal ± mean-cool | soft_mean_fail (2024 ~0.57%) |
| frozen_wrebal_mtd ± mean-cool | **soft** holdout_fail; HO %pos 67% |
| is_blend | soft_mean_fail / is_fail after floor |
...
| 2025_IS | **1.74%** | **75%** | **64%** | IS — soft OK |
| holdout | **1.69%** | **67%** | **71%** | confirm — holdout_fail (%pos) |

Best soft with mean-cool — `wrebal+mtd+mmc_lb6_t0.012_c0.65`: 2024 **1.10%/82%/45%**; HO **1.44%/67%/72%** holdout_fail.
...
Re-verified this wave (`RF=0.08`, `PORT_VOL_TARGET=0.0025`, weights oos_sharpe; `scripts/eval_windowed_consistency.py` → `quest_locked_verify_keepalive_0252`):
...
## Wave: peak-proximity cool (this)

Scripts/helpers:
- `scripts/quest_peak_proximity_consistency_wf.py`
- `apply_peak_proximity_cool` in `overlays.py` (rolling N-bar high; lag-1 eq/peak; cool if proximity≥−eps; hi≤1)
- Soft mean floor 0.0095 both IS windows; HO never for selection; lb∈{21,42,63} × eps∈{0.005,0.01,0.02} × cool∈{0.35,0.5,0.65}

### Board

| Family | Notes |
|--------|-------|
| baseline / peakprox alone | full 27-cell grid — mostly is_fail; 2024 mean ~0.21–0.63% |
| frozen_wrebal ± peakprox | **soft** year_clear_fail (full-2025 <1%) |
| frozen_wrebal_mtd ± peakprox | soft holdout_fail or year_clear_fail; nested HO %pos 75% |
| is_blend | soft_mean_fail / holdout_fail after floor |

**Totals:** soft=**5** hard=**0** promote=**0** (board n=41).

### Best soft (not promote) — `wrebal+peakprox_lb63_e0.01_c0.35_frozen`

| Window | Mean mo | %pos | Top3 | Role |
|--------|--------:|-----:|-----:|------|
| 2024 | **1.27%** | **82%** | **49%** | IS — soft OK |
| 2025_IS | **1.28%** | **88%** | **63%** | IS — hard top3 fail |
| holdout | **1.45%** | **75%** | **60%** | confirm — year_clear_fail (full-2025 ~0.90%) |
...
Session: 2026-09-17 02:30 BST (wave: **causal consecutive win-day cool** on locked sleeve ± frozen HO-robust wrebal / MTD).
...
**Verdict:** Official locked tag **unchanged**. Consecutive win-day cool wave: board **n=29**, soft=**4**, hard=**0**, promote=**0**. Best soft `wrebal+mtd_t0.015_a0.5+wstreak_n8_c0.35` (HO %pos 67%; nested wstreak near-inert). Win-streak alone on locked leaves 2024 ~0.37–0.44%. **Promote: NO.** Still blocked on FTMO CSVs.
...
Re-verified this wave (`RF=0.08`, `PORT_VOL_TARGET=0.0025`, weights oos_sharpe; `scripts/eval_windowed_consistency.py` → `quest_locked_verify_keepalive_0230`):
...
## Wave: consecutive win-day cool (this)

Scripts/helpers:
- `scripts/quest_win_streak_consistency_wf.py`
- `apply_consecutive_win_cool` in `overlays.py` (consecutive positive daily returns; lag-1; cool if streak≥n; hi≤1)
- Soft mean floor 0.0095 both IS windows; HO never for selection; streak_n∈{3,4,5,6,8} × cool∈{0.35,0.5,0.65}

### Board

| Family | Notes |
|--------|-------|
| baseline / wstreak alone | full 15-cell grid — no soft; 2024 mean ~0.37–0.44% |
| frozen_wrebal ± wstreak | soft_mean_fail / holdout_fail |
| frozen_wrebal_mtd ± wstreak | **4 soft** — all holdout_fail (HO %pos 67%) |
| is_blend | soft_mean_fail (2024 < 0.95%) after floor |

**Totals:** soft=**4** hard=**0** promote=**0** (board n=29).

### Best soft (not promote) — `wrebal+mtd_t0.015_a0.5_frozen+wstreak_n8_c0.35`
...
Session: 2026-09-17 01:52 BST (wave: **causal rolling hit-rate cool** on locked sleeve ± frozen HO-robust wrebal / MTD).
...
**Verdict:** Official locked tag **unchanged**. Rolling hit-rate cool wave: board **n=20**, soft=**3**, hard=**0**, promote=**0**. Best soft `wrebal+mtd_t0.015_a0.5` (HO %pos 67%; nested hitrate mild dilutive). Hitrate alone on locked lifts 2024 %pos to ~64–73% but stays under soft mean floor. **Promote: NO.** Still blocked on FTMO CSVs.
...
Re-verified this wave (`RF=0.08`, `PORT_VOL_TARGET=0.0025`, weights oos_sharpe; `scripts/eval_windowed_consistency.py` → `quest_locked_verify_keepalive_0152`):
...
## Wave: rolling hit-rate cool (this)

Scripts/helpers:
- `scripts/quest_rolling_hitrate_consistency_wf.py`
- `apply_rolling_hitrate_cool` in `overlays.py` (hit-rate mean(r_d>0); lag-1; cool if >high or <low; hi≤1)
- Soft mean floor 0.0095 both IS windows; HO never for selection; 108-cell grid pruned to top-k 2024 picks on confirm

### Board

| Family | Notes |
|--------|-------|
| baseline / hitrate alone | top-6 of 108 — no soft; 2024 %pos ~64–73%, mean ≤0.76% |
| frozen_wrebal ± hitrate | soft_mean_fail / is_fail |
| frozen_wrebal_mtd ± hitrate | **3 soft** — all holdout_fail (HO %pos 67%) |
| is_blend | soft_mean_fail (2024 < 0.95%) |

**Totals:** soft=**3** hard=**0** promote=**0** (board n=20).
...
Session: 2026-09-17 01:30 BST (wave: **causal rolling Sharpe cool** on locked sleeve ± frozen HO-robust wrebal / MTD).
...
**Verdict:** Official locked tag **unchanged**. Rolling Sharpe cool wave: board **n=20**, soft=**3**, hard=**0**, promote=**0**. Best soft `wrebal+mtd_t0.015_a0.5` (HO %pos 67%; nested rsharpe mild dilutive). Rsharpe alone on locked lifts 2024 %pos to ~64% but stays under soft mean floor. **Promote: NO.** Still blocked on FTMO CSVs.
...
Re-verified this wave (`RF=0.08`, `PORT_VOL_TARGET=0.0025`, weights oos_sharpe; `scripts/eval_windowed_consistency.py` → `quest_locked_verify_keepalive_0130`):
...
## Wave: rolling Sharpe cool (this)

Scripts/helpers:
- `scripts/quest_rolling_sharpe_consistency_wf.py`
- `apply_rolling_sharpe_cool` in `overlays.py` (annualized Sharpe mean/std×√252; lag-1; cool if >high or <low; hi≤1)
- Soft mean floor 0.0095 both IS windows; HO never for selection; 108-cell grid pruned to top-k 2024 picks on confirm
...
| Family | Notes |
|--------|-------|
| baseline / rsharpe alone | top-6 of 108 — no soft; 2024 %pos ~64%, mean ≤0.62% |
| frozen_wrebal ± rsharpe | soft_mean_fail / holdout_fail |
| frozen_wrebal_mtd ± rsharpe | **3 soft** — all holdout_fail (HO %pos 67%) |
| is_blend | soft_mean_fail (2024 < 0.95%) |
