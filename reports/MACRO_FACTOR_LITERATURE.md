...
| VIX risk gate | **boarded §72 / §73 / §74** | Menkhoff-style global vol risk-off on soft_ew_macro5 (§72), IR3M carry (§73; primary carry_low_vix), and Menkhoff FX mom (§74; primary mom_low_vix; fixed z≥1 → scale 0.35) |
| GPR risk gate | **boarded §72 / §73 / §74** | Caldara–Iacoviello / Liu–Zhang geo risk-off on soft_ew_macro5 (§72), IR3M carry (§73), and Menkhoff FX mom (§74; cool z≥1 for §71/§72/§73 symmetry; lit often 1.5) |
| VIX+GPR stack | **boarded §72 / §73 / §74** | Sequential VIX then GPR cool on soft (`soft_vix_gpr_stack`), carry (`carry_vix_gpr_stack`), and mom (`mom_vix_gpr_stack`) — **not** a cool-knob grid |
| EPU risk gate | **boarded §76 / §77** | Baker–Bloom–Davis US EPU (USEPUINDXM) gate/cool on soft_ew_macro5 (§76; primary soft_low_epu) and BIS REER cheap HML (§77; primary reer_low_epu); monthly z 60m; fixed z≥1 → scale 0.35 (§71–§76 symmetry) |
| TPU risk gate | **boarded §76 / §77** | Categorical Trade-policy TPU gate/cool on soft_ew_macro5 (§76) and BIS REER cheap HML (§77); reer_low_tpu / reer_tpu_cool companions |
| EPU+TPU stack | **boarded §76 / §77** | Sequential EPU then TPU cool on soft (`soft_epu_tpu_stack` §76) and REER value (`reer_epu_tpu_stack` §77) — **not** a cool-knob grid; distinct from standalone EPU/TPU §18 |
...
4. **Menkhoff et al. (2012), JFE** — Currency momentum distinct from carry (price momentum); boarded as scholarly FX wave **§74** (VIX/GPR-conditioned Menkhoff mom; primary `mom_low_vix`).
...
8b. **Du, Keerati & Schreger (2025)** — *Decoupling Dollar and Treasury Privilege* — public gov-bond CIP + U.S. Treasury Premium panel (`cip_dataset_v4`); boarded as scholarly FX wave **§67**; CIP-conditioned Lustig–Verdelhan carry boarded **§68**; soft-stack CIP enrichment boarded **§69**; CIP-conditioned soft EW boarded **§71**; VIX/GPR-conditioned soft EW boarded **§72**; VIX/GPR-conditioned Lustig–Verdelhan carry boarded **§73**; VIX/GPR-conditioned Menkhoff FX momentum boarded **§74**; EPU/TPU-conditioned soft EW boarded **§76**; EPU/TPU-conditioned BIS REER HML-FX value boarded **§77**.
...
| Multi-country CPI / PPI / IP / retail / unemployment / wages / LFP / employment-rate / real GDP / construction / passenger-car regs / merchandise export-value / merchandise import-value / household credit / GFCF investment / private final consumption / soft-signal EW stack | Dahlquist economic momentum + Lustig–Verdelhan consumption-growth risk + Mian–Sufi / Borio household credit | **Mostly boarded** — IP §42 / retail §46 / employment §41 (persons) / PPI §51 / CPI §52 / **UR §54** / **wage/earnings §55** / **LFP/activity §56** / **employment-rate §57** / **real GDP growth §58** / **construction-production §59** / **passenger-car regs §60** / **merchandise export-value §61** / **merchandise import-value §62** / **household-credit §63** / **GFCF investment §64** / **private final consumption §65** / **soft-signal EW recombination §66** (SOFT_LEGS=bci_chg/high_ip/high_ppi/low_gdp/low_cars; primary soft_ew5; board soft=7 hard=7 promote=0) / **Du–Schreger CIP/basis §67** (cip_govt 5y; primary low_cip_xs; soft=2 hard=0 promote=0) / **CIP-conditioned carry §68** (carry×CIP stress; primary carry_low_cip_stress; soft=2 hard=0 promote=0) / **soft-stack CIP enrichment §69** (SOFT_LEGS_CIP=macro5+ust_premium_haven_usd+cip_ew; primary soft_ew7_cip; board soft=7 hard=7 promote=0) / **CIP-conditioned soft EW §71** (soft_ew_macro5 × CIP-stress gate/cool; primary soft_low_cip_stress; soft=5 hard=3 promote=0) / **VIX/GPR-conditioned soft EW §72** (soft_ew_macro5 × VIX/GPR gate/cool; primary soft_low_vix; soft=8 hard=8 promote=0) / **VIX/GPR-conditioned Lustig–Verdelhan carry §73** (IR3M carry × VIX/GPR gate/cool; primary carry_low_vix; soft=0 hard=0 promote=0) / **EPU/TPU-conditioned soft EW §76** (soft_ew_macro5 × EPU/TPU gate/cool; primary soft_low_epu; soft=8 hard=8 promote=0) / **EPU/TPU-conditioned BIS REER HML-FX value §77** (reer_cheap_xs × EPU/TPU gate/cool; primary reer_low_epu; soft=0 hard=0 promote=0). PPI STALE ~2022-12; CPI JPY STALE ~2021-06. Building-permits §43 distinct (leading flow). Capital-sleeve mix §53 boarded (promote=0). |
| News sentiment (FX-specific) | Event studies / war headlines beyond GPR | **Missing** — GDELT / RavenPack / Factiva (licensed) |
| Cross-currency basis / CIP | Intermediary stress / Treasury premium | **Present / boarded §67** — Du–Keerati–Schreger `cip_dataset_v4` gov CIP (`cip_govt` bps, tenor=5y a priori) + UST premium; primary `low_cip_xs`; soft=2 hard=0 promote=0. **CIP-conditioned carry boarded §68** — Lustig–Verdelhan carry × CIP-stress gate/cool; primary `carry_low_cip_stress`; soft=2 hard=0 promote=0. **Soft-stack CIP enrichment boarded §69** — macro soft5 + CIP soft2 EW; primary `soft_ew7_cip`; soft=7 hard=7 promote=0. **CIP-conditioned soft EW boarded §71** — soft_ew_macro5 × CIP-stress gate/cool; primary `soft_low_cip_stress`; soft=5 hard=3 promote=0. **VIX/GPR-conditioned soft EW boarded §72** — soft_ew_macro5 × VIX/GPR gate/cool; primary `soft_low_vix`; soft=8 hard=8 promote=0. **VIX/GPR-conditioned Lustig–Verdelhan carry boarded §73** — IR3M carry × VIX/GPR gate/cool; primary `carry_low_vix`; soft=0 hard=0 promote=0. **VIX/GPR-conditioned Menkhoff FX momentum boarded §74** — Menkhoff mom × VIX/GPR gate/cool; primary `mom_low_vix`; soft=0 hard=0 promote=0. **EPU/TPU-conditioned soft EW boarded §76** — soft_ew_macro5 × EPU/TPU gate/cool; primary `soft_low_epu`; soft=8 hard=8 promote=0. **EPU/TPU-conditioned BIS REER HML-FX value boarded §77** — reer_cheap_xs × EPU/TPU gate/cool; primary `reer_low_epu`; soft=0 hard=0 promote=0 |
