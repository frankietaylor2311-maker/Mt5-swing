"""Tests for OECD MEI labour productivity (ULQELP01) differential FX module."""
...
def test_lp_series_map_full_g10():
    for ccy in ("USD", "EUR", "GBP", "JPY", "CAD", "AUD", "NZD", "CHF"):
        assert ccy in LP_SERIES
        assert LP_SERIES[ccy] is not None
    assert LP_SERIES["USD"] == "ULQELP01USQ657S"
    assert LP_SERIES["EUR"] == "ULQELP01DEQ657S"  # Germany proxy
    assert LP_SERIES["GBP"] == "ULQELP01GBQ657S"
    assert LP_SERIES["JPY"] == "ULQELP01JPQ657S"
    assert LP_SERIES["CAD"] == "ULQELP01CAQ657S"
    assert LP_SERIES["AUD"] == "ULQELP01AUQ657S"
    assert LP_SERIES["NZD"] == "ULQELP01NZQ657S"
    assert LP_SERIES["CHF"] == "ULQELP01CHQ657S"


def test_pub_lag_shifts_known_date():
    """pub_lag_months=3 must move quarter-start index forward by ~3 months."""
    raw = load_labour_prod_panel(pub_lag_months=0, download=True, force=False)
    lagged = load_labour_prod_panel(pub_lag_months=3, download=False, force=False)
...
def test_prepare_scores_high_lp_long_productive():
    """Higher LP YoY → higher high_lp score (productivity prior)."""
    idx = pd.date_range("2010-01-01", periods=120, freq="MS", tz="UTC")
    # EUR LP rises (productive), GBP falls (weak)
    ...
    cfg = LabourProdFxConfig(signal_lag=0, z_window=60, min_periods=24)
    scores = prepare_labour_prod_scores(lp, cfg=cfg)
    last = scores["high_lp"].dropna(how="all").iloc[-1]
    assert float(last["EUR"]) > float(last["GBP"])
    last_lo = scores["low_lp"].dropna(how="all").iloc[-1]
    assert float(last_lo["GBP"]) > float(last_lo["EUR"])


def test_lp_chg_prefers_acceleration():
    """Rising YoY → positive lp_chg score (productivity acceleration)."""
    ...
    cfg = LabourProdFxConfig(signal_lag=0, chg_periods=12)
    scores = prepare_labour_prod_scores(lp, cfg=cfg)
    last = scores["lp_chg"].dropna(how="all").iloc[-1]
    assert float(last["EUR"]) > float(last["GBP"])


def test_lp_no_lookahead():
    """Mutating future LP must not change earlier factor returns."""
    ...
    cfg = LabourProdFxConfig(signal_lag=1, cost_bps_side=1.5)
    f1 = labour_prod_factor_returns(ret, lp, cfg=cfg)
    assert "high_lp_xs" in f1
    assert "low_lp_xs" in f1
    assert "high_lp_z_xs" in f1
    assert "lp_chg_xs" in f1
    assert "us_lp_stress_fx" in f1
    assert "us_lp_haven_usd" in f1
    assert "lp_ew" in f1

    lp2 = lp.copy()
    lp2.iloc[-6:] = lp2.iloc[-6:] + 15.0
    f2 = labour_prod_factor_returns(ret, lp2, cfg=cfg)

    cut = idx[-120]
    for name in ("high_lp_xs", "lp_chg_xs", "us_lp_stress_fx"):
...
def test_us_lp_z_fires_on_depressed_growth():
    """Depressed US LP YoY vs recent history → negative z."""
...
def test_loader_smoke_and_coverage():
    panel = load_labour_prod_panel(pub_lag_months=DEFAULT_PUB_LAG_MONTHS, download=True)
    ...
    us = load_us_labour_prod(download=False)
    assert len(us) > 50
    cov = labour_prod_coverage(panel)
    assert len(cov) >= 8
    mapped = cov[cov["n_obs"] > 0]
    assert (mapped["n_obs"] > 0).all()
    assert panel.attrs.get("series_map", {}).get("EUR") == "ULQELP01DEQ657S"
    assert panel.attrs.get("unit") == "labour_prod_yoy_pct_as_reported"
...
def test_high_lp_maps_to_top_score():
    """High LP YoY maps to top high_lp scores."""
    ...
    cfg = LabourProdFxConfig(signal_lag=0, n_long=2, n_short=2)
    scores = prepare_labour_prod_scores(lp, cfg=cfg)
    last = scores["high_lp"].dropna(how="all").iloc[-1].dropna()
