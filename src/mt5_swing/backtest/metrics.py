...
def _max_daily_dd(equity: pd.Series) -> float:
    """Max of (prior calendar-day close equity - intraday equity) / prior close."""
    if equity.empty:
        return 0.0
    eq = equity.copy()
    ...
    day_closes: dict = {}
    prev_close = float(eq.iloc[0])
    current_day = dates[0]
    day_open = prev_close
    for ts, val, d in zip(eq.index, eq.values, dates):
        if d != current_day:
            day_closes[current_day] = prev_close
            day_open = prev_close
            current_day = d
...
def compute_metrics(
    ...
    *,
    ...
    periods_per_year: float = 252 * 6,  # ~H4 bars
) -> Metrics:
