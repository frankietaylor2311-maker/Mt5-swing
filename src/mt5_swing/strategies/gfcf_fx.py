"""Real GFCF / investment growth differential FX (OECD MEI NAEXKP04).

Literature framing (see also ``fred_gfcf`` docstring)
----------------------------------------------------
- Dahlquist–Hasseltoft (2020) economic-momentum / growth differentials → FX,
  applied to **investment / GFCF** (not aggregate GDP).
- Primary: long **high** relative real GFCF YoY / short low (investment boom → appreciate).
- Honesty alternate: long **low** GFCF (capex stress).
- Distinct from GDP §58, construction §59, IP §42, BCI §39, building permits §43,
  HPI §35, CLI §37, retail §46, cars §60, export/import §61/§62, household credit §63,
  private credit/GDP §32.

Fixed priors (no holdout tuning)
--------------------------------
- ``high_gfcf_xs`` (**primary**): long high relative GFCF YoY / short low
  (n_long=n_short=2 on available foreign panel; full G10 mapped).
- ``low_gfcf_xs``: honesty reverse.
- ``high_gfcf_z_xs``: long high 60m trailing z of GFCF YoY / short low z.
- ``gfcf_chg_xs``: long **accelerating** investment (+Δ12 of YoY) / short decelerating.
- ``us_gfcf_stress_fx``: depressed US GFCF z ≤ −1.0 → long foreign (USD soft).
- ``us_gfcf_haven_usd``: depressed US GFCF z ≤ −1.0 → long USD (haven alt).
- ``gfcf_ew``: EW of ``high_gfcf_xs``, ``gfcf_chg_xs``, ``us_gfcf_stress_fx``.

Score basis (fixed a priori): **real GFCF YoY %** (compounded from Q657S QoQ).
...
def prepare_gfcf_scores(
    gfcf_panel: pd.DataFrame,
    *,
    cfg: GfcfFxConfig | None = None,
) -> dict[str, pd.DataFrame]:
    """Build signed monthly score panels from PIT GFCF **YoY %** (already pub-lagged).

    Scores exclude USD column when present (XS sorts are foreign vs USD).
    Score basis = real GFCF YoY % — compounded from Q657S QoQ.
    """
    cfg = cfg or GfcfFxConfig()
    panel = gfcf_panel.copy()
    ...
    # High GFCF YoY → high score → long (investment boom / economic-momentum → appreciate)
    high = foreign.copy()
    if cfg.signal_lag > 0:
        high = high.shift(int(cfg.signal_lag))
    out["high_gfcf"] = high

    # Low GFCF honesty alternate (capex stress)
    low = (-foreign).copy()
    if cfg.signal_lag > 0:
    ...
    out["high_gfcf_z"] = high_z

    # Accelerating investment → long (score = +Δ12 of YoY)
...
def _us_gfcf_tilt_returns(
    ...
    *,
    ...
    """Binary USD / FX tilt from lagged US GFCF **YoY** z.

    mode='stress': z ≤ z_low → long foreign (short USD) — US investment soft.
...
def gfcf_factor_returns(
    pair_ret: pd.DataFrame,
    gfcf_panel: pd.DataFrame,
    *,
    cfg: GfcfFxConfig | None = None,
    us_gfcf_override: pd.Series | None = None,
) -> dict[str, pd.Series]:
    """Daily returns for GFCF-YoY factors + US tilts + EW."""
    cfg = cfg or GfcfFxConfig()
    scores = prepare_gfcf_scores(gfcf_panel, cfg=cfg)
    factors: dict[str, pd.Series] = {}

    if "high_gfcf" in scores:
        factors["high_gfcf_xs"] = _scores_to_daily_returns(
            scores["high_gfcf"], pair_ret, cfg, name="high_gfcf_xs"
        )
    if "low_gfcf" in scores:
        factors["low_gfcf_xs"] = _scores_to_daily_returns(
            scores["low_gfcf"], pair_ret, cfg, name="low_gfcf_xs"
        )
    if "high_gfcf_z" in scores:
        factors["high_gfcf_z_xs"] = _scores_to_daily_returns(
            scores["high_gfcf_z"], pair_ret, cfg, name="high_gfcf_z_xs"
        )
    if "gfcf_chg" in scores:
        factors["gfcf_chg_xs"] = _scores_to_daily_returns(
            scores["gfcf_chg"], pair_ret, cfg, name="gfcf_chg_xs"
        )

    us = us_gfcf_override
    if us is None and "USD" in gfcf_panel.columns:
        us = gfcf_panel["USD"]
    if us is not None and us.dropna().shape[0] >= cfg.min_periods:
        factors["us_gfcf_stress_fx"] = _us_gfcf_tilt_returns(
            us, pair_ret, cfg=cfg, mode="stress", name="us_gfcf_stress_fx"
        )
        factors["us_gfcf_haven_usd"] = _us_gfcf_tilt_returns(
            us, pair_ret, cfg=cfg, mode="haven", name="us_gfcf_haven_usd"
        )

    blend_keys = [
        k
        for k in ("high_gfcf_xs", "gfcf_chg_xs", "us_gfcf_stress_fx")
        if k in factors
    ]
    if blend_keys:
