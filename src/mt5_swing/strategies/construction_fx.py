"""Construction-production growth differential FX (OECD MEI PRCNTO01).

Literature framing (see also ``fred_construction`` docstring)
-------------------------------------------------------------
- Growth-channel / Taylor-rule FX applied to construction *output* (coincident).
- Dahlquist–Hasseltoft (2020) economic-momentum spirit.
- Primary: long **high** relative construction YoY / short low.
- Honesty alternate: long **low** construction (activity-stress debtor premium).
- Distinct from building-permits §43 (leading housing *flow*), IP §42
  (industry *excl.* construction), GDP §58, CLI §37, BCI §39, retail §46.

Fixed priors (no holdout tuning)
--------------------------------
- ``high_cons_xs`` (**primary**): long high relative construction YoY / short low
  (n_long=n_short=2 on available foreign panel; full G10 mapped).
- ``low_cons_xs``: honesty reverse.
- ``high_cons_z_xs``: long high 60m trailing z of construction YoY / short low z.
- ``cons_chg_xs``: long **accelerating** growth (+Δ12 of YoY) / short decelerating.
- ``us_cons_stress_fx``: depressed US construction z ≤ −1.0 → long foreign (USD soft).
- ``us_cons_haven_usd``: depressed US construction z ≤ −1.0 → long USD (haven alt).
- ``cons_ew``: EW of ``high_cons_xs``, ``cons_chg_xs``, ``us_cons_stress_fx``.

Score basis (fixed a priori): **construction YoY %**.
...
@dataclass
class ConstructionFxConfig:
    """Fixed research priors — do not grid / holdout-tune."""

    signal_lag: int = 1  # months; a priori after pub_lag (mirror GDP §58 / ER §57)
    n_long: int = 2  # available foreign panel (full G10 mapped)
    n_short: int = 2
    z_window: int = 60  # months for trailing z (5y)
    min_periods: int = 24
    z_low: float = -1.0  # US construction YoY z ≤ z_low → stress / haven tilt
    usd_tilt: float = 0.5  # gross |w| when tilt is on
    cost_bps_side: float = 1.5
    chg_periods: int = 12  # Δ12 of construction YoY (acceleration)
...
def prepare_construction_scores(
    cons_panel: pd.DataFrame,
    *,
    cfg: ConstructionFxConfig | None = None,
) -> dict[str, pd.DataFrame]:
    """Build signed monthly score panels from PIT construction **YoY %**.

    Scores exclude USD column when present (XS sorts are foreign vs USD).
    Score basis = construction YoY %.
    """
    cfg = cfg or ConstructionFxConfig()
    panel = cons_panel.copy()
    ...
    # High construction YoY → high score → long (growth / coincident activity)
    high = foreign.copy()
    if cfg.signal_lag > 0:
        high = high.shift(int(cfg.signal_lag))
    out["high_cons"] = high

    # Low construction honesty alternate (activity-stress debtor premium)
    low = (-foreign).copy()
    if cfg.signal_lag > 0:
...
def _us_cons_tilt_returns(
    ...
    *,
    ...
    """Binary USD / FX tilt from lagged US construction **YoY** z.

    mode='stress': z ≤ z_low → long foreign (short USD) — US construction soft.
...
def construction_factor_returns(
    pair_ret: pd.DataFrame,
    cons_panel: pd.DataFrame,
    *,
    cfg: ConstructionFxConfig | None = None,
    us_cons_override: pd.Series | None = None,
) -> dict[str, pd.Series]:
    """Daily returns for construction-YoY factors + US tilts + EW."""
    cfg = cfg or ConstructionFxConfig()
    scores = prepare_construction_scores(cons_panel, cfg=cfg)
    factors: dict[str, pd.Series] = {}

    if "high_cons" in scores:
        factors["high_cons_xs"] = _scores_to_daily_returns(
            scores["high_cons"], pair_ret, cfg, name="high_cons_xs"
        )
    if "low_cons" in scores:
        factors["low_cons_xs"] = _scores_to_daily_returns(
            scores["low_cons"], pair_ret, cfg, name="low_cons_xs"
        )
    if "high_cons_z" in scores:
        factors["high_cons_z_xs"] = _scores_to_daily_returns(
            scores["high_cons_z"], pair_ret, cfg, name="high_cons_z_xs"
        )
    if "cons_chg" in scores:
        factors["cons_chg_xs"] = _scores_to_daily_returns(
            scores["cons_chg"], pair_ret, cfg, name="cons_chg_xs"
        )

    us = us_cons_override
    if us is None and "USD" in cons_panel.columns:
        us = cons_panel["USD"]
    if us is not None and us.dropna().shape[0] >= cfg.min_periods:
        factors["us_cons_stress_fx"] = _us_cons_tilt_returns(
            us, pair_ret, cfg=cfg, mode="stress", name="us_cons_stress_fx"
        )
        factors["us_cons_haven_usd"] = _us_cons_tilt_returns(
            us, pair_ret, cfg=cfg, mode="haven", name="us_cons_haven_usd"
        )

    blend_keys = [
        k
        for k in ("high_cons_xs", "cons_chg_xs", "us_cons_stress_fx")
        if k in factors
    ]
    if blend_keys:
