"""Passenger-car registrations growth differential FX (OECD MEI SLRTCR03).

Literature framing (see also ``fred_passenger_cars`` docstring)
-------------------------------------------------------------
- Dahlquist–Hasseltoft (2020) economic-momentum spirit on durable consumer
  demand (passenger-car registrations YoY).
- Growth-channel / Taylor-rule FX applied to auto demand (leading/coincident).
- Primary: long **high** relative car-regs YoY / short low.
- Honesty alternate: long **low** car-regs (demand-stress debtor premium).
- Distinct from retail-sales §46 (broad retail volume), building-permits §43,
  construction §59, IP §42, BCI §39, CLI §37.

Fixed priors (no holdout tuning)
--------------------------------
- ``high_cars_xs`` (**primary**): long high relative car-regs YoY / short low
  (n_long=n_short=2 on available foreign panel; EUR/GBP/JPY/CAD/AUD live).
- ``low_cars_xs``: honesty reverse.
- ``high_cars_z_xs``: long high 60m trailing z of car-regs YoY / short low z.
- ``cars_chg_xs``: long **accelerating** growth (+Δ12 of YoY) / short decelerating.
- ``us_cars_stress_fx``: depressed US car-regs z ≤ −1.0 → long foreign (USD soft).
- ``us_cars_haven_usd``: depressed US car-regs z ≤ −1.0 → long USD (haven alt).
- ``cars_ew``: EW of ``high_cars_xs``, ``cars_chg_xs``, ``us_cars_stress_fx``.

Score basis (fixed a priori): **passenger-car registrations YoY %**.
...
@dataclass
class PassengerCarsFxConfig:
    """Fixed research priors — do not grid / holdout-tune."""

    signal_lag: int = 1  # months; a priori after pub_lag (mirror GDP §58 / cons §59)
    n_long: int = 2  # available foreign panel (EUR/GBP/JPY/CAD/AUD = 5)
    n_short: int = 2
    z_window: int = 60  # months for trailing z (5y)
    min_periods: int = 24
    z_low: float = -1.0  # US car-regs YoY z ≤ z_low → stress / haven tilt
    usd_tilt: float = 0.5  # gross |w| when tilt is on
    cost_bps_side: float = 1.5
    chg_periods: int = 12  # Δ12 of car-regs YoY (acceleration)
...
def prepare_passenger_cars_scores(
    cars_panel: pd.DataFrame,
    *,
    cfg: PassengerCarsFxConfig | None = None,
) -> dict[str, pd.DataFrame]:
    """Build signed monthly score panels from PIT passenger-car **YoY %**.

    Scores exclude USD column when present (XS sorts are foreign vs USD).
    Score basis = passenger-car registrations YoY %.
    """
    cfg = cfg or PassengerCarsFxConfig()
    panel = cars_panel.copy()
    ...
    # High car-regs YoY → high score → long (growth / durable demand)
    high = foreign.copy()
    if cfg.signal_lag > 0:
        high = high.shift(int(cfg.signal_lag))
    out["high_cars"] = high

    # Low car-regs honesty alternate (demand-stress debtor premium)
    low = (-foreign).copy()
    if cfg.signal_lag > 0:
...
def _us_cars_tilt_returns(
    ...
    *,
    ...
    """Binary USD / FX tilt from lagged US car-regs **YoY** z.

    mode='stress': z ≤ z_low → long foreign (short USD) — US auto demand soft.
...
def passenger_cars_factor_returns(
    pair_ret: pd.DataFrame,
    cars_panel: pd.DataFrame,
    *,
    cfg: PassengerCarsFxConfig | None = None,
    us_cars_override: pd.Series | None = None,
) -> dict[str, pd.Series]:
    """Daily returns for passenger-car-YoY factors + US tilts + EW."""
    cfg = cfg or PassengerCarsFxConfig()
    scores = prepare_passenger_cars_scores(cars_panel, cfg=cfg)
    factors: dict[str, pd.Series] = {}

    if "high_cars" in scores:
        factors["high_cars_xs"] = _scores_to_daily_returns(
            scores["high_cars"], pair_ret, cfg, name="high_cars_xs"
        )
    if "low_cars" in scores:
        factors["low_cars_xs"] = _scores_to_daily_returns(
            scores["low_cars"], pair_ret, cfg, name="low_cars_xs"
        )
    if "high_cars_z" in scores:
        factors["high_cars_z_xs"] = _scores_to_daily_returns(
            scores["high_cars_z"], pair_ret, cfg, name="high_cars_z_xs"
        )
    if "cars_chg" in scores:
        factors["cars_chg_xs"] = _scores_to_daily_returns(
            scores["cars_chg"], pair_ret, cfg, name="cars_chg_xs"
        )

    us = us_cars_override
    if us is None and "USD" in cars_panel.columns:
        us = cars_panel["USD"]
    if us is not None and us.dropna().shape[0] >= cfg.min_periods:
        factors["us_cars_stress_fx"] = _us_cars_tilt_returns(
            us, pair_ret, cfg=cfg, mode="stress", name="us_cars_stress_fx"
        )
        factors["us_cars_haven_usd"] = _us_cars_tilt_returns(
            us, pair_ret, cfg=cfg, mode="haven", name="us_cars_haven_usd"
        )

    blend_keys = [
        k
        for k in ("high_cars_xs", "cars_chg_xs", "us_cars_stress_fx")
        if k in factors
    ]
    if blend_keys:
