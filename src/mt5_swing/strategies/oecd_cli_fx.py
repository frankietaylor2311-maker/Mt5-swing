"""OECD Composite Leading Indicator (CLI) differential FX factors.

Literature framing (see also ``fred_oecd_cli`` docstring)
---------------------------------------------------------
- OECD CLI amplitude-adjusted → leading activity vs coincident IP/UR/CPI.
- Estrella–Mishkin leading indicators; Dahlquist–Hasseltoft-style macro–FX
  differentials applied to *leading* activity.
- Primary: long **high** relative CLI YoY growth / short low.
- Honesty alternate: long **low** CLI growth (stress premium).

Fixed priors (no holdout tuning)
--------------------------------
- ``high_cli_xs`` (**primary**): long high YoY CLI growth / short low.
- ``low_cli_xs``: opposite honesty alternate.
- ``high_cli_z_xs``: long high 60m trailing z of CLI YoY growth.
- ``cli_chg_xs``: long accelerating CLI (Δ12 of YoY) / short decelerating.
- ``us_cli_weak_fx``: depressed US CLI z → long foreign (USD-weak / US soft).
- ``us_cli_haven_usd``: depressed US CLI z → long USD (haven alt).
- ``cli_ew``: EW of ``high_cli_xs``, ``cli_chg_xs``, ``us_cli_weak_fx``.

PIT: loader ``pub_lag_months`` (default 2) + **1 trading-day** weight lag
(no extra month signal lag — fixed a priori for §37). Distinct from macro-diff
CPI/IP/UR, house-price, money-growth, CA/TB, BIS, reserves, IG OAS, equity-diff.
...
@dataclass
class OecdCliFxConfig:
    """Fixed research priors — do not grid / holdout-tune."""

    signal_lag: int = 0  # months; §37 uses 1 trading-day weight lag only
    n_long: int = 2
    n_short: int = 2
    z_window: int = 60  # months for trailing z (5y)
    min_periods: int = 24
    z_low: float = -1.0  # US CLI YoY z ≤ z_low → weak / haven tilt
    usd_tilt: float = 0.5  # gross |w| when tilt is on
    cost_bps_side: float = 1.5
    yoy_periods: int = 12  # YoY diff of CLI level
...
def cli_yoy_diff(panel: pd.DataFrame, *, periods: int = 12) -> pd.DataFrame:
    """YoY difference of CLI levels (≈ YoY change in leading activity)."""
    return panel.diff(int(periods))


def prepare_oecd_cli_scores(
    cli_panel: pd.DataFrame,
    *,
    cfg: OecdCliFxConfig | None = None,
) -> dict[str, pd.DataFrame]:
    """Build signed monthly score panels from PIT CLI levels (already pub-lagged).

    Scores exclude USD column when present (XS sorts are foreign vs USD).
    """
    cfg = cfg or OecdCliFxConfig()
    panel = cli_panel.copy()
    ...
    foreign = cli_yoy_diff(foreign_lvl, periods=cfg.yoy_periods)

    out: dict[str, pd.DataFrame] = {}

    # High CLI YoY → high score → long (leading-activity strength → appreciate)
    high = foreign.copy()
    if cfg.signal_lag > 0:
        high = high.shift(int(cfg.signal_lag))
    out["high_cli"] = high

    # Low CLI honesty alternate
    low = (-foreign).copy()
    if cfg.signal_lag > 0:
...
def _us_cli_tilt_returns(
    ...
    *,
    ...
    """Binary USD / FX tilt from lagged US CLI YoY z.

    mode='weak': z ≤ z_low → long foreign (short USD) — US soft activity.
...
def oecd_cli_factor_returns(
    pair_ret: pd.DataFrame,
    cli_panel: pd.DataFrame,
    *,
    cfg: OecdCliFxConfig | None = None,
    us_cli_override: pd.Series | None = None,
) -> dict[str, pd.Series]:
    """Daily returns for OECD CLI factors + US tilts + EW."""
    cfg = cfg or OecdCliFxConfig()
    scores = prepare_oecd_cli_scores(cli_panel, cfg=cfg)
    factors: dict[str, pd.Series] = {}

    if "high_cli" in scores:
        factors["high_cli_xs"] = _scores_to_daily_returns(
            scores["high_cli"], pair_ret, cfg, name="high_cli_xs"
        )
    if "low_cli" in scores:
        factors["low_cli_xs"] = _scores_to_daily_returns(
            scores["low_cli"], pair_ret, cfg, name="low_cli_xs"
        )
    if "high_cli_z" in scores:
        factors["high_cli_z_xs"] = _scores_to_daily_returns(
            scores["high_cli_z"], pair_ret, cfg, name="high_cli_z_xs"
        )
    if "cli_chg" in scores:
        factors["cli_chg_xs"] = _scores_to_daily_returns(
            scores["cli_chg"], pair_ret, cfg, name="cli_chg_xs"
        )

    us = us_cli_override
    if us is None and "USD" in cli_panel.columns:
        us = cli_panel["USD"]
    if us is not None and us.dropna().shape[0] >= cfg.min_periods + cfg.yoy_periods:
        factors["us_cli_weak_fx"] = _us_cli_tilt_returns(
            us, pair_ret, cfg=cfg, mode="weak", name="us_cli_weak_fx"
        )
        factors["us_cli_haven_usd"] = _us_cli_tilt_returns(
            us, pair_ret, cfg=cfg, mode="haven", name="us_cli_haven_usd"
        )

    blend_keys = [
        k
        for k in ("high_cli_xs", "cli_chg_xs", "us_cli_weak_fx")
        if k in factors
    ]
    if blend_keys:
