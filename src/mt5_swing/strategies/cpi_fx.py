"""Consumer-price (CPI / HICP) differential FX factors (OECD MEI / national).

Literature framing (see also ``fred_cpi`` docstring)
---------------------------------------------------------
- Dahlquist–Hasseltoft (2020) economic momentum: currencies with strong past
  macro trends (incl. CPI) subsequently appreciate.
- Primary: long **high** relative CPI YoY / short low (economic-momentum).
- Honesty alternate: long **low** CPI (purchasing-power / high-inflation short — PPP-style honesty).
- Distinct from PPI §51, IP §42, retail §46, employment §41, ULC §48 / LP §49 / CU §50,
  building-permits §43, CLI/CCI/BCI, macro_diff EW blend, PPP real FX, house-price §35,
  REER §31, money §33, credit §32, DSR §47, WUI, EPU/TPU.

Fixed priors (no holdout tuning)
--------------------------------
- ``high_cpi_xs`` (**primary**): long high YoY CPI / short low
  (n_long=n_short=2 on full G10 foreign panel).
- ``low_cpi_xs``: opposite honesty alternate (PPP / high-inflation short).
- ``high_cpi_z_xs``: long high 60m trailing z of CPI YoY.
- ``cpi_chg_xs``: long accelerating CPI (positive Δ12 of YoY) / short decelerating.
- ``us_cpi_stress_fx``: depressed US CPI YoY z → long foreign (USD soft).
- ``us_cpi_haven_usd``: depressed US CPI YoY z → long USD (haven alt).
- ``cpi_ew``: EW of ``high_cpi_xs``, ``cpi_chg_xs``, ``us_cpi_stress_fx``.

Score basis (fixed a priori): **CPI YoY %** (index→YoY; note JPY free FRED
OECD CPI ends ~2021-06 — hard stale; GBP/CAD/CHF/AUD/NZD mild ~2025).
...
class CpiFxConfig:
    ...
    signal_lag: int = 1  # months; a priori after pub_lag (CPI §52)
    n_long: int = 2  # full G10 foreign panel
    n_short: int = 2
    z_window: int = 60  # months for trailing z (5y)
    min_periods: int = 24
    z_low: float = -1.0  # US CPI YoY z ≤ z_low → stress / haven tilt
...
def prepare_cpi_scores(
    cpi_panel: pd.DataFrame,
    *,
    cfg: CpiFxConfig | None = None,
) -> dict[str, pd.DataFrame]:
    """Build signed monthly score panels from PIT CPI YoY (already pub-lagged).

    Scores exclude USD column when present (XS sorts are foreign vs USD).
    Panel values are already CPI YoY % — used directly (no pct_change).
    """
    cfg = cfg or CpiFxConfig()
    ...
    # High CPI YoY → high score → long (economic-momentum → appreciate)
    high = foreign.copy()
    if cfg.signal_lag > 0:
        high = high.shift(int(cfg.signal_lag))
    out["high_cpi"] = high

    # Low CPI honesty alternate (PPP / high-inflation short)
    low = (-foreign).copy()
    if cfg.signal_lag > 0:
...
def _us_cpi_tilt_returns(
    ...
    *,
    ...
    """Binary USD / FX tilt from lagged US CPI YoY z.

    mode='stress': z ≤ z_low → long foreign (short USD) — US CPI soft.
...
def cpi_factor_returns(
    pair_ret: pd.DataFrame,
    cpi_panel: pd.DataFrame,
    *,
    cfg: CpiFxConfig | None = None,
    us_cpi_override: pd.Series | None = None,
) -> dict[str, pd.Series]:
    """Daily returns for CPI-YoY factors + US tilts + EW."""
    cfg = cfg or CpiFxConfig()
    scores = prepare_cpi_scores(cpi_panel, cfg=cfg)
    factors: dict[str, pd.Series] = {}

    if "high_cpi" in scores:
        factors["high_cpi_xs"] = _scores_to_daily_returns(
            scores["high_cpi"], pair_ret, cfg, name="high_cpi_xs"
        )
    if "low_cpi" in scores:
        factors["low_cpi_xs"] = _scores_to_daily_returns(
            scores["low_cpi"], pair_ret, cfg, name="low_cpi_xs"
        )
    if "high_cpi_z" in scores:
        factors["high_cpi_z_xs"] = _scores_to_daily_returns(
            scores["high_cpi_z"], pair_ret, cfg, name="high_cpi_z_xs"
        )
    if "cpi_chg" in scores:
        factors["cpi_chg_xs"] = _scores_to_daily_returns(
            scores["cpi_chg"], pair_ret, cfg, name="cpi_chg_xs"
        )

    us = us_cpi_override
    if us is None and "USD" in cpi_panel.columns:
        us = cpi_panel["USD"]
    if us is not None and us.dropna().shape[0] >= cfg.min_periods:
        factors["us_cpi_stress_fx"] = _us_cpi_tilt_returns(
            us, pair_ret, cfg=cfg, mode="stress", name="us_cpi_stress_fx"
        )
        factors["us_cpi_haven_usd"] = _us_cpi_tilt_returns(
            us, pair_ret, cfg=cfg, mode="haven", name="us_cpi_haven_usd"
        )

    blend_keys = [
        k
        for k in ("high_cpi_xs", "cpi_chg_xs", "us_cpi_stress_fx")
        if k in factors
    ]
    if blend_keys:
