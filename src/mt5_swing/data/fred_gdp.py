"""FRED / OECD MEI real GDP growth (NAEXKP01) panels.

Literature
----------
- Dahlquist & Hasseltoft (2020, JFE) economic-momentum / growth differentials → FX.
- Classic macro-FX: currencies with *high* relative real GDP growth subsequently
  appreciate vs low-growth peers (growth / economic-momentum prior).
- Distinct from: IP §42, CLI §37, BCI §39, emp persons §41, UR §54, debt/GDP §29,
  CA/GDP §21, emp-rate §57, LFP §56, wages §55.

Free data (verified live on FRED probe 2026-09-24)
-------------------------------------------------
OECD MEI Gross Domestic Product by Expenditure in Constant Prices: Total GDP
— FRED ``NAEXKP01{ISO2}Q657S`` = **Growth rate previous period** (QoQ %, SA).
Panel stores **YoY %** formed by compounding four QoQ observations
(``(∏(1+r_q/100)−1)×100``). Matches US ``GDPC1`` YoY exactly where overlapping.

| Ccy | FRED id (primary)  | Freq | Notes                                         |
|-----|--------------------|------|-----------------------------------------------|
| USD | NAEXKP01USQ657S    | Q    | live ~2026-04; YoY ≡ GDPC1 pct_change(4)      |
| EUR | NAEXKP01DEQ657S    | Q    | **Germany proxy** (EZQ657S stale ~2023-01)    |
| GBP | NAEXKP01GBQ657S    | Q    | live ~2026-04                                 |
| JPY | NAEXKP01JPQ657S    | Q    | live ~2026-04                                 |
| CAD | NAEXKP01CAQ657S    | Q    | live ~2026-04                                 |
| AUD | NAEXKP01AUQ657S    | Q    | live ~2026-04                                 |
| CHF | NAEXKP01CHQ657S    | Q    | live ~2026-04                                 |
| NZD | NAEXKP01NZQ657S    | Q    | live ~2026-01                                 |

Tried / documented (not primary):
- ``NAEXKP01EZQ657S`` — EA YoY-from-QoQ stale ~2023-01 (Germany proxy used).
- ``NAEXKP01*Q652S`` levels — many 404 / stale ~2023-07 (US/GB/CH/EZ only).
- ``NAEXKP01*Q659S`` same-period-previous-year — **404** on FRED.
- ``*LORSGPNOSTSAM`` OECD GDP trend — stale ~2022–2023 (not primary).
- National alts live but not mixed: ``GDPC1``, ``JPNRGDPEXP``,
  ``CLVMNACSCAB1GQEA19``, ``NGDPRSAXDCAUQ`` / ``NGDPRSAXDCGBQ``.

Unit choice (fixed a priori)
----------------------------
Panel stores **real GDP YoY % growth** (compounded from Q657S QoQ). Strategy
...
------------------------------------------------------------------
Quarterly GDP is slow. Default ``pub_lag_months=3``. Observation dated
quarter-start ``t`` is first known at ``t + 3 months``. Quarterly series are
pub-lagged then forward-filled to month-start (no intra-quarter leak).
...
# Currency → FRED OECD MEI real GDP QoQ growth (NAEXKP01 *Q657S)
GDP_SERIES: dict[str, str | None] = {
    "USD": "NAEXKP01USQ657S",
    "EUR": "NAEXKP01DEQ657S",  # Germany proxy; EZQ657S stale ~2023-01
    "GBP": "NAEXKP01GBQ657S",
    "JPY": "NAEXKP01JPQ657S",
    "CAD": "NAEXKP01CAQ657S",
    "AUD": "NAEXKP01AUQ657S",
...
GDP_NATIVE_UNIT = "qoq_pct_q657s"
GDP_SCORE_UNIT = "yoy_pct_compounded"

GDP_FAILED_OR_ALT: dict[str, list[str]] = {
    "eur_ez_stale_germany_proxy": [
        "NAEXKP01EZQ657S",  # stale ~2023-01
        "NAEXKP01DEQ657S",  # Germany proxy (primary)
    ],
    "q659s_same_period_yoy_404": [
        "NAEXKP01USQ659S",
        "NAEXKP01GBQ659S",
        "NAEXKP01DEQ659S",
    ],
    "q652s_levels_partial_stale_or_404": [
        "NAEXKP01USQ652S",  # live but ends ~2023-07
        "NAEXKP01EZQ652S",  # stale ~2023-01
        "NAEXKP01GBQ652S",
        "NAEXKP01CHQ652S",
        "NAEXKP01DEQ652S",  # 404
        "NAEXKP01JPQ652S",  # 404
    ...
    "lorsgpnostsam_oecd_trend_stale": [
        "USALORSGPNOSTSAM",
        "DEULORSGPNOSTSAM",
        "EA19LORSGPNOSTSAM",
    ],
    "national_real_gdp_alts_not_mixed": [
        "GDPC1",
        "JPNRGDPEXP",
        "CLVMNACSCAB1GQEA19",
        "NGDPRSAXDCAUQ",
    ...
    "distinct_channels_not_this_wave": [
        ...
        "LFEMTTTTUSM647S",  # emp persons §41
        "LRHUTTTTUSM156S",  # UR §54
        "GGGDTAUSA188N",  # debt/GDP §29
        "USB6BLTT02STSAQ",  # CA/GDP §21
    ],
}

EUR_EZ_ALT_STALE = "NAEXKP01EZQ657S"
US_GDPC1_ALT = "GDPC1"

DEFAULT_PUB_LAG_MONTHS = 3  # a priori: quarterly GDP is slow
...
def _qoq_pct_to_yoy(qoq_pct: pd.Series) -> pd.Series:
    """Compound four QoQ % observations → YoY % (matches GDPC1 pct_change(4)*100)."""
...
def load_gdp_panel(
    ...
    *,
    ...
    """OECD MEI real GDP YoY % (from Q657S QoQ) → monthly PIT panel.

    Columns = ISO currency codes. Values = real GDP **YoY % growth**.
    ...
    """
    curs = [c.upper() for c in (currencies or list(GDP_SERIES.keys()))]
    ...
    for c in curs:
        sid = GDP_SERIES.get(c)
        if sid is None:
            notes[c] = "unmapped_on_fred_naexkp01"
            continue
        freq = GDP_FREQ.get(c, "Q")
        try:
            ...
            if c == "EUR":
                ...
                notes[c] = f"OECD MEI real GDP {sid} QoQ→YoY %"
            series_map[c] = sid
            freq_map[c] = freq
            quarterly_cols[c] = s
    ...
    stale_note_parts = []
    if "EUR" in series_map and series_map["EUR"] == "NAEXKP01DEQ657S":
        stale_note_parts.append(
            "EUR EA NAEXKP01EZQ657S stale ~2023-01 — Germany DEQ657S proxy used"
        )

    df.attrs["factor"] = "oecd_mei_real_gdp_growth_naexkp01"
    df.attrs["pub_lag_months"] = int(pub_lag_months)
    df.attrs["series_map"] = series_map
    df.attrs["freq_map"] = freq_map
    df.attrs["notes"] = notes
    df.attrs["source"] = (
        "FRED OECD MEI NAEXKP01*Q657S real GDP growth rate previous period (QoQ %) "
        "→ compounded to YoY % "
        "(USD/GBP/JPY/CAD/AUD/CHF/NZD *Q657S; "
        "EUR=NAEXKP01DEQ657S Germany proxy — EZQ657S stale ~2023-01; "
        "full G10 mapped; Q659S 404; Q652S levels partial stale/404; "
        "GDPC1/JPNRGDPEXP/EA19 CLVM national alts documented not mixed)"
    )
    df.attrs["unit"] = GDP_SCORE_UNIT
    df.attrs["native_unit"] = GDP_NATIVE_UNIT
    df.attrs["score_basis"] = "gdp_yoy_pct"
    df.attrs["unmapped"] = [c for c in curs if c not in series_map]
    df.attrs["failed_or_alt"] = GDP_FAILED_OR_ALT
    df.attrs["stale"] = bool(
        "EUR" in series_map and series_map.get("EUR") == "NAEXKP01DEQ657S"
    )
    df.attrs["stale_note"] = (
...
def load_us_gdp(
    *,
    ...
    """US real GDP YoY % (from NAEXKP01USQ657S), PIT-lagged."""
    panel = load_gdp_panel(
        ["USD"],
    ...
    if "USD" not in panel.columns:
        raise RuntimeError("US GDP series failed to load")
    out = panel["USD"].dropna()
    out.name = "US_GDP_YOY"
    out.attrs["series_id"] = GDP_SERIES["USD"]
    out.attrs["pub_lag_months"] = int(pub_lag_months)
    return out


def gdp_coverage(panel: pd.DataFrame | None = None) -> pd.DataFrame:
    """Per-currency coverage summary for reports."""
    if panel is None:
        panel = load_gdp_panel(download=False)
    rows = []
        ...
        rows.append(
            {
                ...
                "end": None,
                "last": float("nan"),
                "note": notes.get(c, "unmapped_on_fred_naexkp01"),
            }
        )
