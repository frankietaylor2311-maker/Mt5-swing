"""FRED / OECD MEI employment-rate (LREM64TT) panels.

Literature
----------
- Dahlquist & Hasseltoft (2020, JFE) economic-momentum labour channel companion —
  employment **rate** (employed/population ages 15–64) as the labour-engagement
  **stock** between participation and joblessness (Dahlquist–Hasseltoft labour-strength prior).
- Primary prior: currencies with *high* relative employment rate
  subsequently appreciate vs low-ER peers. Honesty alternate: long low ER /
  labour-disengagement debtor premium.
- Distinct from: employment persons §41 (LFEMTTTT YoY), UR levels §54
  (LRHUTTTT), LFP/activity §56 (LRAC64TT), wages §55 (LCEAMN), ULC/LP/CU.

Free data (verified live on FRED probe 2026-09-24)
-------------------------------------------------
Employment rate ages 15–64 (%), OECD MEI / national:

| Ccy | FRED id            | Freq | Notes                                      |
|-----|--------------------|------|--------------------------------------------|
| USD | LREM64TTUSM156S    | M    | live ~2026-07                              |
| JPY | LREM64TTJPM156S    | M    | live ~2026-07                              |
| CAD | LREM64TTCAM156S    | M    | live ~2026-08                              |
| AUD | LREM64TTAUM156S    | M    | live ~2026-07                              |
| GBP | LREM64TTGBQ156S    | Q    | live ~2026-04                              |
| EUR | LREM64TTDEQ156S    | Q    | **Germany proxy** (EZQ156S stale ~2022-10) |
| CHF | LREM64TTCHQ156S    | Q    | live ~2026-04                              |
| NZD | LREM64TTNZQ156S    | Q    | live ~2026-04                              |

Tried / documented (not this wave):
- ``LREM64TTEZQ156S`` — EA stale ~2022-10 (Germany proxy used instead).
...
----------------------------
Panel stores **employment-rate levels (%)**. Strategy scores XS-rank on
levels (not YoY of persons). Rising employment rate = +Δ12 of the level.

Publication lag (conservative, fixed a priori — not holdout-tuned)
------------------------------------------------------------------
Employment-rate / labour stats lag similar to other labour releases. Default
...
# Currency → FRED OECD MEI employment rate 15–64 (LREM64TT)
ER_SERIES: dict[str, str | None] = {
    "USD": "LREM64TTUSM156S",
    "JPY": "LREM64TTJPM156S",
    "CAD": "LREM64TTCAM156S",
    "AUD": "LREM64TTAUM156S",
...
ER_FAILED_OR_ALT: dict[str, list[str]] = {
    "eur_ez_stale_germany_proxy": [
        "LREM64TTEZQ156S",  # stale ~2022-10
        "LREM64TTDEQ156S",  # Germany proxy (primary)
    ],
    ...
    "lfp_activity_lrac64tt_distinct_56": [
        "LRAC64TTUSM156S",
        "LRAC64TTDEQ156S",
    ],
    "emratio_ce16ov_us_only_not_this_wave": [
        "EMRATIO",
        "CE16OV",
    ],
}

EUR_EZ_ALT_STALE = "LREM64TTEZQ156S"

DEFAULT_PUB_LAG_MONTHS = 2  # a priori: labour / employment-rate lag; mixed M+Q
...
def load_employment_rate_panel(
    ...
    *,
    ...
    """OECD MEI employment-rate levels (%) → monthly PIT panel.

    Columns = ISO currency codes. Values = employment rate **levels (%)**.
    ...
    """
    curs = [c.upper() for c in (currencies or list(ER_SERIES.keys()))]
    ...
    for c in curs:
        sid = ER_SERIES.get(c)
        if sid is None:
            notes[c] = "unmapped_on_fred_lrem64tt"
            continue
        freq = ER_FREQ.get(c, "M")
        try:
            ...
            if c == "EUR":
                ...
                notes[c] = f"OECD MEI employment rate {sid} quarterly levels %"
            else:
                notes[c] = f"OECD MEI employment rate {sid} monthly levels %"
            series_map[c] = sid
            freq_map[c] = freq
            if freq == "Q":
                quarterly_cols[c] = s
    ...
    stale_note_parts = []
    if "EUR" in series_map and series_map["EUR"] == "LREM64TTDEQ156S":
        stale_note_parts.append(
            "EUR EA LREM64TTEZQ156S stale ~2022-10 — Germany DEQ156S proxy used"
        )

    df.attrs["factor"] = "oecd_mei_employment_rate_lrem64tt"
    ...
    df.attrs["source"] = (
        ...
        "EUR=LREM64TTDEQ156S Germany proxy — EZQ156S stale ~2022-10; "
        "full G10 mapped; EMRATIO/CE16OV US-only + vacancies/hours alts documented not boarded)"
    )
    df.attrs["unit"] = "employment_rate_pct_level"
    df.attrs["score_basis"] = "er_level_pct"
    df.attrs["unmapped"] = [c for c in curs if c not in series_map]
    df.attrs["failed_or_alt"] = ER_FAILED_OR_ALT
    df.attrs["stale"] = bool(
        "EUR" in series_map and series_map.get("EUR") == "LREM64TTDEQ156S"
    )
    df.attrs["stale_note"] = (
...
def load_us_employment_rate(
    *,
    pub_lag_months: int = DEFAULT_PUB_LAG_MONTHS,
    download: bool = True,
    force: bool = False,
) -> pd.Series:
    """US employment rate 15–64 (LREM64TTUSM156S), PIT-lagged."""
    panel = load_employment_rate_panel(
        ["USD"],
    ...
    if "USD" not in panel.columns:
        raise RuntimeError("US employment-rate series failed to load")
    out = panel["USD"].dropna()
    out.name = "US_ER"
    out.attrs["series_id"] = ER_SERIES["USD"]
    out.attrs["pub_lag_months"] = int(pub_lag_months)
    return out


def employment_rate_coverage(panel: pd.DataFrame | None = None) -> pd.DataFrame:
    """Per-currency coverage summary for reports."""
    if panel is None:
        panel = load_employment_rate_panel(download=False)
    rows = []
        ...
        rows.append(
            {
                ...
                "end": None,
                "last": float("nan"),
                "note": notes.get(c, "unmapped_on_fred_lrem64tt"),
            }
        )
