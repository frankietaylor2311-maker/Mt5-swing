"""FRED / OECD MEI merchandise import-value YoY (XTIMVA01) panels.

Literature
----------
- Dahlquist & Hasseltoft (2020, JFE) economic-momentum / growth-channel spirit
  applied to **domestic absorption / import demand** (trade intensity).
- Currencies with *high* relative merchandise import-value YoY subsequently
  appreciate vs low-import-growth peers (domestic-absorption / import-demand prior).
- Distinct from: export-value §61 (XTEXVA01 external demand), trade-balance §30
  (TB/exports ratio), CA §21, ToT (UV 404), retail §46, IP §42, passenger-cars §60,
  construction §59, GDP §58.

Fall-through (§62 probe order, 2026-09-24)
-----------------------------------------
...
4. **This wave:** OECD MEI merchandise import-value YoY ``XTIMVA01{ISO2}M657S``
   (mirrors export-value §61 XTEXVA01; domestic absorption vs external demand).

Free data (verified live on FRED probe 2026-09-24)
-------------------------------------------------
| Ccy | FRED id (primary)  | Freq | Unit / notes                                      |
|-----|--------------------|------|---------------------------------------------------|
| USD | XTIMVA01USM657S    | M    | YoY % as reported (~2026-06)                      |
| EUR | XTIMVA01DEM657S    | M    | **Germany proxy** (EZM657S 404; EZM667S stale)    |
| GBP | XTIMVA01GBM657S    | M    | YoY %                                             |
| JPY | XTIMVA01JPM657S    | M    | YoY %                                             |
| CAD | XTIMVA01CAM657S    | M    | YoY %                                             |
| AUD | XTIMVA01AUM657S    | M    | YoY %                                             |
| CHF | XTIMVA01CHM657S    | M    | YoY %                                             |
| NZD | XTIMVA01NZM657S    | M    | YoY %                                             |

Full G10 mapped. Companion levels ``*M667S`` exist (import **value** in currency
units — NOT volumes; do **not** mix levels into score panel; YoY 657S is primary).

Tried / documented (not primary):
- ``XTIMVA01EZM657S`` — **404** → Germany proxy a priori.
- ``XTIMVA01EZM667S`` — EA levels stale ~2023-04 (not mixed into YoY panel).
- ``XTIMVA01*M667S`` levels — value units, not volumes; not mixed into scores.
...
----------------------------
Panel stores **merchandise import-value YoY %** as reported by OECD M657S.
...
------------------------------------------------------------------
Trade / import-value typically lands ~1–2 months after reference (same band as
...
# Currency → FRED OECD MEI merchandise import-value YoY (XTIMVA01 *M657S)
IVAL_SERIES: dict[str, str | None] = {
    "USD": "XTIMVA01USM657S",
    "EUR": "XTIMVA01DEM657S",  # Germany proxy; EZM657S 404; EZM667S stale ~2023-04
    "GBP": "XTIMVA01GBM657S",
    "JPY": "XTIMVA01JPM657S",
    "CAD": "XTIMVA01CAM657S",
    "AUD": "XTIMVA01AUM657S",
...
IVAL_SCORE_UNIT = "yoy_pct"

IVAL_FAILED_OR_ALT: dict[str, list[str]] = {
    ...
    "eur_ez_m657s_404_germany_proxy": [
        "XTIMVA01EZM657S",  # 404
        "XTIMVA01DEM657S",  # Germany proxy (primary)
    ],
    "eur_ez_m667s_stale_2023_04": ["XTIMVA01EZM667S"],
    ...
    "distinct_channels_not_this_wave": [
        "XTEXVA01USM657S",  # export-value §61
...
EUR_GERMANY_PROXY = "XTIMVA01DEM657S"
EUR_EZ_M657S_404 = "XTIMVA01EZM657S"
EUR_EZ_M667S_STALE = "XTIMVA01EZM667S"
...
def load_import_value_panel(
    ...
    *,
    ...
    """OECD MEI merchandise import-value YoY % → monthly PIT panel.

    Columns = ISO currency codes. Values = import-value **YoY %** (value, not
    volume). Index = UTC month-start of the PIT *known* date after
    ``pub_lag_months``. Full G10 mapped; EUR = Germany proxy.
    """
    curs = [c.upper() for c in (currencies or list(IVAL_SERIES.keys()))]
    ...
    for c in curs:
        sid = IVAL_SERIES.get(c)
        if sid is None:
            notes[c] = "unmapped_on_fred_xtimva01"
            continue
        try:
            s = _load_series(sid, download=download, force=force)
            series_map[c] = sid
            freq_map[c] = IVAL_FREQ.get(c, "M")
            native_map[c] = IVAL_NATIVE_UNIT.get(c, "yoy_pct")
            if c == "EUR":
                notes[c] = (
                ...
                notes[c] = f"OECD MEI merchandise import-value YoY {sid} (monthly; value not volume)"
            monthly_cols[c] = s
    ...
    stale_note_parts = [
        "XTIMVA01EZM657S 404; XTIMVA01EZM667S stale ~2023-04",
        "EUR=XTIMVA01DEM657S Germany proxy a priori",
        "series unit = import VALUE YoY (not volume); *M667S levels not mixed",
    ...
    df.attrs["factor"] = "oecd_mei_merchandise_import_value_xtimva01"
    ...
    df.attrs["source"] = (
        "FRED OECD MEI XTIMVA01{ISO2}M657S merchandise import-value YoY "
        "(full G10; EUR=XTIMVA01DEM657S Germany proxy — EZM657S 404, "
    ...
    df.attrs["unit"] = IVAL_SCORE_UNIT
    df.attrs["score_basis"] = "import_value_yoy_pct"
    df.attrs["unmapped"] = [c for c in curs if c not in series_map]
    df.attrs["failed_or_alt"] = IVAL_FAILED_OR_ALT
...
def load_us_import_value(
    *,
    pub_lag_months: int = DEFAULT_PUB_LAG_MONTHS,
    download: bool = True,
    force: bool = False,
) -> pd.Series:
    """US merchandise import-value YoY % (XTIMVA01USM657S), PIT-lagged."""
    panel = load_import_value_panel(
        ["USD"],
    ...
    if "USD" not in panel.columns:
        raise RuntimeError("US merchandise import-value series failed to load")
    out = panel["USD"].dropna()
    out.name = "US_IVAL_YOY"
    out.attrs["series_id"] = IVAL_SERIES["USD"]
    out.attrs["pub_lag_months"] = int(pub_lag_months)
    return out


def import_value_coverage(panel: pd.DataFrame | None = None) -> pd.DataFrame:
    """Per-currency coverage summary for reports."""
    if panel is None:
        panel = load_import_value_panel(download=False)
    rows = []
        ...
        rows.append(
            {
                ...
                "end": None,
                "last": float("nan"),
                "note": notes.get(c, "unmapped_on_fred_xtimva01"),
            }
        )
